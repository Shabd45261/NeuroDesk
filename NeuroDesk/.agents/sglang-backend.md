...
The canonical reference is `python/sglang/srt/server_args.py:ServerArgs` (line ~304). When SGLang adds new flags, no NeuroDesk code change is needed — they're automatically available via `engine_args:`. The validator picks them up because it introspects the live dataclass.
...
ServerArgs has `tool_call_parser` and `reasoning_parser` fields, and the backend does pass them through to `Engine` so SGLang's own HTTP/OAI surface keeps working. But for the **NeuroDesk** request path the backend constructs fresh per-request parser instances in `_make_parsers` (`backend.py:286`) because the parsers are stateful — the streaming and non-streaming paths each need their own.
...
Putting these in `engine_args:` will set them on `ServerArgs` but the NeuroDesk-level streaming `ChatDelta` will not pick them up. Don't recommend that path.
...
- `core/gallery/importers/sglang.go` — vLLM has an importer that resolves model architecture → parser defaults at gallery-import time. A matching importer for SGLang would let `neurodesk install` populate sensible parsers automatically.
