...
- **Installed directory**: `backends/<name>/` — this is what NeuroDesk actually runs. It is populated by `make backends/<name>` which builds a Docker image, exports it, and installs it via `neurodesk backends install`.
...
Backend gRPC processes log to NeuroDesk's stdout/stderr. Look for lines tagged with the backend's model ID:
...
This runs the Docker build (`Dockerfile.python`), exports the image to `backend-images/<name>.tar`, and installs it into `backends/<name>/`. It also rebuilds the `neurodesk` Go binary (without extra tags).
...
After rebuilding a backend, you must restart NeuroDesk for it to pick up the new backend files. The backend gRPC process is spawned on demand when the model is first loaded.
...
# Restart
./neurodesk run --debug [your flags]
```
...
# Restart NeuroDesk to respawn the gRPC process
```
...
1. Start NeuroDesk and confirm the backend registers: look for `Registering backend name="<name>"` in the logs
