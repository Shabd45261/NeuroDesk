...
When adding a new backend to NeuroDesk, you need to update several files to ensure the backend is properly built, tested, and registered. Here's a step-by-step guide based on the pattern used for adding backends like `moonshine`:
...
and NeuroDesk picks one per host: it drops builds whose backend cannot run here or
...
- **`docs/content/features/backends.md`** - add the backend to the right
  category in the "NeuroDesk supports various types of backends" list (and add a
...
NeuroDesk team** (a from-scratch port like `parakeet.cpp`, `ced.cpp`,
...
engines ... developed and maintained by the NeuroDesk project itself". Add a row
...
CID=$(docker create --entrypoint=/run.sh neurodesk-backend:<backend>)
...
3. **If your backend has no reliable auto-detect signal** (preference-only — e.g. `sglang`, `tinygrad`, `whisperx`):
   - Do NOT create an importer. Instead add the backend name to the curated pref-only slice in `core/http/endpoints/neurodesk/backend.go` that feeds `/backends/known`. A single line addition.
