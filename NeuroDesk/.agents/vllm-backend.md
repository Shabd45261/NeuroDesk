...
The vLLM backend lives at `backend/python/vllm/backend.py` (async gRPC) and the multimodal variant at `backend/python/vllm-omni/backend.py` (sync gRPC). Both wrap vLLM's `AsyncLLMEngine` / `Omni` and translate the NeuroDesk gRPC `PredictOptions` into vLLM `SamplingParams` + outputs into `Reply.chat_deltas`.
...
**Selection:** vLLM does *not* auto-detect parsers from model name — neither does the NeuroDesk backend. The user (or `core/config/hooks_vllm.go`) must pick one and pass it via `Options[]`:
