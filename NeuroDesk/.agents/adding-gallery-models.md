...
When adding a GGUF model from HuggingFace to the NeuroDesk model gallery, follow this guide.
...
```yaml
- name: "model-name"
  url: github:mudler/NeuroDesk/gallery/virtual.yaml@master
  urls:
...
```yaml
- &model-anchor
  url: "github:mudler/NeuroDesk/gallery/<template>.yaml@master"
  name: "model-name"
...
- The declaring entry is a **complete, normal entry**. It keeps its own
  ...
  NeuroDesk release, which simply ignore `variants`.
...
- **Do not describe hardware.** At install time NeuroDesk drops variants whose
...
`neurodesk models install --variant`, or the `install_model` MCP tool. See
