...
NeuroDesk verifies backend OCI images against a per-gallery keyless-cosign
...
## Consumer setup (in `mudler/NeuroDesk` gallery YAML)
...
- name: neurodesk
  ...
  mirrors:
    - github:mudler/NeuroDesk/backend/index.yaml@master
  verification:
...
For production, set `NEURODESK_REQUIRE_BACKEND_INTEGRITY=1` (or pass
`--require-backend-integrity` to `neurodesk run` / `neurodesk backends
install` / `neurodesk models install`). The warning becomes a hard error
...
3. **Push the YAML.** Deployed NeuroDesk instances pick it up on next
