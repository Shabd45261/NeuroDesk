...
## Testing in NeuroDesk's UI

1. Make sure NeuroDesk is running (e.g. `http://localhost:8080`)
...
5. Add a new client MCP server:
   ...
   - **Use CORS proxy**: enabled (default) — required because the browser can't hit `localhost:3001` directly due to CORS; NeuroDesk's proxy at `/api/cors-proxy` handles it
