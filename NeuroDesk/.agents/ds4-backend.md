...
NeuroDesk wraps the engine's C API (`ds4/ds4.h`) with a fresh C++ gRPC server at
...
- **Worker CLI**: `neurodesk worker ds4-distributed -- <ds4-worker args>` resolves the
...
ds4 is also listed in `core/http/endpoints/neurodesk/backend.go`'s pref-only
