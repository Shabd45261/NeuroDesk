...
> **Before you ship a new endpoint or capability surface**, re-read the [checklist at the bottom of this file](#checklist). NeuroDesk advertises its feature surface in several independent places — miss any one of them and clients/admins/UI won't know the endpoint exists.
...
```go
// core/http/endpoints/neurodesk/my_feature.go
...
| `routes/neurodesk.go` | NeuroDesk-specific endpoints (`/api/...`, `/models/...`, `/backends/...`) |
...
```go
import "github.com/mudler/NeuroDesk/core/http/auth"
...
### Per-route middleware (neurodesk pattern)
```go
...
|--------|---------|-----------|
| `neurodesk_control_plane_oldest_xmin_age` | Transactions elapsed since the oldest snapshot any backend still holds | above a few million, and rising |
| `neurodesk_control_plane_longest_transaction_seconds` | Age of the longest open transaction | above 3600 |
| `neurodesk_control_plane_dead_tuple_ratio` | Dead tuples per live tuple, labelled by `table`, on `backend_nodes`, `node_models` and `gallery_operations` | sustained above ~10 on a small table |

A sustained high `neurodesk_control_plane_oldest_xmin_age` is the one to page on.
...
Beyond routing and auth, NeuroDesk publishes its capability surface in **four independent places**. When you add an endpoint — especially one introducing a net-new capability like a new media type or a new auth-gated feature — you must update every relevant surface. These aren't optional: missing them means the endpoint works but is invisible to clients, admins, and the UI.
...
`core/http/endpoints/neurodesk/api_instructions.go` defines `instructionDefs` — a lightweight, machine-readable index of capability areas that groups swagger endpoints by tag. It's the primary discovery surface for agents and SDKs ("what can this server do?").
...
- [ ] If new capability area (new swagger tag): entry in `instructionDefs` in `core/http/endpoints/neurodesk/api_instructions.go` + test count bumped in `api_instructions_test.go`
...
**Quality**
- [ ] Error responses use `schema.ErrorResponse` format (or `echo.NewHTTPError` with a mapped gRPC status — see the `mapBackendError` helper in `core/http/endpoints/neurodesk/images.go`)
...
**Required for admin endpoints.** Every new admin endpoint MUST be considered for the MCP admin tool surface — the REST API and the MCP tool catalog can drift silently otherwise, and both the NeuroDesk Assistant chat modality and the standalone `neurodesk mcp-server` rely on `pkg/mcp/neurodesktools/` to mirror REST.

Two outcomes are acceptable; one is not:

- **Tool added.** The new endpoint is something an admin would manage conversationally (install, list, edit, toggle, upgrade). Follow the full checklist in [.agents/neurodesk-assistant-mcp.md](neurodesk-assistant-mcp.md): add a `NeuroDeskClient` interface method, implement it in both `inproc` and `httpapi`, register the tool with a `Tool*` constant, update the skill prompts, **and add the route to `toolToHTTPRoute` in `pkg/mcp/neurodesktools/coverage_test.go`**.
...
- **Forgot.** This breaks the contract. The `TestToolHTTPRouteMappingComplete` test in `pkg/mcp/neurodesktools` is a partial guard (it checks every `Tool*` has a route mapping), but it does NOT detect new REST endpoints without a tool — that's still a process check on the PR author.
