...
// Required (one of):
//
//	BACKEND_IMAGE            Docker image tag to test (e.g. neurodesk-backend:llama-cpp).
//
...
var _ = Describe("Backend container", Ordered, func() {
	...
	// architecture. NeuroDesk built ds4's CUDA objects with no -arch/-gencode, so
	...
	It("analyzes voice via VoiceAnalyze", func() {
		...
		for _, s := range res.GetSegments() {
			...
			// NeuroDesk capitalises those to Female / Male / Child. Custom
...
// coordinator (NeuroDesk's ds4 backend) listens and owns a low layer slice; one
...
// paths, or the `neurodesk worker ds4-distributed` CLI wrapper (that resolves
