...
import (
	...
	neurodeskapp "github.com/mudler/NeuroDesk/core/application"
	"github.com/mudler/NeuroDesk/core/config"
	httpapi "github.com/mudler/NeuroDesk/core/http"
...
var (
	...
	neuroDeskApp        *neurodeskapp.Application
...
var _ = BeforeSuite(func() {
	...
	// Classifier-mode pipeline (NeuroDesk extension): responses are
	...
	neuroDeskApp, err = neurodeskapp.New(
	...
	neuroDeskApp.ModelLoader().SetExternalBackend("mock-backend", mockBackendPath)
	neuroDeskApp.ModelLoader().SetExternalBackend("opus", mockBackendPath)
	if cloudProxyPath != "" {
		neuroDeskApp.ModelLoader().SetExternalBackend("cloud-proxy", cloudProxyPath)
	}

	// Create HTTP app
	app, err = httpapi.API(neuroDeskApp)
	Expect(err).ToNot(HaveOccurred())
...
var _ = AfterSuite(func() {
	...
	if neuroDeskApp != nil {
		if err := neuroDeskApp.Shutdown(); err != nil {
...
func TestNeuroDesk(t *testing.T) {
	RegisterFailHandler(Fail)
	RunSpecs(t, "NeuroDesk E2E test suite")
}
