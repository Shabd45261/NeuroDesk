...
#   - `make docker-build-sherpa-onnx` has produced neurodesk-backend:sherpa-onnx
...
IMAGE=${BACKEND_IMAGE:-neurodesk-backend:sherpa-onnx}
...
WORK_DIR=${WORK_DIR:-$(mktemp -d -t neurodesk-sherpa-realtime.XXXXXX)}
if [[ -z "${KEEP_WORK:-}" ]]; then
