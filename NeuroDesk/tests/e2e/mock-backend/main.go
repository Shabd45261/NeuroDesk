	...
	if strings.Contains(in.Prompt, "AUTOPARSER_CONTENT") {
		return &pb.Reply{
			...
			ChatDeltas: []*pb.ChatDelta{
				...
				{Content: "NeuroDesk is an open-source AI platform."},
			},
		}, nil
	}
...
func main() {
	xlog.SetLogger(xlog.NewLogger(xlog.LogLevel(os.Getenv("NEURODESK_LOG_LEVEL")), os.Getenv("NEURODESK_LOG_FORMAT")))
