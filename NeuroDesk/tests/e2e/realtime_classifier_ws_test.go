...
// Classifier mode (NeuroDesk extension): the pipeline scores each user turn
...
var _ = Describe("Realtime classifier mode", Label("Realtime"), func() {
	...
	It("echoes the classifier config in session.created", func() {
		...
		Expect(ok).To(BeTrue())
		classifier, ok := session["neurodesk_classifier"].(map[string]any)
		Expect(ok).To(BeTrue(), "session.created should carry neurodesk_classifier")
	...
	It("picks the hinted option and emits its canned reply and tool call", func() {
		...
		result := drainUntil(c, "neurodesk.classifier.result", 30*time.Second)
	...
	It("honors a client-pushed option list via session.update", func() {
		...
		// neurodesk_classifier, then a turn that hints the NEW option.
		c := openSession()
		defer func() { _ = c.Close() }()
		...
		sendClientEvent(c, map[string]any{
			...
			"session": map[string]any{
				"type": "realtime",
				"neurodesk_classifier": map[string]any{
					"enabled":   true,
					"threshold": 0.6,
	...
	It("runs normal generation when the response disables the classifier", func() {
		...
		sendClientEvent(c, map[string]any{
			...
			"response": map[string]any{
				"output_modalities":  []string{"text"},
				"neurodesk_classifier": map[string]any{"enabled": false},
			},
		})
		...
		for time.Now().Before(deadline) {
			...
			switch evt["type"] {
			case "neurodesk.classifier.result":
				sawClassifier = true
