...
// Backend worker processes (the per-model gRPC servers NeuroDesk spawns) are
// deliberately placed in their own process group by the process manager so
// NeuroDesk's graceful shutdown can signal the whole group. That graceful path
// (SIGTERM -> grace -> SIGKILL, driven by pkg/signals + pkg/model) only runs
// when NeuroDesk itself receives a catchable signal and lives long enough to run
// its handlers. If NeuroDesk is SIGKILLed (e.g. a supervising process's
...
const (
	...
	EnvBackendParentWatch = "NEURODESK_BACKEND_PARENT_WATCH"
	...
	EnvBackendParentWatchInterval = "NEURODESK_BACKEND_PARENT_WATCH_INTERVAL"
...
// signal that the original parent (here, the NeuroDesk process that spawned this
...
// replacement for — NeuroDesk's graceful SIGTERM->grace->SIGKILL teardown.
