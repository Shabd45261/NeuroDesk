...
func (c Config) Validate() error {
	...
	if c.ServiceUserJWT == "" || c.ServiceUserSeed == "" {
		return fmt.Errorf("NEURODESK_NATS_REQUIRE_AUTH requires NEURODESK_NATS_SERVICE_JWT and NEURODESK_NATS_SERVICE_SEED")
	}
	if c.AccountSeed == "" {
		return fmt.Errorf("NEURODESK_NATS_REQUIRE_AUTH is set but NEURODESK_NATS_ACCOUNT_SEED is empty")
	}
	return nil
}
...
func (c Config) WarnIfInsecure(distributed bool) {
	...
	xlog.Warn("NATS is used without JWT credentials — any client on the bus can publish backend.install. " +
		"Set NEURODESK_NATS_ACCOUNT_SEED + NEURODESK_NATS_SERVICE_JWT (see docs/features/distributed-mode.md).")
}
