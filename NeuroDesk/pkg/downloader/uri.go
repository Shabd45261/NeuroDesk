...
import (
	...
	"github.com/mudler/NeuroDesk/pkg/xio"
)
...
func (uri URI) DownloadFileWithContext(ctx context.Context, filePath, sha string, fileN, total int, downloadStatus func(string, string, string, float64), opts ...DownloadOption) error {
	...
	// Release the .partial before promoting it to the final path. The writer
	// was opened without FILE_SHARE_DELETE (Go's os.OpenFile does not request
	// it on Windows), so a deferred Close here would run *after* os.Rename and
	// the rename would fail on every Windows install: an open handle is a lock
	// on the source file. Close first, then move.
	if err := outFile.Close(); err != nil {
		return fmt.Errorf("failed to close temporary file %q: %v", tmpFilePath, err)
	}

	if err := promotePartialFile(tmpFilePath, filePath); err != nil {
...
// promotePartialFile moves the verified .partial onto its final path. The
// caller must have finished writing and closed the .partial first. On
// Windows the destination can briefly be locked — the antivirus scans a
// freshly-written multi-GB blob, or another reader samples it — so a bounded
// retry rides out the transient lock. If it still refuses and a stale
// destination is in the way, pushing that stale file aside and retrying is
// safe: the caller verified the source checksum against the metadata SHA
// before this call, so replacing whatever is there with the verified blob
// cannot corrupt the install.
func promotePartialFile(tmpFilePath, filePath string) error {
	const maxAttempts = 5
	for attempt := 0; ; attempt++ {
		...
		// Fixed short backoff between attempts — long enough for a scanner
		// pass to release its handle, short enough not to stall installs.
		time.Sleep(time.Duration((attempt+1)*250) * time.Millisecond)
		if runtime.GOOS == "windows" {
			// Go's windows rename uses MOVEFILE_REPLACE_EXISTING, so a plain
			// destination file is no obstacle; refusal here means a lock or a
			// stale leftover from an earlier interrupted attempt. Clearing the
			// destination and trying again costs nothing when the source is
			// verified. If the destination is itself still open elsewhere,
			// os.Remove fails too and the next attempt re-raises the rename
			// error.
