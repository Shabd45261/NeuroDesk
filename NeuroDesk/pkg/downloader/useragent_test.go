...
func expectUserAgent(site, got string) {
	...
	Expect(got).To(ContainSubstring("NeuroDesk/v9.9.9"), "%s: User-Agent does not name the build version", site)
}
...
var _ = Describe("the outbound User-Agent", func() {
	...
	// Model files are the bulk of what NeuroDesk pulls; they go through
