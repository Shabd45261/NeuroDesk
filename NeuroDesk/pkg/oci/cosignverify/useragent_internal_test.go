...
var _ = Describe("registry User-Agent", func() {
	It("identifies NeuroDesk during signature verification requests", func() {
		...
		ref, err := name.ParseReference("registry.example.com/neurodesk/backend:latest")
		Expect(err).NotTo(HaveOccurred())
		_, err = remote.Head(ref, verifier.remoteOptions(context.Background())...)
		Expect(err).NotTo(HaveOccurred())
		Expect(userAgent).To(ContainSubstring("NeuroDesk/v-test"))
	})
})
