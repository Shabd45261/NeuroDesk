...
import (
	...
	"github.com/mudler/NeuroDesk/pkg/oci/cosignverify"
...
// gate on NEURODESK_COSIGN_LIVE=1.
var _ = Describe("VerifyImage", func() {
	BeforeEach(func() {
		if os.Getenv("NEURODESK_COSIGN_LIVE") == "" {
			Skip("set NEURODESK_COSIGN_LIVE=1 to run live cosign verification")
		}
	})
	...
	//   NEURODESK_COSIGN_LIVE=1
	//   NEURODESK_COSIGN_LIVE_IMAGE=<image-ref-signed-with-new-bundle-format>
	//   NEURODESK_COSIGN_LIVE_ISSUER=<expected OIDC issuer>
	//   NEURODESK_COSIGN_LIVE_IDENTITY_REGEX=<expected identity SAN regex>
	//
	// No defaults — we don't have a stable third-party image known to be
	// signed in the new-bundle-format yet. Once the neurodesk-backends CI
	// is signing images, plug one of those refs in here.
	It("verifies a signed image when NEURODESK_COSIGN_LIVE_IMAGE is set", func() {
		image := os.Getenv("NEURODESK_COSIGN_LIVE_IMAGE")
		issuer := os.Getenv("NEURODESK_COSIGN_LIVE_ISSUER")
		identityRegex := os.Getenv("NEURODESK_COSIGN_LIVE_IDENTITY_REGEX")
		if image == "" || issuer == "" || identityRegex == "" {
			Skip("set NEURODESK_COSIGN_LIVE_IMAGE / _ISSUER / _IDENTITY_REGEX to run the positive case")
		}
