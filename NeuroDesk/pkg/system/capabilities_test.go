...
var _ = Describe("DetectedCapability", func() {
	...
	BeforeEach(func() {
		previous = os.Getenv("NEURODESK_FORCE_META_BACKEND_CAPABILITY")
	})
	...
	It("returns the forced capability verbatim without map fallback", func() {
		Expect(os.Setenv("NEURODESK_FORCE_META_BACKEND_CAPABILITY", "nvidia-cuda-12")).To(Succeed())
		state := &SystemState{}
	...
	It("does not fall back to default when the capability is unusual", func() {
		Expect(os.Setenv("NEURODESK_FORCE_META_BACKEND_CAPABILITY", "metal")).To(Succeed())
		state := &SystemState{}
