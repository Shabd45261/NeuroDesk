...
const (
	backendTempDirEnv       = "NEURODESK_BACKEND_TEMP_DIR"
	backendRuntimeDirPrefix = "process-"
	backendRuntimeMarker    = ".neurodesk-backend-runtime"
	backendRuntimeMagic     = "neurodesk-backend-runtime-v1\n"
)
...
// runtime from one abandoned when NeuroDesk was killed or crashed.
...
func backendRuntimeRoot() string {
	...
	// Always append a NeuroDesk- and user-specific namespace. Even if an operator
	// points the configurable base at /tmp, the sweeper never inspects unrelated
	// process-* directories in that shared parent.
	return filepath.Join(base, fmt.Sprintf("neurodesk-%d", os.Getuid()), "backend-runtime")
}
...
func newBackendProcessRuntime() (*backendProcessRuntime, error) {
	...
	// Serialize sweeping with creation. Otherwise a second NeuroDesk instance
