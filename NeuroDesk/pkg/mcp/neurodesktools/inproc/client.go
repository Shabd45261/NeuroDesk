// Package inproc provides an in-process NeuroDeskClient that calls NeuroDesk
// services directly. Used by the chat handler when a chat session opts into
// the NeuroDesk Assistant modality, avoiding an HTTP loopback to the same
...
import (
	...
	"github.com/mudler/NeuroDesk/core/gallery/importers"
	...
	"github.com/mudler/NeuroDesk/core/services/nodes/prefixcache"
	"github.com/mudler/NeuroDesk/core/services/routing/billing"
	"github.com/mudler/NeuroDesk/core/services/routing/corpus"
	"github.com/mudler/NeuroDesk/core/services/routing/pii"
	"github.com/mudler/NeuroDesk/core/services/routing/router"
	"github.com/mudler/NeuroDesk/core/services/voiceprofile"
	...
	"github.com/mudler/NeuroDesk/pkg/utils"
	"github.com/mudler/NeuroDesk/pkg/vram"
	"gopkg.in/yaml.v3"
)

// Client implements neurodesktools.NeuroDeskClient by calling NeuroDesk services
...
// Compile-time assertion that *Client satisfies neurodesktools.NeuroDeskClient.
var _ neurodesktools.NeuroDeskClient = (*Client)(nil)

// ---- Models / gallery (read) ----

func (c *Client) GallerySearch(_ context.Context, q neurodesktools.GallerySearchQuery) ([]gallery.Metadata, error) {
...
func (c *Client) ListInstalledModels(_ context.Context, capability neurodesktools.Capability) ([]neurodesktools.InstalledModel, error) {
	...
	out := make([]neurodesktools.InstalledModel, 0, len(configs))
	for _, m := range configs {
		out = append(out, neurodesktools.InstalledModel{
			Name:         m.Name,
			Backend:      m.Backend,
...
func (c *Client) GetJobStatus(_ context.Context, jobID string) (*neurodesktools.JobStatus, error) {
	if jobID == "" {
...
func (c *Client) GetModelConfig(ctx context.Context, name string) (*neurodesktools.ModelConfigView, error) {
	view, err := c.modelAdmin.GetConfig(ctx, name)
	if err != nil {
		return nil, err
	}
	return &neurodesktools.ModelConfigView{Name: view.Name, YAML: view.YAML, JSON: view.JSON}, nil
}

// ---- Models / gallery (write) ----

func (c *Client) InstallModel(ctx context.Context, req neurodesktools.InstallModelRequest) (string, error) {
	if req.ModelName == "" {
...
func (c *Client) ImportModelURI(ctx context.Context, req neurodesktools.ImportModelURIRequest) (*neurodesktools.ImportModelURIResponse, error) {
	if req.URI == "" {
	...
	if err != nil {
		...
		if errors.As(err, &amb) {
			...
			return &neurodesktools.ImportModelURIResponse{
...
// mirroring neurodesk.ImportModelEndpoint's create path: validate, validate the
...
func (c *Client) createAlias(name, target string) error {
	...
	// 0600: the NeuroDesk process is the sole reader/writer of model configs,
...
func (c *Client) ListAliases(_ context.Context) ([]neurodesktools.AliasInfo, error) {
	// Mirror neurodesk.ListAliasesEndpoint: every config whose Alias is set.
	out := []neurodesktools.AliasInfo{}
	for _, cfg := range c.ConfigLoader.GetAllModelsConfigs() {
		if cfg.IsAlias() {
			out = append(out, neurodesktools.AliasInfo{Name: cfg.Name, Target: cfg.Alias})
		}
	}
	return out, nil
}
...
func (c *Client) ListBackends(_ context.Context) ([]neurodesktools.Backend, error) {
	systemBackends, err := c.Gallery.ListBackends()
	if err != nil {
		return nil, fmt.Errorf("list backends: %w", err)
	}
	out := make([]neurodesktools.Backend, 0, len(systemBackends))
	for name := range systemBackends {
		out = append(out, neurodesktools.Backend{Name: name, Installed: true})
	}
	return out, nil
}
...
func (c *Client) InstallBackend(ctx context.Context, req neurodesktools.InstallBackendRequest) (string, error) {
	if req.BackendName == "" {
...
func (c *Client) SystemInfo(_ context.Context) (*neurodesktools.SystemInfo, error) {
	info := &neurodesktools.SystemInfo{
...
func (c *Client) ListNodes(_ context.Context) ([]neurodesktools.Node, error) {
...
func (c *Client) ListScheduling(ctx context.Context) ([]neurodesktools.ModelSchedulingConfig, error) {
	...
	return neurodesktools.SchedulingConfigsFromNodes(configs), nil
}

func (c *Client) GetScheduling(ctx context.Context, modelName string) (*neurodesktools.ModelSchedulingConfig, error) {
	if modelName == "" {
	...
	out := neurodesktools.SchedulingConfigFromNode(*config)
	return &out, nil
}

func (c *Client) SetScheduling(ctx context.Context, req neurodesktools.SetSchedulingRequest) (*neurodesktools.ModelSchedulingConfig, error) {
	if req.ModelName == "" {
		return nil, errors.New("model_name is required")
	}
	if c.NodeRegistry == nil {
...
func (c *Client) VRAMEstimate(ctx context.Context, req neurodesktools.VRAMEstimateRequest) (*vram.EstimateResult, error) {
...
func (c *Client) currentBranding() *neurodesktools.Branding {
	b := c.AppConfig.Branding
	return &neurodesktools.Branding{
...
func (c *Client) GetBranding(_ context.Context) (*neurodesktools.Branding, error) {
	return c.currentBranding(), nil
}

func (c *Client) SetBranding(_ context.Context, req neurodesktools.SetBrandingRequest) (*neurodesktools.Branding, error) {
...
func (c *Client) ListVoiceProfiles(ctx context.Context) ([]neurodesktools.VoiceProfile, error) {
...
func (c *Client) CreateVoiceProfile(ctx context.Context, req neurodesktools.CreateVoiceProfileRequest) (*neurodesktools.VoiceProfile, error) {
	if req.AudioBase64 == "" {
...
func capabilityToFlag(capability neurodesktools.Capability) (config.ModelConfigUsecase, bool) {
	switch capability {
	case neurodesktools.CapabilityAny:
		return 0, false
	case neurodesktools.CapabilityChat:
		return config.FLAG_CHAT, true
	case neurodesktools.CapabilityCompletion:
		return config.FLAG_COMPLETION, true
	case neurodesktools.CapabilityEmbeddings:
		return config.FLAG_EMBEDDINGS, true
	case neurodesktools.CapabilityImage:
		return config.FLAG_IMAGE, true
	case neurodesktools.CapabilityTTS:
		return config.FLAG_TTS, true
	case neurodesktools.CapabilityTranscript:
		return config.FLAG_TRANSCRIPT, true
	case neurodesktools.CapabilityRerank:
		return config.FLAG_RERANK, true
	case neurodesktools.CapabilityVAD:
...
func (c *Client) GetUsageStats(ctx context.Context, q neurodesktools.UsageStatsQuery) (*neurodesktools.UsageStats, error) {
	...
	totals := neurodesktools.UsageTotals{}
	buckets := make([]neurodesktools.UsageBucket, 0, len(rows))
	for _, r := range rows {
		buckets = append(buckets, neurodesktools.UsageBucket{
			Bucket:           r.Bucket,
			Model:            r.Model,
			UserID:           r.UserID,
			UserName:         r.UserName,
	...
	return &neurodesktools.UsageStats{
		Viewer:  neurodesktools.UsageViewer{ID: viewerID, Name: viewerName, Role: viewerRole},
		Period:  period,
		Totals:  totals,
		Buckets: buckets,
	}, nil
...
func (c *Client) GetPIIEvents(ctx context.Context, q neurodesktools.PIIEventsQuery) ([]neurodesktools.PIIEvent, error) {
	if c.PIIEvents == nil {
	...
	out := make([]neurodesktools.PIIEvent, 0, len(events))
	for _, e := range events {
		out = append(out, neurodesktools.PIIEvent{
			ID:            e.ID,
...
func (c *Client) GetRouterDecisions(ctx context.Context, q neurodesktools.RouterDecisionsQuery) ([]neurodesktools.RouterDecision, error) {
	...
	out := make([]neurodesktools.RouterDecision, 0, len(rows))
	for _, r := range rows {
		out = append(out, neurodesktools.RouterDecision{
			ID:             r.ID,
...
func (c *Client) GetMiddlewareStatus(ctx context.Context) (*neurodesktools.MiddlewareStatus, error) {
	router := neurodesktools.MiddlewareRouterStatus{
		Configured: false,
		Models:     []string{},
		Note:       "Intelligent routing is not yet implemented.",
	}
	piiSection := neurodesktools.MiddlewarePIIStatus{
	...
	if c.ConfigLoader != nil {
		for _, cfg := range c.ConfigLoader.GetAllModelsConfigs() {
			cfg := cfg
			piiSection.Models = append(piiSection.Models, neurodesktools.MiddlewarePIIModel{
				Name:              cfg.Name,
				Backend:           cfg.Backend,
	...
	return &neurodesktools.MiddlewareStatus{PII: piiSection, Router: router}, nil
}
...
func (c *Client) GetRouterCorpusStats(_ context.Context, routerModel string) (*neurodesktools.RouterCorpusStats, error) {
	...
	return &neurodesktools.RouterCorpusStats{
		Router:          cfg.Name,
		StoreName:       storeName,
...
func (c *Client) SeedRouterCorpus(ctx context.Context, req neurodesktools.RouterCorpusSeedRequest) (*neurodesktools.RouterCorpusSeedResult, error) {
	...
	return &neurodesktools.RouterCorpusSeedResult{
		Router:      cfg.Name,
		Added:       added,
		Skipped:     skipped,
		Total:       stats.Total,
...
func (c *Client) ClearRouterCorpus(ctx context.Context, routerModel string) (*neurodesktools.RouterCorpusClearResult, error) {
	if c.RouterCorpus == nil {
		return nil, errors.New("router corpus manager unavailable")
	}
	cfg, storeName, err := corpus.ResolveKNNRouter(c.ConfigLoader, c.AppConfig, routerModel)
	...
	return &neurodesktools.RouterCorpusClearResult{Router: cfg.Name, Cleared: cleared}, nil
}
