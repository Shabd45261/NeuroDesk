...
func registerModelTools(s *mcp.Server, client NeuroDeskClient, opts Options) {
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "List models currently installed on this NeuroDesk. Optional capability filter (chat, completion, embeddings, image, tts, transcript, rerank, vad).",
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		// error regardless of which NeuroDeskClient backs the tool.
		if args.ModelName == "" {
