...
// fakeNeuroDesk is a minimal HTTP server that mimics the relevant NeuroDesk
// admin endpoints. Only the routes the httpapi.Client touches need to exist.
func fakeNeuroDesk() *httptest.Server {
...
var _ = Describe("httpapi.Client against the NeuroDesk admin REST surface", func() {
	var (
		srv *httptest.Server
		c   *Client
		ctx context.Context
	...
	Describe("GallerySearch", func() {
		It("filters by tag, applies limit, and preserves tags on the result", func() {
			hits, err := c.GallerySearch(ctx, neurodesktools.GallerySearchQuery{Query: "qwen", Tag: "chat", Limit: 5})
	...
	Describe("InstallModel", func() {
		It("returns the job id parsed from the apply response", func() {
			id, err := c.InstallModel(ctx, neurodesktools.InstallModelRequest{ModelName: "qwen2.5-7b-instruct"})
			Expect(err).ToNot(HaveOccurred())
		...
		It("forwards a chosen variant to the apply endpoint", func() {
			...
			_, err := New(srv.URL, "").InstallModel(context.Background(), neurodesktools.InstallModelRequest{
	...
	Describe("Scheduling", func() {
		...
		It("sets a scheduling config", func() {
			out, err := c.SetScheduling(ctx, neurodesktools.SetSchedulingRequest{ModelName: "qwen", MinReplicas: 1, MaxReplicas: 2})
			Expect(err).ToNot(HaveOccurred())
			Expect(out.ModelName).To(Equal("qwen"))
...
func schedulingJSONKeys(config *neurodesktools.ModelSchedulingConfig) map[string]any {
...
var _ = Describe("ErrHTTPNotFound", func() {
	...
	Context("on the legacy 500-with-could-not-find body", func() {
		It("treats it as not-found until NeuroDesk returns a proper 404", func() {
			srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
				http.Error(w, "could not find any status for ID", http.StatusInternalServerError)
			}))
			DeferCleanup(srv.Close)
