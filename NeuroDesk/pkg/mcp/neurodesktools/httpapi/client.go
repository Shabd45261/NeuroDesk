// Package httpapi provides a NeuroDeskClient that talks to a remote NeuroDesk
// instance over its REST API. Used by the standalone "neurodesk mcp-server"
...
import (
	...
	"github.com/mudler/NeuroDesk/pkg/httpclient"
	neurodesktools "github.com/mudler/NeuroDesk/pkg/mcp/neurodesktools"
	"github.com/mudler/NeuroDesk/pkg/vram"
)

// Client is a thin REST wrapper. It maps each NeuroDeskClient method to the
...
func (c *Client) GallerySearch(ctx context.Context, q neurodesktools.GallerySearchQuery) ([]gallery.Metadata, error) {
...
func (c *Client) ListInstalledModels(ctx context.Context, capability neurodesktools.Capability) ([]neurodesktools.InstalledModel, error) {
	_ = capability // Capability filtering is unavailable over the welcome HTTP shape today; see TODO below.
	// /v1/models is the OpenAI-compat shape; we use the NeuroDesk welcome JSON
	// for richer info.
	var welcome struct {
		ModelsConfig []struct {
	...
	out := make([]neurodesktools.InstalledModel, 0, len(welcome.ModelsConfig))
	for _, m := range welcome.ModelsConfig {
		out = append(out, neurodesktools.InstalledModel{Name: m.Name, Backend: m.Backend})
	}
	return out, nil
}
...
func (c *Client) GetJobStatus(ctx context.Context, jobID string) (*neurodesktools.JobStatus, error) {
	if jobID == "" {
	...
	return &neurodesktools.JobStatus{
		ID:                 jobID,
...
// GetModelConfig is intentionally a stub for the HTTP client: NeuroDesk's
...
// LLM. Tracked under the neurodesk-assistant follow-ups (see
// .agents/neurodesk-assistant-mcp.md) — once a JSON-only
// GET /api/models/config-yaml/:name endpoint lands on the server, this
// method calls it and the stub goes away.
//
// FIXME(neurodesk-assistant): wire to a JSON read-back endpoint.
func (c *Client) GetModelConfig(_ context.Context, _ string) (*neurodesktools.ModelConfigView, error) {
...
func (c *Client) ImportModelURI(ctx context.Context, req neurodesktools.ImportModelURIRequest) (*neurodesktools.ImportModelURIResponse, error) {
	...
	return &neurodesktools.ImportModelURIResponse{JobID: raw.ID}, nil
}
...
func (c *Client) ListAliases(ctx context.Context) ([]neurodesktools.AliasInfo, error) {
	// /api/aliases returns []{name,target} directly - pass it through.
	var out []neurodesktools.AliasInfo
...
func (c *Client) ListBackends(ctx context.Context) ([]neurodesktools.Backend, error) {
	var raw []struct {
		Name      string `json:"name"`
	...
	out := make([]neurodesktools.Backend, 0, len(raw))
	for _, b := range raw {
		out = append(out, neurodesktools.Backend{Name: b.Name, Installed: b.Installed})
	}
	return out, nil
}
...
func (c *Client) SystemInfo(ctx context.Context) (*neurodesktools.SystemInfo, error) {
	var welcome struct {
	...
	info := &neurodesktools.SystemInfo{Version: welcome.Version}
...
func (c *Client) ListNodes(ctx context.Context) ([]neurodesktools.Node, error) {
	var raw []struct {
		ID          string `json:"id"`
	...
	out := make([]neurodesktools.Node, 0, len(raw))
	for _, n := range raw {
		out = append(out, neurodesktools.Node{
			ID:          n.ID,
			Address:     n.Address,
...
func (c *Client) ListScheduling(ctx context.Context) ([]neurodesktools.ModelSchedulingConfig, error) {
	var out []neurodesktools.ModelSchedulingConfig
...
// independent of the neurodesk endpoint package.
...
func (c *Client) GetBranding(ctx context.Context) (*neurodesktools.Branding, error) {
	var raw brandingResponse
	if err := c.do(ctx, http.MethodGet, routeBranding, nil, &raw); err != nil {
		return nil, err
	}
	return (*neurodesktools.Branding)(&raw), nil
}

func (c *Client) SetBranding(ctx context.Context, req neurodesktools.SetBrandingRequest) (*neurodesktools.Branding, error) {
...
func (c *Client) ListVoiceProfiles(ctx context.Context) ([]neurodesktools.VoiceProfile, error) {
	var response struct {
		Data []neurodesktools.VoiceProfile `json:"data"`
	}
...
func (c *Client) CreateVoiceProfile(ctx context.Context, req neurodesktools.CreateVoiceProfileRequest) (*neurodesktools.VoiceProfile, error) {
	var profile neurodesktools.VoiceProfile
...
func (c *Client) GetUsageStats(ctx context.Context, q neurodesktools.UsageStatsQuery) (*neurodesktools.UsageStats, error) {
	...
	out := &neurodesktools.UsageStats{
		Viewer: neurodesktools.UsageViewer{ID: raw.Viewer.ID, Name: raw.Viewer.Name, Role: raw.Viewer.Role},
		Period: period,
		...
		Buckets: make([]neurodesktools.UsageBucket, 0, len(raw.Usage)),
	}
	for _, b := range raw.Usage {
		out.Buckets = append(out.Buckets, neurodesktools.UsageBucket{
			Bucket:           b.Bucket,
			Model:            b.Model,
			UserID:           b.UserID,
			UserName:         b.UserName,
...
func (c *Client) GetPIIEvents(ctx context.Context, q neurodesktools.PIIEventsQuery) ([]neurodesktools.PIIEvent, error) {
	...
	var raw struct {
		Events []neurodesktools.PIIEvent `json:"events"`
	}
	if err := c.do(ctx, http.MethodGet, path, nil, &raw); err != nil {
		return nil, err
	}
...
func (c *Client) GetMiddlewareStatus(ctx context.Context) (*neurodesktools.MiddlewareStatus, error) {
	var out neurodesktools.MiddlewareStatus
...
func (c *Client) GetRouterDecisions(ctx context.Context, q neurodesktools.RouterDecisionsQuery) ([]neurodesktools.RouterDecision, error) {
	...
	var raw struct {
		Decisions []neurodesktools.RouterDecision `json:"decisions"`
	}
	if err := c.do(ctx, http.MethodGet, path, nil, &raw); err != nil {
		return nil, err
	}
...
func (c *Client) GetRouterCorpusStats(ctx context.Context, routerModel string) (*neurodesktools.RouterCorpusStats, error) {
	var out neurodesktools.RouterCorpusStats
...
func (c *Client) SeedRouterCorpus(ctx context.Context, req neurodesktools.RouterCorpusSeedRequest) (*neurodesktools.RouterCorpusSeedResult, error) {
	...
	body := struct {
		Entries []neurodesktools.RouterCorpusEntry `json:"entries"`
	}{Entries: req.Entries}
	var out neurodesktools.RouterCorpusSeedResult
...
func (c *Client) ClearRouterCorpus(ctx context.Context, routerModel string) (*neurodesktools.RouterCorpusClearResult, error) {
	var out neurodesktools.RouterCorpusClearResult
