...
// NewServer builds an MCP server that exposes NeuroDesk's admin surface as
// tools, backed by the supplied NeuroDeskClient. The same server type is used
// in-process (paired in-memory transport) and out-of-process (stdio).
func NewServer(client NeuroDeskClient, opts Options) *mcp.Server {
