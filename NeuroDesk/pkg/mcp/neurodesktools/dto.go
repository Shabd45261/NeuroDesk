...
import "github.com/mudler/NeuroDesk/core/services/voiceprofile"

// DTOs for the NeuroDeskClient interface. Where the same shape already exists
...
// GET /api/aliases emits. Kept aligned with neurodesk.AliasInfo so the
...
// SystemInfo summarises the NeuroDesk deployment.
...
type SetBrandingRequest struct {
	InstanceName    *string `json:"instance_name,omitempty"    jsonschema:"New instance display name (replaces \"NeuroDesk\" in headers, footers, and the browser tab). Pass an empty string to reset to default."`
...
// user; the NeuroDeskClient implementation enforces the role check.
...
// pkg/vram.EstimateResult — used directly via the NeuroDeskClient interface
