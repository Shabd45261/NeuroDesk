...
if [[ -n "${CUDA_DOCKER_ARCH:-}" ]]; then
  ...
  rm -rf /NeuroDesk/backend/cpp/llama-cpp-*-build
fi

cd /NeuroDesk/backend/cpp/llama-cpp
BUILD_TARGET=$(/NeuroDesk/.docker/llama-cpp-build-target.sh "${TARGETARCH}" "${BUILD_TYPE:-}")
