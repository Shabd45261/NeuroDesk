...
if [[ -n "${CUDA_DOCKER_ARCH:-}" ]]; then
  ...
  rm -rf /NeuroDesk/backend/cpp/turboquant-*-build
fi

cd /NeuroDesk/backend/cpp/turboquant

BUILD_TARGET=$(/NeuroDesk/.docker/turboquant-build-target.sh "${TARGETARCH}" "${BUILD_TYPE:-}")
if [ "$BUILD_TARGET" = "turboquant-cpu-all" ]; then
  ...
  if [ "${TARGETARCH}" = "arm64" ]; then
    sh /NeuroDesk/.docker/apt-mirror.sh || true
