...
if [ -f "$RPC_CMAKE" ] && grep -q "GGML_RPC_RDMA_APPLE" "$RPC_CMAKE" && ! grep -q "NEURODESK_RDMA_IFACE" "$RPC_CMAKE"; then
    echo "==> ggml-rpc carries the Apple RDMA transport, re-declaring its weak librdma link as INTERFACE"
    cat >> "$RPC_CMAKE" <<'EOF'

# NEURODESK_RDMA_IFACE: added by backend/cpp/llama-cpp/prepare.sh
...
#pragma once
#define NEURODESK_LEGACY_LOAD_MODE ${LEGACY_LOAD_MODE}
#define NEURODESK_HAS_SERVER_METRICS ${HAS_SERVER_METRICS}
#define NEURODESK_HAS_MTMD_INIT_OPT ${HAS_MTMD_INIT_OPT}
#define NEURODESK_HAS_N_CPU_FFN_HELPER ${HAS_N_CPU_FFN_HELPER}
EOF
