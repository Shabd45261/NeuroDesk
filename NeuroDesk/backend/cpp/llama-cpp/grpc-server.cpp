...
//
// This is a gRPC server for llama.cpp compatible with the NeuroDesk proto
...
#define NEURODESK_HAS_SERVER_SCHEMA 1
...
#if NEURODESK_HAS_MTMD_INIT_OPT
#define NEURODESK_MTMD_INIT_OPT_ARG(value) , value
#else
#define NEURODESK_MTMD_INIT_OPT_ARG(value)
#endif

// gRPC bearer token auth for distributed mode.
// Reads NEURODESK_GRPC_AUTH_TOKEN from the environment. When set, rejects
...
static void params_parse(server_context& /*ctx_server*/, const backend::ModelOptions* request,
    ...
    // NeuroDesk keeps n_parallel=1 by default, which would skip that auto path
    ...
    // single-slot path that NeuroDesk ships with: without it, idle slots are
    ...
    // default (32); the previous NeuroDesk-specific 8 was unnecessarily tight
    ...
    // (NEURODESK_TURBOQUANT_NO_CHECKPOINT_MIN_STEP is injected by
    // backend/cpp/turboquant/patch-grpc-server.sh).
#ifndef NEURODESK_TURBOQUANT_NO_CHECKPOINT_MIN_STEP
    ...
    // enum (ggml-org/llama.cpp#20834), so the three independent NeuroDesk settings
...
#ifndef NEURODESK_LLAMA_CPP_NO_SCORE_TASK
        ...
        // --- draft model MoE on CPU (upstream --spec-draft-cpu-moe / --spec-draft-n-cpu-moe) ---
            ...
            if (optval != NULL) {
                try {
                    int n = std::stoi(optval_str);
                    if (n < 0) n = 0;
#if NEURODESK_HAS_N_CPU_FFN_HELPER
    ...
    // NeuroDesk keeps mmap, mlock and direct-I/O as three independent settings,
    ...
    // NEURODESK_LEGACY_LOAD_MODE in the generated llama_compat.h accordingly.
#if NEURODESK_LEGACY_LOAD_MODE
    ...
    // over the NeuroDesk-resolved field it maps to (e.g. --ctx-size beats
    ...
    if (!extra_argv.empty()) {
        ...
        // passthrough flag can't silently clobber NeuroDesk's resolved value.
        const int saved_n_parallel = params.n_parallel;
        // Newer upstream parsers assert that these fields still contain their
        // negative initialization sentinels. NeuroDesk resolves them from the
...
#ifndef NEURODESK_LLAMA_CPP_NO_TTS_TASK
...
public:
    ...
    grpc::Status PredictStream(grpc::ServerContext* context, const backend::PredictOptions* request, grpc::ServerWriter<backend::Reply>* writer) override {
        ...
        try {
            ...
            if (has_mtmd) {
                // multimodal
                inputs.push_back(process_mtmd_prompt(ctx_server.impl->mctx, prompt_str, files NEURODESK_MTMD_INIT_OPT_ARG(ctx_server.impl->init_opt)));
            } else {
                 // Everything else, including multimodal completions.
                inputs = tokenize_input_prompts(ctx_server.impl->vocab, ctx_server.impl->mctx, prompt_str, true, true NEURODESK_MTMD_INIT_OPT_ARG(ctx_server.impl->init_opt));
            }
...
#ifdef NEURODESK_HAS_SERVER_SCHEMA
    ...
    grpc::Status Embedding(ServerContext* context, const backend::PredictOptions* request, backend::EmbeddingResult* embeddingResult) override {
        ...
        */
        ...
        auto tokenized_prompts = tokenize_input_prompts(ctx_server.impl->vocab, ctx_server.impl->mctx, prompt, true, true NEURODESK_MTMD_INIT_OPT_ARG(ctx_server.impl->init_opt));
    ...
    grpc::Status Rerank(ServerContext* context, const backend::RerankRequest* request, backend::RerankResult* rerankResult) override {
        ...
        auto rd = ctx_server.get_response_reader();
        {
            ...
            for (size_t i = 0; i < documents.size(); i++) {
                auto tmp = format_prompt_rerank(ctx_server.impl->model_tgt, ctx_server.impl->vocab, ctx_server.impl->mctx, request->query(), documents[i] NEURODESK_MTMD_INIT_OPT_ARG(ctx_server.impl->init_opt));
    ...
    grpc::Status prepareTTSTask(const backend::TTSRequest* request, bool stream, server_task & task) {
        ...
        auto wrapper = mtmd_helper_bitmap_init_from_file(ctx_server.impl->mctx, opts.voice_path.c_str(), false NEURODESK_MTMD_INIT_OPT_ARG(ctx_server.impl->init_opt));
        if (!wrapper.bitmap) {
...
#ifdef NEURODESK_LLAMA_CPP_NO_SCORE_TASK
        (void) request;
        (void) response;
        return grpc::Status(grpc::StatusCode::UNIMPLEMENTED,
...
// request slots data using task queue
        ...
        response->set_slot_id(0);
        response->set_prompt_json_for_slot("");
#if NEURODESK_HAS_SERVER_METRICS
    ...
    grpc::Status runTranscriptionAsCompletion(grpc::ServerContext* context,
        ...
        // Read audio bytes from the path NeuroDesk's HTTP layer wrote.
...
int main(int argc, char** argv) {
  ...
  while ((option = getopt_long(argc, argv, "a:", long_options, &option_index)) != -1) {
    ...
    // Best-effort backstop: self-terminate if the NeuroDesk process that spawned
    ...
    // Initialize bearer token auth if NEURODESK_GRPC_AUTH_TOKEN is set
    const char* auth_token = std::getenv("NEURODESK_GRPC_AUTH_TOKEN");
    if (auth_token != nullptr && auth_token[0] != '\0') {
        g_grpc_auth_token = auth_token;
        std::cout << "gRPC auth enabled via NEURODESK_GRPC_AUTH_TOKEN" << std::endl;
    }
