...
//
// NeuroDesk spawns this backend as a child process and, on a clean shutdown,
// tears it down itself (SIGTERM -> grace -> SIGKILL). That graceful path only
// runs when NeuroDesk receives a catchable signal and lives long enough to run
// its handlers. If NeuroDesk is SIGKILLed (e.g. a supervising process's grace
...
inline const char *kEnvParentWatch()         { return "NEURODESK_BACKEND_PARENT_WATCH"; }
inline const char *kEnvParentWatchInterval() { return "NEURODESK_BACKEND_PARENT_WATCH_INTERVAL"; }
