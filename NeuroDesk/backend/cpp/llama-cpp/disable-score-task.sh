...
# NeuroDesk's slot-based Score patches. The RPC remains present in the shared
...
if grep -q '^#define NEURODESK_LLAMA_CPP_NO_SCORE_TASK' "$SRC"; then
    echo "==> $SRC already disables the NeuroDesk score task, skipping"
    exit 0
fi

awk '
    !done && /^#include/ {
        print "#define NEURODESK_LLAMA_CPP_NO_SCORE_TASK 1"
...
echo "==> NeuroDesk score task disabled in $SRC"
