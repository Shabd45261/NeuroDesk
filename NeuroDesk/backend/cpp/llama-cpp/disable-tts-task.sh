...
# NeuroDesk's SERVER_TASK_TYPE_TTS patch. The RPCs remain present in the shared
...
if grep -q '^#define NEURODESK_LLAMA_CPP_NO_TTS_TASK' "$SRC"; then
    echo "==> $SRC already disables the NeuroDesk TTS task, skipping"
    exit 0
fi

awk '
    !done && /^#include/ {
        print "#define NEURODESK_LLAMA_CPP_NO_TTS_TASK 1"
...
echo "==> NeuroDesk TTS task disabled in $SRC"
