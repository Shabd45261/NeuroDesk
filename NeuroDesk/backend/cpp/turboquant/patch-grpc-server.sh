...
#   2. Define NEURODESK_TURBOQUANT_NO_CHECKPOINT_MIN_STEP at the top of the file
...
# (flat-field sed renames + a coarse NEURODESK_LEGACY_LLAMA_CPP_SPEC define). The
...
# 2. Define NEURODESK_TURBOQUANT_NO_CHECKPOINT_MIN_STEP at the top of the file so
...
if grep -q '^#define NEURODESK_TURBOQUANT_NO_CHECKPOINT_MIN_STEP' "$SRC"; then
    echo "==> $SRC already defines NEURODESK_TURBOQUANT_NO_CHECKPOINT_MIN_STEP, skipping"
else
    echo "==> patching $SRC to define NEURODESK_TURBOQUANT_NO_CHECKPOINT_MIN_STEP at the top"
    # Insert the define before the very first `#include` so it precedes the
    # checkpoint_min_step references.
    awk '
        !done && /^#include/ {
            print "#define NEURODESK_TURBOQUANT_NO_CHECKPOINT_MIN_STEP 1"
        ...
        END {
            if (!done) {
                print "patch-grpc-server.sh: no #include anchor found to insert NEURODESK_TURBOQUANT_NO_CHECKPOINT_MIN_STEP" > "/dev/stderr"
                exit 1
            }
        }
    ' "$SRC" > "$SRC.tmp"
    mv "$SRC.tmp" "$SRC"
    echo "==> NEURODESK_TURBOQUANT_NO_CHECKPOINT_MIN_STEP define OK"
fi
