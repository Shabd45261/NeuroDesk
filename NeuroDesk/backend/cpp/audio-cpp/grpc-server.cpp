// audio.cpp NeuroDesk gRPC backend.
//
...
// upstream expects churn, and upstream is Apache-2.0 while NeuroDesk is MIT.
//
...
// One model per process, matching every other NeuroDesk backend. The mutex guards
...
// NeuroDesk's VADRequest carries raw floats and no sample rate at all. Every
// NeuroDesk VAD backend treats them as 16 kHz mono (backend/go/silero-vad/vad.go
...
// worse than an aliased band, and because every other NeuroDesk ASR path already
...
public:
    ...
    GStatus Diarize(ServerContext *, const backend::DiarizeRequest *request,
        ...
        try {
            ...
            // correct output for overlapped speech. NeuroDesk is overlap-tolerant
    ...
    // Serves the four tasks NeuroDesk's AudioTransform can represent: voice
    ...
    // are real files in the caller's output directory that NeuroDesk's caller
    ...
    //
    // TTSRequest.voice is overloaded across NeuroDesk backends, some reading it as
    ...
    GStatus TTS(ServerContext *, const backend::TTSRequest *request,
        ...
        try {
            ...
            // dst is NeuroDesk's own generated-content path and not anything the
            ...
            // back and what every other NeuroDesk TTS backend puts here.
    ...
    GStatus AudioTranscriptionLive(
        ...
        try {
            ...
            // cost the caller its fallback. Unreachable from NeuroDesk today,
    ...
    //
    // None of these is deferred work. Each is an RPC whose NeuroDesk contract has
