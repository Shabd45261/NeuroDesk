"""Face recognition engine implementations for the NeuroDesk insightface backend.
...
# at github.com/deepinsight/insightface/releases). NeuroDesk's gallery extracts
...
class InsightFaceEngine:
    ...
    We reimplement the same loop here so we can:
      ...
      1. Load packs from whatever directory NeuroDesk's gallery extracted
      ...
      2. Skip insightface's built-in auto-download entirely: weight
         delivery is NeuroDesk's gallery `files:` job now, checksum-
    ...
    def prepare(self, options: dict[str, str]) -> None:
        ...
        if pack_dir is None:
            raise ValueError(
                ...
                f"`neurodesk models install insightface-{self.model_pack.replace('_', '-')}`"
            )
...
def _locate_insightface_pack(options: dict[str, str], name: str) -> str | None:
    ...
    NeuroDesk's gallery `files:` extracts the pack zip straight into the
...
def _resolve_model_path(path: str, model_dir: str | None = None) -> str:
    """Resolve an ONNX file path across the paths NeuroDesk might deliver it from.
    ...
    Search order:
      ...
      2. `model_dir` (typically `os.path.dirname(ModelOptions.ModelFile)`) —
         this is how NeuroDesk surfaces gallery-managed files. When the gallery
