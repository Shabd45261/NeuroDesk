...
gallery. Simulates what NeuroDesk does at runtime: for each config, sets
...
def _run_one(cfg: dict, img_b64: str, model_dir: str) -> tuple[bool, str]:
    # Mirror NeuroDesk's gallery flow: populate model_dir with the
    ...
    load_res = svc.LoadModel(
        backend_pb2.ModelOptions(
            ...
            # NeuroDesk's models directory. The backend anchors relative
...
def main() -> int:
    # Honor NEURODESK_MODELS_PATH to re-use cached downloads across runs;
    # default to a fresh temp dir.
    model_dir = os.environ.get("NEURODESK_MODELS_PATH")
    if not model_dir:
        import tempfile
