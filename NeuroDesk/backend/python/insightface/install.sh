...
#
#   1. Weights should follow NeuroDesk's gallery-managed download flow
#      like every other backend. For OpenCV Zoo (YuNet + SFace) the
#      gallery entries in gallery/index.yaml list the ONNX files via
#      `files:` with URI + SHA-256 — NeuroDesk fetches them into the
#      models directory on `neurodesk models install`.
#
