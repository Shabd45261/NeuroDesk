#!/usr/bin/env python3
"""
Liquid Audio backend for NeuroDesk.
...
def _patch_liquid_audio_local_paths():
    ...
    which only accepts `owner/repo` ids. NeuroDesk's gallery hands us absolute
...
class BackendServicer(backend_pb2_grpc.BackendServicer):
    ...
    def Free(self, request, context):
        # Called by NeuroDesk when unloading the model. Drop GPU tensors so the
    ...
    def LoadModel(self, request, context):
        try:
            ...
            # When NeuroDesk's gallery has already staged the weights on disk,
    ...
    def StopFineTune(self, request, context):
        ...
        # NeuroDesk's job manager kills the backend process on stop. The flag below
