...
# AsyncLLM engine runs on the GPU through MLX. NeuroDesk's backend.py is UNCHANGED
...
# but INTO NeuroDesk's managed venv (ensureVenv) instead of a throwaway
# ~/.venv-vllm-metal, so the backend integrates with NeuroDesk's venv lifecycle
...
# runtime 6.33.6 crash in mudler/NeuroDesk#10718. Regenerate once the dependency set
