...
# NeuroDesk runtime image and the FROM-scratch backend image both
...
# vllm's own CLI instead of NeuroDesk's backend.py gRPC server. The
# follower speaks ZMQ directly to the head node's vllm ranks — there
# is no NeuroDesk gRPC on the follower side. Reaches this path via
# `neurodesk p2p-worker vllm`.
if [ "${1:-}" = "serve" ]; then
    ensureVenv
