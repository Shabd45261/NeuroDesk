...
# Options[] entries NeuroDesk itself acts on, in their normalized spelling.
...
def normalize_option_key(key):
    ...
    so accept that spelling everywhere NeuroDesk looks options up by name.
    """
...
def apply_options_to_engine_args(engine_args, options):
    ...
    type. Both ``--flag:value`` (NeuroDesk convention) and ``--flag=value``
    ...
    for opt in options:
        ...
        if name not in fields:
            # NeuroDesk reads these itself; whether the engine dataclass carries
