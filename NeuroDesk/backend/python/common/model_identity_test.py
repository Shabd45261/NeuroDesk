...
class TestInterceptorInstalled(unittest.TestCase):
    ...
    get_auth_interceptors() returns early when NEURODESK_GRPC_AUTH_TOKEN is
    ...
    def setUp(self):
        self._saved = os.environ.get("NEURODESK_GRPC_AUTH_TOKEN")
        os.environ.pop("NEURODESK_GRPC_AUTH_TOKEN", None)

    def tearDown(self):
        if self._saved is None:
            os.environ.pop("NEURODESK_GRPC_AUTH_TOKEN", None)
        else:
            os.environ["NEURODESK_GRPC_AUTH_TOKEN"] = self._saved
    ...
    def test_installed_alongside_auth_when_enabled(self):
        os.environ["NEURODESK_GRPC_AUTH_TOKEN"] = "secret"
        interceptors = grpc_auth.get_auth_interceptors()
        self.assertTrue(
            any(isinstance(i, model_identity.ModelIdentityInterceptor) for i in interceptors)
        )
