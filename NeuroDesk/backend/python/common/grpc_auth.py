"""Shared gRPC bearer token authentication interceptor for NeuroDesk Python backends.

When the environment variable NEURODESK_GRPC_AUTH_TOKEN is set, requests without
...
def get_auth_interceptors(*, aio: bool = False):
    """Return the gRPC server interceptors every NeuroDesk Python backend installs.
    ...
    NEURODESK_GRPC_AUTH_TOKEN is set.
    ...
    # every NeuroDesk Python backend invokes exactly once while building its gRPC
    ...
    token = os.environ.get("NEURODESK_GRPC_AUTH_TOKEN", "")
    if not token:
        return interceptors
    if aio:
