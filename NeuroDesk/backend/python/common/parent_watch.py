"""Parent-death watcher (best-effort backstop) for NeuroDesk Python backends.

NeuroDesk spawns each backend as a child process and, on a clean shutdown, tears
it down itself (SIGTERM -> grace -> SIGKILL). That graceful path only runs when
NeuroDesk receives a catchable signal and lives long enough to run its handlers.
If NeuroDesk is SIGKILLed (e.g. a supervising process's grace period elapses
...
Env vars (shared verbatim across the Go, C++ and Python backends):
  NEURODESK_BACKEND_PARENT_WATCH           enabled by default; a falsey value
                                         ("false"/"0"/"no"/"off", case-insensitive)
                                         disables it.
  NEURODESK_BACKEND_PARENT_WATCH_INTERVAL  poll interval as a Go-style duration
...
ENV_PARENT_WATCH = "NEURODESK_BACKEND_PARENT_WATCH"
ENV_PARENT_WATCH_INTERVAL = "NEURODESK_BACKEND_PARENT_WATCH_INTERVAL"
...
def _parent_died(orig_ppid):
    ...
    NeuroDesk process that spawned this backend) has exited: the orphan is handed
...
def start_parent_death_watcher():
    ...
    never a replacement for — NeuroDesk's graceful teardown.
    """
    global _started
    if not _enabled():
        return
