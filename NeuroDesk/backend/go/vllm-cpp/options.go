...
// NeuroDesk materializes the artifact, so they must replace the gallery's static
...
// snapshots/* - and it NEVER downloads. NeuroDesk keeps models in its own
...
// what NeuroDesk's own downloader produces), then the whole reference under the
...
func resolveDraftModelPath(speculativeConfig, modelsDir string) (string, error) {
	...
	return "", fmt.Errorf(
		"vllm-cpp: DFlash draft checkpoint %q not found (looked in: %s). "+
			"The engine does not download drafts - install the draft model into NeuroDesk first, "+
