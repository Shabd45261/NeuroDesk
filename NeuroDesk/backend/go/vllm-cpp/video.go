...
// engine's own shape rather than from NeuroDesk:
//
...
// the gRPC call carries NeuroDesk's application context.
...
func (v *VllmCpp) loadVideo(opts *pb.ModelOptions, dit string) error {
	...
	// Relative option paths resolve against NeuroDesk's models directory, which
...
func imageDimensions(path string) (int32, int32, error) {
	...
	f, err := os.Open(path) // #nosec G304 -- a path staged by NeuroDesk for this request
	if err != nil {
