...
// NeuroDesk gRPC backend over the vllm.cpp C ABI.
//
...
// NeuroDesk requests batch continuously inside the engine (the reason this
...
func (v *VllmCpp) Load(opts *pb.ModelOptions) error {
	...
	// engine never downloads one. Resolve it against NeuroDesk's models directory
	...
	// generic NeuroDesk knob every backend honours, max_model_len is the
