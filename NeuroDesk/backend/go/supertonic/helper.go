...
//
// Local modifications (if any) are marked with "NeuroDesk:" comments.
...
func LoadTextToSpeech(onnxDir string, useGPU bool, cfg Config) (*TextToSpeech, error) {
	...
	fmt.Println("Using CPU for inference") // NeuroDesk: drop redundant newline (vet)
...
func InitializeONNXRuntime() error {
	...
	if libPath == "" {
		...
		if libPath == "" {
			// NeuroDesk: default to the platform-native shared library
