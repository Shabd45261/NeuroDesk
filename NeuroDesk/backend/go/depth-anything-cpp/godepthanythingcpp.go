...
// matches (case-sensitive), or "" if absent. Mirrors how other NeuroDesk backends
...
// statistics + camera pose as a JSON string. NeuroDesk wraps the string into the
...
func writeDepthPNG(dst string, depth []float32, h, w int) error {
	...
	// dst is the gRPC-provided output path chosen by the NeuroDesk core (the
