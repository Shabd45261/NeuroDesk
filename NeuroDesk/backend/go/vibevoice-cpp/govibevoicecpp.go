...
// convention is that Options[] carry paths relative to the NeuroDesk
...
func (v *VibevoiceCpp) Load(opts *pb.ModelOptions) error {
	...
	// ModelPath is the NeuroDesk core's models root, propagated over
...
func (v *VibevoiceCpp) AudioTranscription(_ context.Context, req *pb.TranscriptRequest) (pb.TranscriptResult, error) {
	...
	for i, s := range segs {
		// NeuroDesk's whisper backend uses int64 100ns ticks for
