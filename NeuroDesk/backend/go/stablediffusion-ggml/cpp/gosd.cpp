...
int load_model(const char *model, char *model_path, char* options[], int threads, int diff) {
    ...
    // LLAMACPP_GRPC_SERVERS env var. NeuroDesk's p2p worker mode populates that
...
static int ffmpeg_mux_raw_to_mp4(sd_image_t* frames, int num_frames, int fps,
    ...
    if (pid == 0) {
        ...
        // Force MP4 container. Distributed NeuroDesk hands us a staging
        // path (e.g. /staging/neurodesk-output-NNN.tmp) with a non-standard
