...
// the NeuroDesk convention and, crucially, lets multiple stores each have their
// own Valkey config (a face registry on one server, a router cache on another)
// within a single NeuroDesk process — something a single VALKEY_* env surface
...
const (
	...
	// this name so operators can spot NeuroDesk's traffic via CLIENT LIST. It is
	// always set on the client, even if the operator clears the client_name option.
	_defaultClientName = "neurodesk-valkey-store"
