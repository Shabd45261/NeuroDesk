...
// cloud-proxy is a NeuroDesk backend that forwards request traffic to an
...
func main() {
	...
	// captured by NeuroDesk and tee'd to a log file. Force plain text when
	// NEURODESK_LOG_FORMAT is unset and stdout isn't a terminal.
	format := os.Getenv("NEURODESK_LOG_FORMAT")
	if format == "" && !term.IsTerminal(int(os.Stdout.Fd())) {
		format = xlog.TextFormat
	}
	xlog.SetLogger(xlog.NewLogger(xlog.LogLevel(os.Getenv("NEURODESK_LOG_LEVEL")), format))
	flag.Parse()
