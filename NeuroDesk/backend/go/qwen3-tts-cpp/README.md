...
The following Base models accept NeuroDesk Voice Library profiles:
...
```bash
neurodesk models install qwen3-tts-cpp
```
...
results and rejects saved `neurodesk://voice-profiles/...` references. It does
not disable Qwen's named-speaker or VoiceDesign modes. Setting it to `true`
cannot add cloning to a backend that lacks NeuroDesk's reference-audio contract.
...
curl http://localhost:8080/v1/audio/speech \
  ...
  -d '{
    ...
    "voice": "neurodesk://voice-profiles/PROFILE_ID"
  }' \
  --output speech.wav
```
