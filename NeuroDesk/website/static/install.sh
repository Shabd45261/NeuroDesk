...
#
# Environment:
#   NEURODESK_VERSION   pin a release, for example v4.7.1 (default: latest)
#   INSTALL_DIR       where to put the binary (default: /usr/local/bin,
#                     falling back to ~/.local/bin when that is not writable)
#   NEURODESK_FORCE     set to 1 to reinstall even if the same build is present
#   NEURODESK_NO_SUDO   set to 1 to never escalate, use ~/.local/bin instead
#   GITHUB_TOKEN      optional, only used to avoid GitHub API rate limits

set -eu

REPO="mudler/NeuroDesk"
...
cleanup() {
	if [ -n "$TMPDIR_NEURODESK" ] && [ -d "$TMPDIR_NEURODESK" ]; then
		rm -rf "$TMPDIR_NEURODESK"
	fi
}
	...
	case "$uname_s" in
		...
		*)
			die "unsupported operating system: ${uname_s}" \
				"NeuroDesk publishes binaries for Linux and macOS. On Windows, use WSL2 or the container image: docker run -p 8080:8080 neurodesk/neurodesk:latest"
			;;
	esac
	...
	case "$uname_m" in
		...
		*)
			die "unsupported CPU architecture: ${uname_m}" \
				"NeuroDesk publishes amd64 and arm64 binaries. Build from source instead: https://localai.io/docs/getting-started/build/"
			;;
	esac

	if [ "$OS" = "darwin" ] && [ "$ARCH" = "amd64" ]; then
		die "there is no Intel macOS binary in the NeuroDesk releases" \
			"On an Intel Mac, use the container image (docker run -p 8080:8080 neurodesk/neurodesk:latest) or build from source: https://localai.io/docs/getting-started/build/"
	fi
}
...
resolve_version() {
	if [ -n "${NEURODESK_VERSION:-}" ]; then
		VERSION="$NEURODESK_VERSION"
		case "$VERSION" in
			v*) ;;
			*) VERSION="v${VERSION}" ;;
		esac
	...
	if [ -z "$VERSION" ]; then
		die "could not work out the latest NeuroDesk release" \
			"The GitHub API may be rate limiting you. Pin a version instead, for example: NEURODESK_VERSION=v4.7.1 sh install.sh"
	fi
}
...
choose_target() {
	...
	if [ "${NEURODESK_NO_SUDO:-0}" != "1" ] && have sudo; then
		SUDO="sudo"
		return 0
	fi
...
SUMS="NeuroDesk-${VERSION}-checksums.txt"

choose_target
step "install to    ${TARGET_DIR}${SUDO:+ (via sudo)}"

TMPDIR_NEURODESK="$(mktemp -d 2>/dev/null || mktemp -d -t neurodesk)" ||
...
EXPECTED=""
if fetch_to_file "${DL}/${VERSION}/${SUMS}" "${TMPDIR_NEURODESK}/${SUMS}" 2>/dev/null; then
	EXPECTED="$(awk -v f="$ASSET" '$2 == f || $2 == "*" f {print $1}' "${TMPDIR_NEURODESK}/${SUMS}" | head -n 1)"
fi

if [ -n "$EXPECTED" ] && [ "${NEURODESK_FORCE:-0}" != "1" ] && [ -x "${TARGET_DIR}/${BIN}" ]; then
...
if ! fetch_to_file "${DL}/${VERSION}/${ASSET}" "${TMPDIR_NEURODESK}/${BIN}"; then
...
chmod 0755 "${TMPDIR_NEURODESK}/${BIN}"

# Write to a sibling name and rename, so a running neurodesk is never replaced
# underneath itself and a failed copy cannot leave a half-written binary.
if ! $SUDO cp "${TMPDIR_NEURODESK}/${BIN}" "${TARGET_DIR}/${BIN}.new"; then
