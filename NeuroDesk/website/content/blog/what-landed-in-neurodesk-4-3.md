---
title: "NeuroDesk 4.3: signed backends, and the prompt cache that was off"
date: 2026-05-24
...
Here is a gap that had been sitting in NeuroDesk for a while. The gallery YAML tells NeuroDesk which OCI image to pull for a backend, and then NeuroDesk pulls it. Nothing checked that the bytes coming back were the bytes we built. A compromised registry, or somebody in the middle, and you would never know.

4.3.0 closes that, and fixes a default that had been quietly costing everybody a lot of prefill time.

## Signed backends

Every backend image merged by CI is now signed with [sigstore](https://www.sigstore.dev/)/cosign, keyless via Fulcio and Rekor, including each per-arch entry under the manifest list ([#9823](https://github.com/mudler/NeuroDesk/pull/9823)). It uses OCI 1.1 referrers rather than the legacy `:tag.sig` convention.
...
verification:
  ...
  identity_regex: "^https://github\\.com/mudler/NeuroDesk/\\.github/workflows/backend_merge\\.yml@.*$"
...
Strict mode is `--require-backend-integrity`, or `NEURODESK_REQUIRE_BACKEND_INTEGRITY=true`. It turns a missing policy or an empty SHA256 from a warning into a hard failure.
...
`llama-cpp` has a server-side prompt cache. NeuroDesk was not enabling it. So every agent turn, every coding-assistant call, every OpenAI-compatible CLI with a long system prompt, re-prefilled that whole prompt from scratch.
...
Two defaults flipped ([#9925](https://github.com/mudler/NeuroDesk/pull/9925), [#9951](https://github.com/mudler/NeuroDesk/pull/9951)):
...
The usage page could tell you how many tokens were spent. It could not tell you who spent them ([#9920](https://github.com/mudler/NeuroDesk/pull/9920)).
...
If you run NeuroDesk in production, the two things to look at here are strict mode (once your gallery has a `verification:` block) and whether the prompt cache change speeds up your workload. I would like to hear numbers from real setups, mine are one data point.

[Full release notes](https://github.com/mudler/NeuroDesk/releases/tag/v4.3.0).
