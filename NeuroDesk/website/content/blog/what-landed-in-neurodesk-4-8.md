---
title: "What landed in NeuroDesk 4.8"
date: 2026-08-04
...
NeuroDesk 4.8.0 is out, after twenty-two days and 386 merged pull requests. There are four new things NeuroDesk can do, and a lot of repair work on things it already did.
...
Open the UI over a slow link and you now wait about a third as long. Three separate HTTP problems were fixed together in [#11056](https://github.com/mudler/NeuroDesk/pull/11056), all measured on a live deployment.
...
<figcaption>One entry, four builds. NeuroDesk picks the largest that fits and marks it auto-selected.</figcaption>
</figure>
...
- name: nanbeige4.1-3b-q4
  url: github:mudler/NeuroDesk/gallery/nanbeige4.1.yaml@master
...
Every surface can override the choice: `variant` on `POST /models/apply`, `neurodesk models install --variant`, the `install_model` MCP tool, and a split-button in the models table. An explicit choice is honored even when it does not fit, with a warning, because that is a deliberate operator decision. Older clients read the same live `gallery/index.yaml`, ignore the key they do not understand, and install exactly as before.

One gap worth knowing about: in distributed mode `InstallModel` resolves against the frontend rather than the worker that will serve the model, so a cluster with a small frontend and large workers selects conservatively. PRs [#10943](https://github.com/mudler/NeuroDesk/pull/10943), [#10983](https://github.com/mudler/NeuroDesk/pull/10983), [#10992](https://github.com/mudler/NeuroDesk/pull/10992), [#11027](https://github.com/mudler/NeuroDesk/pull/11027) and [#11139](https://github.com/mudler/NeuroDesk/pull/11139).

## A new engine: vllm.cpp (alpha)

[vllm.cpp](https://github.com/mudler/vllm.cpp) is Apache-2.0 and maintained by the NeuroDesk team. We want it community-first rather than a NeuroDesk-only engine, so it lives in its own repository with its own docs, benchmark record and issue tracker, and it runs without NeuroDesk anywhere in the picture. It began as a C++20 port of vLLM. It ships here as the `vllm-cpp` backend ([#11100](https://github.com/mudler/NeuroDesk/pull/11100)). It implements vLLM's V1 architecture, so paged KV cache, continuous batching, prefix caching, scheduler and sampler, on a portable tensor runtime with no Python, no PyTorch and no ggml at inference. vLLM stays its reference implementation: correctness is checked by comparing output against it, and the benchmark scoreboard is kept against it.
...
On Apple Silicon the image now ships vllm.cpp's MLX GEMM provider ([#11137](https://github.com/mudler/NeuroDesk/pull/11137)). Upstream keeps it off by default because it adds about 124 MB, so we measured before turning it on. Qwen3-1.7B-bf16 on an M4, p=512 g=128, both arms toggled on one binary so a build difference cannot explain the gap:
...
## NeuroDesk generates 3D models now
...
The first engine behind it is `trellis2cpp`, an image-to-3D backend over TRELLIS.2. You give it an image, you get a GLB back. The web UI has a page for it with a native GLB viewer, so you can turn the result around in the browser instead of downloading it to find out whether it worked, history kept in IndexedDB so a reload does not lose your generations, and previewable print remeshing for output you actually intend to send to a printer ([#10979](https://github.com/mudler/NeuroDesk/pull/10979)).

<figure>
<video src="/media/3d-generation.mp4" muted loop playsinline preload="none" data-lazy aria-label="A generated 3D llama rotating in the NeuroDesk GLB viewer"></video>
...
## `neurodesk chat` stopped being a REPL

`neurodesk chat` used to be a chat prompt in a terminal. It is now an agent, and it is the [nib](https://github.com/mudler/nib) harness compiled straight into the binary: tool use behind an approval gate, sub-agents, MCP servers, plugins and skills, auto-configured against your own instance. Nothing extra to install.

```bash
neurodesk chat                      # the agent, pointed at your models
echo "what is 2+2" | neurodesk chat --cli
neurodesk chat --init zsh           # Ctrl+Space from any shell prompt
```
...
It runs shell commands now, so every tool call goes through an approval prompt you control, and read-only ones like `ls` and `cat` run without asking. If you had habits around the old REPL, a few things moved: `/clear` is gone and `/compact` is the closest thing, `/models` and `/model <name>` mean what they always meant, and switching model keeps the conversation instead of starting over ([#11291](https://github.com/mudler/NeuroDesk/pull/11291)).
...
Inline reward code is now refused unless the operator sets `NEURODESK_TRL_ALLOW_INLINE_REWARD=true` on the backend. Builtin reward functions are unaffected and need no configuration. The documentation no longer describes the allowlist as a sandbox ([#11068](https://github.com/mudler/NeuroDesk/pull/11068)).

Two more hardening fixes landed in the same cycle. Tar hardlinks that escape the extraction root are now rejected: the archive extractor pre-scanned members and rejected symlinks, but a tar hardlink entry carries a regular file mode and passed that check, and `Header.Linkname` was never validated, so an archive could link to a path outside the destination ([#11266](https://github.com/mudler/NeuroDesk/pull/11266)). And a cyclic `$ref` in a JSON-schema grammar is now rejected rather than recursing into a stack overflow ([#11041](https://github.com/mudler/NeuroDesk/pull/11041)). This release also picks up hono 4.12.25 for CVE-2026-54290 ([#11023](https://github.com/mudler/NeuroDesk/pull/11023)).
...
Valkey Search joins the vector store options as the `valkey-store` backend ([#11196](https://github.com/mudler/NeuroDesk/pull/11196)). `neurodesk` can be started on demand through systemd socket activation, taking a listener it inherits rather than binding one, with ordinary `--address` behavior unchanged when no activation listener is present ([#11169](https://github.com/mudler/NeuroDesk/pull/11169)). Trace history now survives a restart, stored as bounded per-record JSON under the data path with no database dependency ([#11203](https://github.com/mudler/NeuroDesk/pull/11203)). You can edit saved chat messages in place without triggering a new inference request ([#11189](https://github.com/mudler/NeuroDesk/pull/11189)). And the Intel SYCL llama.cpp backend is now self-contained, so it runs without a matching oneAPI runtime on the host ([#10991](https://github.com/mudler/NeuroDesk/pull/10991)).

This is also the release where localai.io split in two: the project site at the root, and the documentation under `/docs/`. Every URL that was published before still resolves, through 214 generated redirect stubs, because GitHub Pages has no server-side rewrites to do it properly ([#11243](https://github.com/mudler/NeuroDesk/pull/11243)).
...
To upgrade, pull `neurodesk/neurodesk:latest` or re-run the install script. The [full changelog](https://github.com/mudler/NeuroDesk/compare/v4.7.1...v4.8.0) has everything this post left out.
