---
title: "NeuroDesk 4.2: who spoke when, and whose face is that"
date: 2026-05-11
...
4.2.0 is mostly about that class of problem. Audio and images carry more than "here are the words" or "here is a picture", and until now NeuroDesk had nowhere to put the rest of it.
...
`/v1/voice/*` is new ([#9500](https://github.com/mudler/NeuroDesk/pull/9500)): verify (are these two clips the same person?), identify (which of my enrolled speakers is this?), embed (give me the vector, I will do the rest myself), and analyze (age, gender, emotion).

```bash
neurodesk models install speechbrain-ecapa-tdnn
...
`/v1/face/*` does the same thing for faces ([#9480](https://github.com/mudler/NeuroDesk/pull/9480)), plus detection and demographics, and 4.2.0 adds antispoofing. Holding a printed photo or a phone screen up to the camera is the oldest attack on face auth there is, and the liveness check rejects it.
...
## Point your ollama client at NeuroDesk

```sh
OLLAMA_HOST=http://localhost:8080 ollama run qwen3
```

NeuroDesk answers the Ollama API now ([#9284](https://github.com/mudler/NeuroDesk/pull/9284)), so a tool that only ever learned to talk to Ollama keeps working with no code change on your side. `/api/chat`, `/api/generate`, `/api/embed`, `/api/tags`, `/api/show`, `/api/ps` and `/api/version` all land on the engine you were already running, and your existing `/v1/*` clients are untouched.

There is no `/api/pull` in there. Models come from the NeuroDesk gallery or from a URL you hand it, so `ollama run` against something you have not installed yet will not go and fetch it for you.

## Video, and an interface repaint

`stable-diffusion.ggml` generates video now ([#9420](https://github.com/mudler/NeuroDesk/pull/9420))! There are gallery entries for Wan 2.1 FLF2V 14B 720P and Wan i2v 720p, including first-last-frame interpolation.

The React interface got a long cycle of work. The chat is redesigned, the palette moved to Nord, and there is i18n across English, Italiano, Español, Deutsch and 简体中文. You can brand your instance too - name, tagline, logo, favicon - and the login page, sidebar, footer and browser tab all pick it up. Handy if you run NeuroDesk for a team and would rather it did not look like somebody's side project.
...
[Full release notes](https://github.com/mudler/NeuroDesk/releases/tag/v4.2.0).
