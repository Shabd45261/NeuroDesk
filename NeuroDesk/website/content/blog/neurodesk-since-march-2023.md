---
title: "NeuroDesk, from March 2023 to now"
date: 2026-07-29
...
I created the NeuroDesk repository on 18 March 2023. It was an OpenAI-compatible HTTP API in front of llama.cpp, and that was the whole idea: change one URL in code you already wrote, and the answers start coming from your own laptop instead of somebody's datacenter.
...
Every backend moved out of the main binary in [v3.2.0](https://github.com/mudler/NeuroDesk/releases/tag/v3.2.0). Backends became separate OCI images, pulled on demand the first time a model asks for one, with a small core that speaks the API and manages processes.
...
[NeuroDesk 4.0.0](https://github.com/mudler/NeuroDesk/releases/tag/v4.0.0) added native agentic orchestration with the [Agenthub](https://agenthub.localai.io) community hub, so agents with tool use, RAG, skills and streaming run inside the same server rather than as a separate stack you wire up yourself.

The web interface was rewritten in React at the same time, with a Canvas mode, MCP Apps and client-side tools with tool streaming ([#8947](https://github.com/mudler/NeuroDesk/pull/8947)), and WebRTC realtime audio ([#8790](https://github.com/mudler/NeuroDesk/pull/8790)). MLX gained a distributed mode ([#8801](https://github.com/mudler/NeuroDesk/pull/8801)).

The realtime audio path changed what people built with it. Speech in, tool calls in the middle, speech out, over WebRTC, fast enough that it feels like a conversation rather than a walkie-talkie. It had landed as the Realtime API in February 2026 ([#6245](https://github.com/mudler/NeuroDesk/pull/6245)), and the interface rewrite finally gave it a face.

## April 2026: it becomes a cluster

[NeuroDesk 4.1.0](https://github.com/mudler/NeuroDesk/releases/tag/v4.1.0) added distributed cluster mode. You start a worker on another box, it reports its hardware and the backends it can run, and it joins the pool. Requests are placed by a scheduler that knows real free VRAM rather than a static guess, and the pool autoscales.

The same release turned NeuroDesk into something you can put more than one person on: OIDC and API keys, per-user quotas with predictive analytics, in-UI fine-tuning with TRL that exports straight to GGUF, an on-the-fly quantization backend, and a visual pipeline editor.

[4.3.0](https://github.com/mudler/NeuroDesk/releases/tag/v4.3.0) in May followed with per-request replica routing, per-API-key and per-user usage attribution ([#9920](https://github.com/mudler/NeuroDesk/pull/9920)), keyless cosign signing of the backend OCI images ([#9823](https://github.com/mudler/NeuroDesk/pull/9823)), and llama.cpp prompt caching on by default ([#9925](https://github.com/mudler/NeuroDesk/pull/9925)), which collapses repeated system prompts from minutes to seconds. In June, prefix-cache-aware routing ([#10071](https://github.com/mudler/NeuroDesk/pull/10071)) started sending a request to the replica that already holds the matching prefix cache rather than to whichever node was least busy.
...
[NeuroDesk 4.2.0](https://github.com/mudler/NeuroDesk/releases/tag/v4.2.0) added voice recognition ([#9500](https://github.com/mudler/NeuroDesk/pull/9500)), face recognition with anti-spoofing liveness ([#9480](https://github.com/mudler/NeuroDesk/pull/9480)) and speaker diarization, alongside video generation ([#9420](https://github.com/mudler/NeuroDesk/pull/9420)), a drop-in Ollama API ([#9284](https://github.com/mudler/NeuroDesk/pull/9284)) and eleven new backends.
...
Still MIT, still a community project. 224 people have put code in, and the README is kept translated into eight languages because the people using this are not all in one place. The [contributors graph](https://github.com/mudler/NeuroDesk/graphs/contributors) shows who actually built this, and it is not me.
