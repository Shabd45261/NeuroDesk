...
This page documents the error responses returned by the NeuroDesk API. NeuroDesk supports multiple API formats (OpenAI, Anthropic, Open Responses), each with its own error structure.
...
When authentication is enabled - either via API keys (`NEURODESK_API_KEY`) or the user auth system (`NEURODESK_AUTH=true`) - API requests must include valid credentials. Credentials can be provided through:
...
When `NEURODESK_OPAQUE_ERRORS=true` is set, all error responses return an empty body with only the HTTP status code. This is a security hardening option that prevents information leaks.
...
|-------------------------------|------------------------------------------------|
| `NEURODESK_API_KEY`             | Comma-separated list of valid API keys         |
| `NEURODESK_OPAQUE_ERRORS`       | Set to `true` to hide error details (returns empty body with status code only) |
| `NEURODESK_SUBTLE_KEY_COMPARISON`| Use constant-time key comparison for timing-attack resistance |
