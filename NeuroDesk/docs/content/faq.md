...
No. You install only the backends your models use. NeuroDesk's core is a single binary (or container) that provides the OpenAI-compatible API, request routing, the web UI, and agents. Each inference backend (llama.cpp, vLLM, whisper.cpp, stable-diffusion, MLX, and others) is a separate artifact, installed only when a model needs it.

In practice:

- **You install one backend, not all of them.** Run a model with `neurodesk run <model>` and the matching backend is pulled automatically; nothing else is downloaded.
- **Each backend is purpose-built for its engine.** NeuroDesk builds a dedicated gRPC backend around each engine, so every one stays independently optimized without a single binary trying to support every model architecture at once.
- **You manage backends individually** with `neurodesk backends list/install/uninstall` or from the web UI.
...
The easiest way is the built-in model gallery: open the web interface at `http://localhost:8080`, go to the Models page, and install a model by name. You can also install from the command line with `neurodesk models install <model-name>`. Most gguf-based models work, and you can point NeuroDesk at any compatible GGUF from Hugging Face (https://huggingface.co/models?search=gguf). Be cautious about downloading models from untrusted sources, as there may be security vulnerabilities in llama.cpp or ggml that could be maliciously exploited.

### Where are models stored?

NeuroDesk stores downloaded models in the following locations by default:
...
- **Launcher application**: `~/.neurodesk/models` (in your home directory)

You can customize the model storage location using the `NEURODESK_MODELS_PATH` environment variable or `--models-path` command line flag. This is useful if you want to store models outside your home directory for backup purposes or to avoid filling up your home directory with large model files.
...
### Benchmarking NeuroDesk and llama.cpp shows different results!

NeuroDesk applies a set of defaults when loading models with the llama.cpp backend, one of these is mirostat sampling - while it achieves better results, it slows down the inference. You can disable this by setting `mirostat: 0` in the model config file. See also the advanced section ({{%relref "advanced/advanced-usage" %}}) for more information and [this issue](https://github.com/mudler/NeuroDesk/issues/2780).
...
Yes! If the client uses OpenAI and supports setting a different base URL to send requests to, you can use the NeuroDesk endpoint. This allows to use this with every application that was supposed to work with OpenAI, but without changing the application!
...
NeuroDesk ships with a built-in web interface. Once NeuroDesk is running, open `http://localhost:8080` in your browser to manage models, chat, and configure the server. There is nothing extra to install. Because NeuroDesk also exposes an OpenAI-compatible API, you can additionally plug it into any existing application that talks to the OpenAI API by pointing that application at the NeuroDesk endpoint.
