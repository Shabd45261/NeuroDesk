...
NeuroDesk can forward chat-completion and Anthropic Messages requests to an
...
and NeuroDesk bypasses templating, MCP injection, and the local model loader
...
- Mix local and cloud models in the same NeuroDesk instance - clients hit one
  endpoint, NeuroDesk dispatches per model.
- Apply NeuroDesk's auth, usage tracking, and PII redaction to cloud traffic
...
1. Request hits NeuroDesk on `/v1/chat/completions` (OpenAI-shaped) or
...
# Useful when the NeuroDesk alias differs from the upstream's canonical name.
proxy:
  mode: passthrough
  provider: openai
...
Then start NeuroDesk with the API key in the environment:
...
In translate mode the cloud-proxy backend converts NeuroDesk's internal proto
