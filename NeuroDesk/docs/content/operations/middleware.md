...
NeuroDesk ships a request-middleware layer that sits between the HTTP API and
...
its response, NeuroDesk restores the original value before returning JSON or SSE
...
applies to NeuroDesk API routes; the MITM proxy keeps its own output-redaction
policy.
...
`NEURODESK_PII_DEFAULT_DETECTORS` environment variable (comma-separated model
...
The same surface is mirrored through the NeuroDesk Assistant MCP server:
...
NeuroDesk ships three classifier implementations. Pick one with `classifier:`
in the router YAML:
...
router:
  ...
  knn:
    ...
    # embedding_revision: "2026-07" # bump for remote/in-place weight changes NeuroDesk cannot identify
...
the live in-memory index, NeuroDesk fails the classifier build instead of
querying mixed embedding spaces. Restart NeuroDesk to rebuild the empty live
...
router:
  ...
  # A model loaded by NeuroDesk that supports the Score gRPC primitive
...
- [MITM proxy]({{< relref "mitm-proxy.md" >}}) - apply the same PII
  ...
  NeuroDesk holding their API keys.
