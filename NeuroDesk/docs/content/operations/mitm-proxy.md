...
description = "Redact PII from cloud-AI traffic without NeuroDesk holding API keys"
...
NeuroDesk can act as a local HTTPS proxy that **redacts PII from your Claude
...
- You want to use **Claude Code with a Claude Pro/Max subscription** but still
  apply the same PII redaction NeuroDesk applies to API-key traffic.
- You run Codex CLI on a corporate laptop and need an audit trail of prompts.
- You want NeuroDesk to enforce egress policies for AI traffic without
...
2. Clients set `HTTPS_PROXY=http://neurodesk:port` and add the CA to their
...
would. NeuroDesk never holds the credential - it just observes and rewrites
the request body.

## Quick start

Start NeuroDesk with the MITM listener:

```bash
neurodesk run --mitm-listen :8443
```
...
gets its prompts and tool inputs scanned by NeuroDesk's PII filter, and any
PII the model emits in its streaming response is masked before reaching
your terminal. Events appear in the NeuroDesk middleware admin page under
...
|---|---|---|
| `--mitm-listen` / `NEURODESK_MITM_LISTEN` | empty (disabled) | Address to bind the proxy listener on |
| `--mitm-ca-dir` / `NEURODESK_MITM_CA_DIR` | `<data-path>/mitm-ca` | Where to persist the CA cert + key |
...
Events are persisted via the same `pii.EventStore` the rest of NeuroDesk
...
- The proxy listener accepts plaintext HTTP `CONNECT` requests - bind it
  ...
  port.
- The MITM CA is **separate** from any TLS cert NeuroDesk's main HTTP API
...
NeuroDesk ships two cloud-related proxy modes; pick by who holds the credential:

|  | Cloud-proxy backend (`backend: proxy-*`) | MITM proxy (`--mitm-listen`) |
|---|---|---|
| Client config | `neurodesk:8080` as **API endpoint** | `neurodesk:8443` as **HTTPS_PROXY** |
| Holds API key | NeuroDesk | Client (CLI's own auth) |
...
| Routes through NeuroDesk's auth/usage tracking | Yes | Yes (per-correlation-id events) |

For shared deployments where NeuroDesk owns the API key and clients are
