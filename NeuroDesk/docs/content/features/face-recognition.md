...
NeuroDesk supports face recognition: face verification (1:1), face
...
```bash
neurodesk models install face-detect-yunet-sface
```
...
```bash
neurodesk models install insightface-opencv
```
...
NeuroDesk's built-in in-memory vector store - no external database to
stand up.
...
registered faces are lost when NeuroDesk restarts. Persistent storage
