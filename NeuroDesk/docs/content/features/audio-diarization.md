...
NeuroDesk exposes this through the `/v1/audio/diarization` endpoint, modelled after `/v1/audio/transcriptions`. Four backends are supported today:
...
```bash
neurodesk run vibevoice-cpp-asr
```
...
Sherpa-onnx needs two ONNX models: pyannote segmentation and a speaker-embedding extractor. Place them under your NeuroDesk models directory and reference them from the YAML:
...
Both `model:` and `diarize.embedding_model=` are resolved relative to the NeuroDesk models directory.
