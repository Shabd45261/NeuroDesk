...
NeuroDesk supports generating embeddings for text or list of tokens.
...
NeuroDesk provides a model gallery with pre-configured embedding models. To use a gallery model:
...
- The `sentencetransformers` backend is an optional backend of NeuroDesk and uses Python. If you are running `NeuroDesk` from the containers you are good to go and should be already configured for use.
- For local execution, you also have to specify the extra backend in the `EXTERNAL_GRPC_BACKENDS` environment variable.
    - Example: `EXTERNAL_GRPC_BACKENDS="sentencetransformers:/path/to/NeuroDesk/backend/python/sentencetransformers/sentencetransformers.py"`
...
`/v1/embeddings` also accepts a chat conversation via `messages` (a NeuroDesk
extension), plus a per-request `pooling` scheme that NeuroDesk applies itself to
...
declares whether an embedding result is final or per-token; NeuroDesk rejects a
...
llama.cpp chooses this layout when the model is loaded. NeuroDesk automatically
...
- Example that uses LLamaIndex and NeuroDesk as embedding: [here](https://github.com/mudler/NeuroDesk-examples/tree/main/query_data).
