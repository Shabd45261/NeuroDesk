...
Object detection in NeuroDesk is implemented through dedicated backends that can identify and locate objects within images. Each backend provides different capabilities and model architectures.
...
NeuroDesk provides a dedicated `/v1/detection` endpoint for object detection tasks. This endpoint is specifically designed for object detection and returns structured detection results with bounding boxes and confidence scores.
...
The RF-DETR backend is implemented as a Python-based gRPC service that integrates seamlessly with NeuroDesk. It provides object detection capabilities using the RF-DETR model architecture and supports multiple hardware configurations:
...
1. **Using the Model Gallery (Recommended)**
   ...
   The easiest way to get started is using the model gallery. The `rfdetr-base` model is available in the official NeuroDesk gallery:

   ```bash
   # Install and run the rfdetr-base model
   neurodesk run rfdetr-base
   ```
...
You can browse and install this model through the NeuroDesk web interface or using the command line.
...
1. **Install the backend**
   ...
   ```bash
   neurodesk backends install rfdetr-cpp
   ```
...
2. **Using the Model Gallery (Recommended)**
   ...
   ```bash
   # Detection variants
   neurodesk run rfdetr-cpp-nano
   neurodesk run rfdetr-cpp-small
   neurodesk run rfdetr-cpp-base
   neurodesk run rfdetr-cpp-medium
   neurodesk run rfdetr-cpp-large

   # Segmentation variants (return per-instance PNG masks)
   neurodesk run rfdetr-cpp-seg-nano
   neurodesk run rfdetr-cpp-seg-small
   neurodesk run rfdetr-cpp-seg-medium
   neurodesk run rfdetr-cpp-seg-large
   neurodesk run rfdetr-cpp-seg-xlarge
   neurodesk run rfdetr-cpp-seg-2xlarge
   ```
...
```bash
neurodesk run --debug rfdetr-base
```

## Object Detection Category

NeuroDesk includes a dedicated **object-detection** category for models and backends that specialize in identifying and locating objects within images. This category currently includes:
