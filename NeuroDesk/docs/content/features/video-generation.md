...
NeuroDesk can generate videos from text prompts and optional image or audio conditioning via the `/video` endpoint. Supported backends include `diffusers`, `stablediffusion`, `vllm-omni`, `vllm-cpp` (MiniMax-H3, which generates video **and** audio together), and the dedicated `longcat-video` backend.
...
NeuroDesk's `longcat-video` backend serves Meituan's official LongCat video-generation models through the `/video` API and the Studio **Video** page.
...
{{% notice warning %}}
LongCat is a large, CUDA-only model family. NeuroDesk publishes this backend for Linux with NVIDIA CUDA 12 or CUDA 13 on x86_64 and CUDA 13 on ARM64. CPU, ROCm, and macOS images are not available. Avatar 1.5 also loads components from the base checkpoint, so reserve substantial disk and GPU or unified memory.
{{% /notice %}}
...
```bash
neurodesk models install longcat-video
neurodesk models install longcat-video-avatar-1.5
```
...
The required OCI backend is installed automatically when NeuroDesk first loads the model. The hardware detector selects the CUDA 12, CUDA 13, or CUDA 13 ARM64 variant.

#### DGX Spark and NVIDIA ARM64

Use a NeuroDesk CUDA 13 ARM64 image as described in [GPU acceleration]({{%relref "features/GPU-acceleration" %}}). The backend defaults to PyTorch SDPA, avoiding the FlashAttention dependency that is commonly unavailable on Blackwell ARM64 systems.
...
Avatar output is generated at 25 FPS and is muxed with the submitted audio. When neither `num_frames` nor `params.num_segments` is provided, NeuroDesk derives the continuation count from the audio duration, up to the model's `max_segments` setting.
...
- **HTTP 412**: the installed NeuroDesk runtime cannot select a compatible NVIDIA backend image.
...
The `vllm-cpp` backend — NeuroDesk's own C++ port of vLLM — also serves MiniMax-H3, which generates **video and audio jointly**. The clip comes back as an MP4 with a real AAC track rather than a silent render.
...
```bash
neurodesk models install minimax-h3-fl2va-q4
```
...
- The frame count sits on a **17n+5** grid (…, 90, 107, 124, 141, …). A count off the grid is rounded up, and NeuroDesk logs the value it actually rendered.
...
The engine writes frames and a WAV and composes the `ffmpeg` command line; NeuroDesk runs it. That process boundary is deliberate upstream, so the backend image ships no `ffmpeg`: install one on the host, or point `options: [ffmpeg:/path/to/ffmpeg]` at a binary. Without it, generation succeeds and the mux fails with a message saying so.
