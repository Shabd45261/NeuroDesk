...
With the stores feature you can now do it through the NeuroDesk API. 
...
Two backends ship with NeuroDesk:
...
the data survives a NeuroDesk restart — unlike the in-memory default. It requires a reachable server
...
| `client_name` | `neurodesk-valkey-store` | Connection name reported by `CLIENT LIST`. Always set. |
