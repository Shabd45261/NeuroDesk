...
NeuroDesk provides a web-based interface for managing application settings at runtime. These settings can be configured through the web UI and are automatically persisted to a configuration file, allowing changes to take effect immediately without requiring a restart.

## Accessing Runtime Settings

Navigate to the **Settings** page from the management interface at `http://localhost:8080/manage`. The settings page provides a comprehensive interface for configuring various aspects of NeuroDesk.
...
By default, NeuroDesk will skip evicting models that have active API calls to prevent interrupting ongoing requests. When all models are busy and eviction is needed:
...
- **Load and pre-warm galleries on boot**: Load model galleries and pre-warm their remote size and VRAM estimates when NeuroDesk starts. Disable this setting to skip both startup operations.
...
All settings are automatically saved to `runtime_settings.json` in the `NEURODESK_CONFIG_DIR` directory (default: `BASEPATH/configuration`). This file is watched for changes, so modifications made directly to the file will also be applied at runtime.
...
- An environment variable explicitly set to its default value is indistinguishable from an unset one, so the value from `runtime_settings.json` wins in that case. In particular, an explicit `NEURODESK_THREADS=0` behaves like an unset `NEURODESK_THREADS`.
  ...
  "galleries": [
    {
      ...
      "mirrors": ["github:mudler/NeuroDesk/gallery/index.yaml@master"],
      "name": "neurodesk"
    }
  ],
  "backend_galleries": [
    {
      ...
      "mirrors": ["github:mudler/NeuroDesk/backend/index.yaml@master"],
      "name": "neurodesk"
    }
  ],
...
The runtime settings system supports dynamic configuration file watching. When `NEURODESK_CONFIG_DIR` is set, NeuroDesk monitors the following files for changes:
...
4. **Secure API Keys**: API keys are sensitive information. Ensure proper file permissions on configuration files (they should be readable only by the NeuroDesk process).
...
2. Verify the `NEURODESK_CONFIG_DIR` is set correctly
