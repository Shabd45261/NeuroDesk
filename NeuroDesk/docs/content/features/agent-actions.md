...
You can add your own actions without rebuilding NeuroDesk by pointing it at a directory of custom action definitions:

```bash
NEURODESK_AGENT_POOL_CUSTOM_ACTIONS_DIR=/path/to/custom-actions neurodesk run
```
...
The actions below ship with NeuroDesk. The exact catalog can change between releases, so treat `GET /api/agents/actions` (and the Agents UI action picker) as the source of truth for the version you are running.
...
| Action | What it does | Typically requires |
|--------|--------------|--------------------|
| `generate_image` | Generate an image (through NeuroDesk's own image endpoint). | An installed image-generation model. |
