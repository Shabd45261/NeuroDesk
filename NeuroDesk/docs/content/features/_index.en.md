...
NeuroDesk provides a comprehensive set of features for running AI models locally. The pages in this section are grouped by capability, and the left navigation is ordered to match these groups.
...
- **[API discovery]({{% relref "features/api-discovery" %}})** - How NeuroDesk advertises its capability surface.
...
- **[NeuroDesk Assistant]({{% relref "features/neurodesk-assistant" %}})** - Chat to administer your NeuroDesk instance.
- **[Terminal agent]({{% relref "features/terminal-agent" %}})** - `neurodesk chat`, an agent in your shell that runs commands behind an approval gate.
...
To start using these features, make sure you have [NeuroDesk installed]({{% relref "getting-started/install" %}}) and have [downloaded some models]({{% relref "getting-started/models" %}}). Then explore the feature pages above to learn how to use each capability.
