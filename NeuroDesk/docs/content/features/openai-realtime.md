...
NeuroDesk supports the [OpenAI Realtime API](https://platform.openai.com/docs/guides/realtime) which enables low-latency, multi-modal conversations (voice and text) over WebSocket.
...
Make sure all referenced models (`silero-vad-ggml`, `whisper-large-turbo`, `qwen3-4b`, `tts-1`) are also installed or defined in your NeuroDesk instance.
...
To avoid that, NeuroDesk **warms the pipeline by default**: it loads the VAD, transcription, LLM and TTS backends into memory *before* the session is announced, and the session start **blocks until they are all ready**. The loads run concurrently, so the wait is the slowest single model, not the sum. This means:
...
### Classifier mode (NeuroDesk extension)
...
```json
{"type": "session.update", "session": {"type": "realtime", "neurodesk_classifier": {
...
Like `tools`, the option list is replaced wholesale on each update. A `response.create` may carry its own `neurodesk_classifier` to override the session for one response — `{"enabled": false}` runs normal generation once.
...
- Every classified response also emits a `neurodesk.classifier.result` event carrying the full softmax distribution, the chosen option id (empty when the fallback applied), the threshold, and the scoring latency — useful for visualizing confidence in a client UI.
...
The concrete scoring model must declare `score` in `known_usecases`. A single llama.cpp model can serve ordinary inference and classification concurrently by declaring multiple use cases, for example `known_usecases: [chat, completion, score]`; NeuroDesk reserves the scoring slots only when `score` is present. Scoring also requires the unified KV cache, which is enabled by default, so a score-enabled model cannot set `kv_unified:false`.
...
The unified OpenAI interface is also supported. Send `multipart/form-data` with an `sdp` field that contains the offer and a JSON `session` field. NeuroDesk reads the model from the session object:
...
NeuroDesk also accepts its original JSON request format for compatibility. A JSON request contains top-level `sdp` and `model` fields and receives a JSON response with `sdp` and `session_id` fields.
...
```bash
neurodesk backends install opus
```
...
NEURODESK_WEBRTC_NAT_1TO1_IPS=192.168.1.10

# ...or restrict ICE gathering to specific interfaces
NEURODESK_WEBRTC_ICE_INTERFACES=eth0
```

{{% notice tip %}}
For a browser on another LAN machine talking to NeuroDesk in a host-networked
container, set `NEURODESK_WEBRTC_NAT_1TO1_IPS` to the host's LAN IP. This is the
...
all realtime WebRTC ICE traffic through one shared port, start NeuroDesk with
`--web-rtc-udp-port 3478` or set `NEURODESK_WEBRTC_UDP_PORT=3478`.

When running in a container, publish the same port with the UDP protocol:

```bash
docker run -p 8080:8080 -p 3478:3478/udp \
  -e NEURODESK_WEBRTC_UDP_PORT=3478 neurodesk/neurodesk:latest
```

Allow the selected UDP port through the host and network firewalls. If NeuroDesk
...
`output_modalities` is the GA field name. For compatibility, NeuroDesk also accepts the legacy Realtime *beta* field name `modalities` as an alias (a lot of community sample code still sends `modalities: ["text"]`):
...
Because such a response arrives on the same socket as everything else, attach `metadata` to correlate it. NeuroDesk echoes the map back verbatim on both `response.created` and `response.done`:
...
This event is a NeuroDesk extension to the OpenAI Realtime API and is server-emitted only. Standard OpenAI Realtime clients ignore event types they do not recognize, so enabling it is non-breaking.

## Examples

- [Realtime voice assistant demo (Go)](https://github.com/localai-org/neurodesk-realtime-demo): a minimal Go client for the Realtime (WebSocket) API with a full talk-back voice loop and an example tool call. Ships a `docker compose` setup that brings up a realtime-capable NeuroDesk for you.
- [Realtime voice assistant example (Python)](https://github.com/mudler/NeuroDesk-examples/tree/main/realtime): thin-client architecture (Silero VAD on the client, heavy lifting on NeuroDesk), suited to running the client on a Raspberry Pi.
