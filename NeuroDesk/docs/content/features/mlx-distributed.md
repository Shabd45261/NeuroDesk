...
The simplest way to use MLX distributed is with NeuroDesk's P2P auto-discovery.

### 1. Start NeuroDesk with P2P

```bash
docker run -ti --net host \
  --name neurodesk \
  neurodesk/neurodesk:latest-metal-darwin-arm64 run --p2p
```
...
docker run -ti --net host \
  -e TOKEN="<your-token>" \
  --name neurodesk-mlx-worker \
  neurodesk/neurodesk:latest-metal-darwin-arm64 worker p2p-mlx
```

Workers auto-register on the P2P network. The NeuroDesk server discovers them and generates a hostfile for MLX distributed.
...
The `mlx-distributed` backend is started automatically by NeuroDesk like any other backend. You configure distributed inference through the model YAML file using the `options` field:
...
JACCL requires a **coordinator** - a TCP service that helps all ranks establish RDMA connections. Rank 0 (the NeuroDesk machine) is always the coordinator. Workers are told the coordinator address via their `--coordinator` CLI flag (see [Starting Workers](#jaccl-workers) below).
...
NeuroDesk starts the rank 0 process (gRPC server) automatically when the model is loaded. But you still need to start **worker processes** (ranks 1, 2, ...) on the other machines. These workers participate in every forward pass but don't serve any API - they wait for commands from rank 0.
...
```bash
neurodesk worker mlx-distributed --hostfile hosts.json --rank 1
```
...
- Rank 0: started automatically by NeuroDesk on `192.168.1.10`
- Rank 1: `neurodesk worker mlx-distributed --hostfile hosts.json --rank 1` on `192.168.1.11`
- Rank 2: `neurodesk worker mlx-distributed --hostfile hosts.json --rank 2` on `192.168.1.12`

### JACCL Workers

```bash
neurodesk worker mlx-distributed \
...
The `--coordinator` address is the IP of the machine running NeuroDesk (rank 0) with any available port. Rank 0 binds the coordinator service there; workers connect to it to establish RDMA connections.

### Worker Startup Order

Start workers **before** loading the model in NeuroDesk. When NeuroDesk sends the LoadModel request, rank 0 initializes `mx.distributed` which tries to connect to all ranks listed in the hostfile. If workers aren't running yet, it will time out.

## Advanced: Manual Rank 0

For advanced use cases, you can also run rank 0 manually as an external gRPC backend instead of letting NeuroDesk start it automatically:

```bash
# On Mac A: start rank 0 manually
neurodesk worker mlx-distributed --hostfile hosts.json --rank 0 --addr 192.168.1.10:50051

# On Mac B: start rank 1
neurodesk worker mlx-distributed --hostfile hosts.json --rank 1

# On any machine: start NeuroDesk pointing at rank 0
neurodesk run --external-grpc-backends "mlx-distributed:192.168.1.10:50051"
```
...
| `--addr` | `MLX_DISTRIBUTED_ADDR` | `localhost:50051` | gRPC API listen address (rank 0 only, for NeuroDesk or external access) |
...
- **All ranks download the model independently.** Each node auto-downloads from Hugging Face on first use via `mlx_lm.load()`. On rank 0 (started by NeuroDesk), models are downloaded to NeuroDesk's model directory (`HF_HOME` is set automatically). On workers, models go to the default HF cache (`~/.cache/huggingface/hub`) unless you set `HF_HOME` yourself.
...
- **Rank assignment:** In P2P mode, rank 0 is always the NeuroDesk server. Worker ranks are assigned by sorting node IDs.
