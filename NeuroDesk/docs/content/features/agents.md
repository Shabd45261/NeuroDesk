...
![The in-process agent loop: agents call NeuroDesk's own chat API in a loop, streaming progress over SSE](/images/diagrams/agents-loop.png)

NeuroDesk includes a built-in agent platform powered by [LocalAGI](https://github.com/mudler/LocalAGI). Agents are autonomous AI entities that can reason, use tools, maintain memory, and interact with external services, all running locally as part of the NeuroDesk process.

LocalAGI is embedded in NeuroDesk. There is nothing separate to install or run.
...
1. Download an agent configuration from the [Agent Hub](https://agenthub.localai.io) or export one from another NeuroDesk instance
...
|----------|---------|-------------|
| `NEURODESK_DISABLE_AGENTS` | `false` | Disable the agent pool feature entirely |
| `NEURODESK_AGENT_POOL_API_URL` | _(self-referencing)_ | Default API URL for agents. By default, agents call back into NeuroDesk's own API (`http://127.0.0.1:<port>`). Set this to point agents to an external LLM provider. |
| `NEURODESK_AGENT_POOL_API_KEY` | _(NeuroDesk key)_ | Default API key for agents. Defaults to the first NeuroDesk API key. Set this when using an external provider. |
| `NEURODESK_AGENT_POOL_DEFAULT_MODEL` | _(empty)_ | Default LLM model for new agents |
| `NEURODESK_AGENT_POOL_MULTIMODAL_MODEL` | _(empty)_ | Default multimodal (vision) model for agents |
| `NEURODESK_AGENT_POOL_TRANSCRIPTION_MODEL` | _(empty)_ | Default transcription (speech-to-text) model for agents |
| `NEURODESK_AGENT_POOL_TRANSCRIPTION_LANGUAGE` | _(empty)_ | Default transcription language for agents |
| `NEURODESK_AGENT_POOL_TTS_MODEL` | _(empty)_ | Default TTS (text-to-speech) model for agents |
| `NEURODESK_AGENT_POOL_STATE_DIR` | _(data path)_ | Directory for persisting agent state. Defaults to `NEURODESK_DATA_PATH` if set, otherwise falls back to `NEURODESK_CONFIG_DIR` |
| `NEURODESK_AGENT_POOL_TIMEOUT` | `5m` | Default timeout for agent operations |
| `NEURODESK_AGENT_POOL_ENABLE_SKILLS` | `false` | Enable the skills service |
| `NEURODESK_AGENT_POOL_VECTOR_ENGINE` | `chromem` | Vector engine for knowledge base (`chromem` or `postgres`) |
| `NEURODESK_AGENT_POOL_EMBEDDING_MODEL` | `granite-embedding-107m-multilingual` | Embedding model for knowledge base |
| `NEURODESK_AGENT_POOL_CUSTOM_ACTIONS_DIR` | _(empty)_ | Directory for custom action plugins |
| `NEURODESK_AGENT_POOL_DATABASE_URL` | _(empty)_ | PostgreSQL connection string for collections (required when vector engine is `postgres`) |
| `NEURODESK_AGENT_POOL_MAX_CHUNKING_SIZE` | `400` | Maximum chunk size for document ingestion |
| `NEURODESK_AGENT_POOL_CHUNK_OVERLAP` | `0` | Overlap between document chunks |
| `NEURODESK_AGENT_POOL_ENABLE_LOGS` | `false` | Enable detailed agent logging |
| `NEURODESK_AGENT_POOL_COLLECTION_DB_PATH` | _(empty)_ | Custom path for the collections database |
| `NEURODESK_AGENT_HUB_URL` | `https://agenthub.localai.io` | URL for the Agent Hub (shown in the UI) |
...
```bash
NEURODESK_AGENT_POOL_VECTOR_ENGINE=postgres
NEURODESK_AGENT_POOL_DATABASE_URL=postgresql://localrecall:localrecall@postgres:5432/localrecall?sslmode=disable
```
...
These are read directly from the NeuroDesk process environment by the embedded store (the same as `DATABASE_URL` and `HYBRID_SEARCH_*`).
...
services:
  neurodesk:
    image: neurodesk/neurodesk:latest
    ports:
      - 8080:8080
    environment:
      - MODELS_PATH=/models
      - NEURODESK_DATA_PATH=/data
      - NEURODESK_AGENT_POOL_DEFAULT_MODEL=hermes-3-llama3.1-8b
      - NEURODESK_AGENT_POOL_EMBEDDING_MODEL=granite-embedding-107m-multilingual
      - NEURODESK_AGENT_POOL_ENABLE_SKILLS=true
      - NEURODESK_AGENT_POOL_ENABLE_LOGS=true
    volumes:
      - models:/models
      - neurodesk_data:/data
      - neurodesk_config:/etc/neurodesk
volumes:
  models:
  neurodesk_data:
  neurodesk_config:
...
The pool-level defaults (API URL, API key, models) can be set via environment variables. Individual agents can further override these in their configuration, allowing them to use different LLM providers (OpenAI, other NeuroDesk instances, etc.) on a per-agent basis.
...
{{% notice warning %}}
Skills are **disabled by default**. The skills service only runs when you start NeuroDesk with `NEURODESK_AGENT_POOL_ENABLE_SKILLS=true`. With the default `NEURODESK_AGENT_POOL_ENABLE_SKILLS=false`, the Skills UI and the `/api/agents/skills` endpoints are inactive, and an imported agent that expects a skill will not find it.
{{% /notice %}}

### Enable skills

Start NeuroDesk with the skills service turned on:

```bash
NEURODESK_AGENT_POOL_ENABLE_SKILLS=true neurodesk run
```
...
If a skill you expect is missing, confirm NeuroDesk was started with `NEURODESK_AGENT_POOL_ENABLE_SKILLS=true`.
...
Some agent actions (image generation, PDF creation, audio synthesis) produce files. These files are automatically managed by NeuroDesk through a confined **outputs directory**.

### How It Works

1. Actions generate files to their configured `outputDir` (which can be any path on the filesystem)
2. After each agent response, NeuroDesk automatically copies generated files into `{stateDir}/outputs/`
...
The outputs directory is created at `{stateDir}/outputs/` where `stateDir` defaults to `NEURODESK_AGENT_POOL_STATE_DIR` (or `NEURODESK_DATA_PATH` / `NEURODESK_CONFIG_DIR` as fallbacks). You can query the current outputs directory path via:
...
Agents run in-process within NeuroDesk. By default, each agent calls back into NeuroDesk's own API (`http://127.0.0.1:<port>/v1/chat/completions`) for LLM inference. This means:

- No external dependencies - everything runs in a single binary
- Agents use the same models loaded in NeuroDesk
...
```
User → POST /api/agents/:name/chat → NeuroDesk
  → AgentPool → Agent reasoning loop
    → POST /v1/chat/completions (self-referencing)
      → NeuroDesk model inference → response
