...
covering roughly thirty model families in one engine. NeuroDesk exposes it through the
...
audio.cpp is developed by ShugoAI LLC, not by the NeuroDesk project. Model packages live in
[huggingface.co/audio-cpp/audio.cpp-gguf](https://huggingface.co/audio-cpp/audio.cpp-gguf).

## Installing

```bash
neurodesk backends install audio-cpp
```

Or install it from the **Backends** page in the web UI. `audio-cpp` is a
preference-only backend: NeuroDesk never picks it automatically during model import,
...
backend routes each NeuroDesk endpoint onto an audio.cpp task and refuses, with the
family's real capability list in the message, when the loaded model cannot serve it.

| NeuroDesk endpoint | gRPC RPC | audio.cpp tasks tried |
|---|---|---|
...
- **`AudioTransformStream`**: no family advertises streaming for any task
  ...
  call in disguise.
- **`AudioToAudioStream`**: NeuroDesk's contract is an OpenAI-Realtime shaped audio
...
options:
  ...
  - task:tts
  # ggml compute backend inside audio.cpp (not the NeuroDesk backend above).
  - backend:cuda
  - device:0
  - threads:8
