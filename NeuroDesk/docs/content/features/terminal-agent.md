...
`neurodesk chat` opens a terminal agent that runs against your NeuroDesk server. It is not a
separate tool: the agent is compiled into the `neurodesk` binary, so if you have NeuroDesk you
already have it.
...
The agent is [nib](https://github.com/mudler/nib) embedded in NeuroDesk, which is why its
plugin, skill and MCP ecosystem works here unchanged.

{{% notice warning %}}
**Breaking change.** `neurodesk chat` used to be a plain chat prompt. It is now an agent that
...
`neurodesk chat --init` prints a shell widget that binds `Ctrl+Space` to the agent, so you can
...
# bash
echo 'eval "$(neurodesk chat --init bash)"' >> ~/.bashrc
# fish
echo 'neurodesk chat --init fish | source' >> ~/.config/fish/config.fish
```
...
`--yolo`, or `NEURODESK_CHAT_YOLO=1`, approves every tool call without asking. It is the right
...
Redirecting the output is not the same thing and is not refused: `neurodesk chat > out.txt`
...
Config, plugins and skills live in `~/.config/neurodesk/chat/`, or under `$XDG_CONFIG_HOME`
when you have set it. Override it with `--config-dir`, or with
`NEURODESK_CHAT_CONFIG_DIR`. The directory is user-scoped rather than server-scoped, because
the agent is a client and may be pointed at a remote NeuroDesk.
...
Everything after the first positional argument is forwarded verbatim, so NeuroDesk's own flags
have to come first:

```bash
neurodesk chat --config-dir /srv/agent plugin list
```
...
{{% notice warning %}}
**Pass `--yes` in scripts.** The management commands do not read the streams NeuroDesk hands
