...
NeuroDesk supports two authentication modes: **legacy API key authentication** (simple shared keys) and a full **user authentication system** with roles, sessions, OAuth, and per-user usage tracking.

## Legacy API Key Authentication

The simplest way to protect your NeuroDesk instance is with API keys. Set one or more keys via environment variable or CLI flag:

```bash
# Single key
NEURODESK_API_KEY=sk-my-secret-key neurodesk run

# Multiple keys (comma-separated)
NEURODESK_API_KEY=key1,key2,key3 neurodesk run
```
...
When you configure database authentication or legacy API keys, NeuroDesk makes the HTTP surface **private by default**. An anonymous request succeeds only for an approved method and path or an explicit deployment override. If you configure neither authentication mode, the authentication middleware does not restrict requests.

### Public discovery APIs

The following discovery requests are available anonymously:

- `GET /.well-known/neurodesk.json`
...
NeuroDesk also permits the requests needed for health checks, credential acquisition, and the login UI. These routes do not make the rest of the API public.
...
- `NEURODESK_DISABLE_API_KEY_REQUIREMENT_FOR_HTTP_GET=true` enables the override.
- `NEURODESK_HTTP_GET_EXEMPTED_ENDPOINTS` sets the regular expressions for the exempt `GET` routes.

The endpoint expressions have no effect unless you enable `NEURODESK_DISABLE_API_KEY_REQUIREMENT_FOR_HTTP_GET`. Review custom expressions carefully because they can expose protected reads.
...
Set `NEURODESK_AUTH=true` or provide a GitHub OAuth Client ID or OIDC Client ID (which auto-enables auth):

```bash
# Enable with SQLite (default, stored at {DataPath}/database.db)
NEURODESK_AUTH=true neurodesk run

# Enable with GitHub OAuth
GITHUB_CLIENT_ID=your-client-id \
GITHUB_CLIENT_SECRET=your-client-secret \
NEURODESK_BASE_URL=http://localhost:8080 \
neurodesk run

# Enable with OIDC provider (e.g. Keycloak)
NEURODESK_OIDC_ISSUER=https://keycloak.example.com/realms/myrealm \
NEURODESK_OIDC_CLIENT_ID=your-client-id \
NEURODESK_OIDC_CLIENT_SECRET=your-client-secret \
NEURODESK_BASE_URL=http://localhost:8080 \
neurodesk run

# Enable with PostgreSQL
NEURODESK_AUTH=true \
NEURODESK_AUTH_DATABASE_URL=postgres://user:pass@host/dbname \
neurodesk run
```
...
|---|---|---|
| `NEURODESK_AUTH` | `false` | Enable user authentication and authorization |
| `NEURODESK_AUTH_DATABASE_URL` | `{DataPath}/database.db` | Database URL - `postgres://...` for PostgreSQL, or a file path for SQLite |
| `GITHUB_CLIENT_ID` | | GitHub OAuth App Client ID (auto-enables auth when set) |
| `GITHUB_CLIENT_SECRET` | | GitHub OAuth App Client Secret |
| `NEURODESK_OIDC_ISSUER` | | OIDC issuer URL for auto-discovery (e.g. `https://accounts.google.com`) |
| `NEURODESK_OIDC_CLIENT_ID` | | OIDC Client ID (auto-enables auth when set) |
| `NEURODESK_OIDC_CLIENT_SECRET` | | OIDC Client Secret |
| `NEURODESK_BASE_URL` | | Base URL for OAuth callbacks (e.g. `http://localhost:8080`) |
| `NEURODESK_ADMIN_EMAIL` | | Email address to auto-promote to admin role on login |
| `NEURODESK_REGISTRATION_MODE` | `approval` | Registration mode: `open`, `approval`, or `invite` |
| `NEURODESK_DISABLE_LOCAL_AUTH` | `false` | Disable local email/password registration and login (for OAuth/OIDC-only deployments) |

> **Note: network-backed storage.** File-based SQLite relies on POSIX file locking, which is unreliable over network filesystems (SMB/CIFS/NFS, e.g. Azure Files / Azure Container Apps shared volumes). On such storage the auth DB can fail to migrate with `database is locked`. Use PostgreSQL (`NEURODESK_AUTH_DATABASE_URL=postgres://...`) when the data directory lives on shared or network storage, or place `database.db` on a local volume.

### Disabling Local Authentication

If you want to enforce OAuth/OIDC-only login and prevent users from registering or logging in with email/password, set `NEURODESK_DISABLE_LOCAL_AUTH=true` (or pass `--disable-local-auth`):

```bash
# OAuth-only setup (no email/password)
NEURODESK_DISABLE_LOCAL_AUTH=true \
GITHUB_CLIENT_ID=your-client-id \
GITHUB_CLIENT_SECRET=your-client-secret \
NEURODESK_BASE_URL=http://localhost:8080 \
neurodesk run
...
The **first user** to sign in is automatically assigned the admin role. Additional users can be promoted to admin via the admin user management API or by setting `NEURODESK_ADMIN_EMAIL` to their email address.
...
Share the invite URL (`/invite/<code>`) with the user. When they open it, the registration form is pre-filled with the invite code. NeuroDesk validates the code only when the user submits registration. Invite codes are single-use - once consumed, they cannot be reused. Expired or used invites are rejected.
...
2. Set the **Authorization callback URL** to `{NEURODESK_BASE_URL}/api/auth/github/callback`
3. Set `GITHUB_CLIENT_ID` and `GITHUB_CLIENT_SECRET` environment variables
4. Set `NEURODESK_BASE_URL` to your publicly-accessible URL
...
2. Set the redirect URL to `{NEURODESK_BASE_URL}/api/auth/oidc/callback`
3. Set the three environment variables: `NEURODESK_OIDC_ISSUER`, `NEURODESK_OIDC_CLIENT_ID`, `NEURODESK_OIDC_CLIENT_SECRET`

NeuroDesk uses OIDC auto-discovery (the `/.well-known/openid-configuration` endpoint) and requests the standard scopes: `openid`, `profile`, `email`.
...
# Google
NEURODESK_OIDC_ISSUER=https://accounts.google.com

# Authentik
NEURODESK_OIDC_ISSUER=https://authentik.example.com/application/o/neurodesk/

# Okta
NEURODESK_OIDC_ISSUER=https://your-org.okta.com
```
...
When authentication is enabled, NeuroDesk automatically tracks per-user token usage for inference endpoints. Usage data includes:
...
- **Legacy** - request authenticated with an env-configured `NEURODESK_API_KEY`. Visible to admins only.
...
The default Dockerfile includes `GO_TAGS="auth"`, so all Docker images ship with auth support. When building from source without the `auth` tag, setting `NEURODESK_AUTH=true` has no effect - the system operates without authentication.
