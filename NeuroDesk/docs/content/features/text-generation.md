...
NeuroDesk supports generating text with GPT with `llama.cpp` and other backends (such as `rwkv.cpp` as ) see also the [Model compatibility]({{%relref "reference/compatibility-table" %}}) for an up-to-date list of the supported model families.
...
NeuroDesk supports the Anthropic Messages API, which is compatible with Claude clients. This endpoint provides a structured way to send messages and receive responses, with support for tools, streaming, and multimodal content.
...
NeuroDesk supports the Open Responses API specification, which provides a standardized interface for AI model interactions with support for background processing, streaming, tool calling, and advanced features like reasoning.
...
In distributed mode NeuroDesk replicates response metadata across frontend
...
The `ggml` file format has been deprecated. If you are using `ggml` models and you are configuring your model with a YAML file, specify, use a NeuroDesk version older than v2.25.0. For `gguf` models, use the `llama` backend. The go backend is deprecated as well but still available as `go-llama`.
...
NeuroDesk supports `llama.cpp` models out of the box. You can use the `llama.cpp` model in the same way as any other model. 
...
NeuroDesk supports model galleries which are indexes of models. For instance, the huggingface gallery contains a large curated index of models from the huggingface model hub for `ggml` or `gguf` models.

For instance, if you have the galleries enabled and NeuroDesk already running, you can just start chatting with models in huggingface by running:
...
NeuroDesk will automatically download and configure the model in the `model` directory.
...
| `kv_unified` or `unified_kv` | boolean | Use a single unified KV buffer shared across all sequences. Default: `true` (NeuroDesk override; upstream defaults to `false` but auto-enables it when slot count is auto). **Required for `cache_idle_slots` and scoring**: without it the server force-disables idle-slot saving at init, and score-enabled models are rejected at load time. | `kv_unified:false` |
...
```
NEURODESK_DISABLE_HARDWARE_DEFAULTS=true
```
...
All three are on by default since NeuroDesk v4.3, so the prompt cache works out of the box for the common single-slot setup. If you're on an older release, or you've explicitly disabled one of them, add the following to recover the behaviour:
...
The backend is distributed as a separate container image and can be installed from the NeuroDesk backend gallery, or specified directly in a model configuration. GGUF models loaded with this backend benefit from ik_llama.cpp's optimized CPU kernels - especially useful for MoE models and large quantized models that would otherwise be GPU-bound.
...
[llama-cpp-turboquant](https://github.com/TheTom/llama-cpp-turboquant) is a `llama.cpp` fork that adds the **TurboQuant KV-cache** quantization scheme. It reuses the upstream `llama.cpp` codebase and ships as a drop-in alternative backend inside NeuroDesk, sharing the same gRPC server sources as the stock `llama-cpp` backend - so any GGUF model that runs on `llama-cpp` also runs on `turboquant`.
...
`turboquant` ships as a separate container image in the NeuroDesk backend gallery. Install it like any other backend:

```bash
neurodesk backends install turboquant
```
...
NeuroDesk has a built-in integration with vLLM, and it can be used to run models. You can check out `vllm` performance [here](https://github.com/vllm-project/vllm#performance).
...
`neurodesk p2p-worker vllm` follower lets a single model span multiple
...
decoding, and multi-modal generation. NeuroDesk ships a gRPC backend that
...
> NeuroDesk sglang backend pins a release that includes it; if you've
...
inside `ChatDelta` - the NeuroDesk Python backend wires them up
...
[vllm.cpp](https://github.com/mudler/vllm.cpp) is the NeuroDesk team's C++ port of
...
as a path as given, then as the last path segment under NeuroDesk's models
directory (`z-lab/Qwen3.6-27B-DFlash` → `<models>/Qwen3.6-27B-DFlash`, which is
what NeuroDesk's own downloader produces), then as the whole reference under the
models directory. Install the draft into NeuroDesk first, or give an absolute path
...
> with `backend: vllm-cpp`, NeuroDesk reads the checkpoint's `config.json` and, if
...
NeuroDesk has a built-in integration with Transformers, and it can be used to run models.
