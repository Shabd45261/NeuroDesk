...
weight = 23
url = '/features/neurodesk-assistant'
+++

{{< agentic-routing current="neurodesk-assistant" >}}

NeuroDesk Assistant is an admin-only chat modality. When enabled on a chat session, the conversation is wired to an in-process MCP server that exposes NeuroDesk's own admin/management surface as tools. You can install models, manage backends, edit model configs and check system status by chatting, with no REST calls or YAML edits.

The same MCP server is published as a Go package and can also be served over **stdio** to control a remote NeuroDesk instance from outside (e.g. from a desktop MCP host, Cursor, or `mcphost`).
...
Either toggle it off in **Settings → NeuroDesk Assistant** (takes effect without restart), or hard-disable at startup:

```bash
NEURODESK_DISABLE_ASSISTANT=true neurodesk run
```

When disabled, the chat handler refuses requests with `metadata.neurodesk_assistant=true` and returns 503. The Manage toggle is hidden in the UI.
...
You can run the same admin tool surface as a stdio MCP server pointed at any NeuroDesk HTTP API:

```bash
neurodesk mcp-server --target http://remote.neurodesk:8080 --api-key <admin-key>
# read-only mode - skips registration of every mutating tool
neurodesk mcp-server --target http://remote.neurodesk:8080 --read-only
```

Useful for hooking NeuroDesk admin into Claude Desktop, Cursor, or any MCP host. The tool catalog is identical to the in-process variant.
...
The MCP server lives at `pkg/mcp/neurodesktools/`. Tools are registered in `tools_*.go`; skill prompts (the markdown the LLM sees) are embedded from `prompts/`. To add a new admin tool, add a method to the `NeuroDeskClient` interface plus implementations in `inproc/` and `httpapi/`, then register the tool and add or update a skill prompt. See `.agents/neurodesk-assistant-mcp.md` for the full contributor checklist.
