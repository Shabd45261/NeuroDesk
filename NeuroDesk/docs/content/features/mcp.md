...
NeuroDesk now supports the **Model Context Protocol (MCP)**, enabling powerful agentic capabilities by connecting AI models to external tools and services. This feature allows your NeuroDesk models to interact with various MCP servers, providing access to real-time data, APIs, and specialized tools.
...
Remote model MCP connections originate from the NeuroDesk process. If NeuroDesk runs in Docker, the URL must therefore resolve and be reachable **from the NeuroDesk container**, not only from the host browser. For another service in the same Compose project, use its Compose service name and container port. Host-only DNS names, VPN DNS, and private routes must also be made available inside the container.
...
MCP servers can provide reusable prompt templates. NeuroDesk supports discovering and expanding prompts from MCP servers.
  ...
  stdio: |
    {
      "mcpServers": {
        "searxng": {
          ...
          "args": [
            "run", "-i", "--rm",
            "quay.io/mudler/tests:duckduckgo-neurodesk"
          ]
        }
      }
    }
...
1. **Tool Discovery**: NeuroDesk connects to configured MCP servers and discovers available tools
2. **Tool Injection**: Discovered tools are injected into the model's tool/function list alongside any user-provided tools
3. **Inference Loop**: The model generates a response. If it calls MCP tools, NeuroDesk executes them server-side, appends results to the conversation, and re-runs inference
...
MCP sessions are automatically managed by NeuroDesk:
...
- **Graceful shutdown**: All MCP sessions are closed when NeuroDesk shuts down

This means you don't need to manually manage MCP connections - they follow the model's lifecycle automatically.

## Supported MCP Servers

NeuroDesk is compatible with any MCP-compliant server.
...
services:
  neurodesk:
    ...
    #image: neurodesk/neurodesk:latest-gpu-nvidia-cuda-13
    #image: neurodesk/neurodesk:latest-gpu-nvidia-cuda-12
    container_name: neurodesk
    restart: always
    entrypoint: [ "/bin/bash" ]
    command: >
     -c "apt-get update &&
    ...
    environment:
      ...
      - NEURODESK_WATCHDOG_BUSY_TIMEOUT=15m
      - NEURODESK_API_KEY=my-beautiful-api-key
...
In addition to server-side MCP (where the backend connects to MCP servers), NeuroDesk supports **client-side MCP** where the browser connects directly to MCP servers. This is inspired by llama.cpp's WebUI and works alongside server-side MCP.
...
Since browsers enforce CORS restrictions, NeuroDesk provides a built-in proxy at `/api/cors-proxy`. When "Use CORS proxy" is enabled (default), requests to external MCP servers are routed through:
...
NeuroDesk supports the [MCP Apps extension](https://modelcontextprotocol.io/extensions/apps/overview), which allows MCP tools to declare interactive HTML UIs. When a tool has `_meta.ui.resourceUri` in its definition, calling that tool renders the app's HTML inline in the chat as a sandboxed iframe.
...
- The CORS proxy can forward requests to any HTTP/HTTPS URL. It is only available when MCP is enabled (`NEURODESK_DISABLE_MCP` is not set).
...
You can completely disable MCP functionality in NeuroDesk by setting the `NEURODESK_DISABLE_MCP` environment variable to `true`, `1`, or `yes`:

```bash
export NEURODESK_DISABLE_MCP=true
```
...
- Run NeuroDesk without MCP capabilities for security reasons
...
NEURODESK_DISABLE_MCP=true neurodesk run

# Or in Docker
docker run -e NEURODESK_DISABLE_MCP=true neurodesk/neurodesk:latest
```
