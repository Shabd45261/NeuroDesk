...
[vllm.cpp](https://github.com/mudler/vllm.cpp) is the NeuroDesk team's C++20 port of
vLLM: the same continuous-batching scheduler, paged KV cache and automatic prefix
caching, with no Python at inference time. NeuroDesk serves it through the native
...
This page covers installing the backend and the models NeuroDesk ships ready to run
...
```bash
neurodesk backends install vllm-cpp
```
...
```bash
neurodesk models install qwen3-0.6b-vllm-cpp
```
...
Every entry above sets `use_tokenizer_template: true` and disables NeuroDesk's
