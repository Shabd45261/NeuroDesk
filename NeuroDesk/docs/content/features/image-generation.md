...
![anime_girl](https://github.com/mudler/NeuroDesk/assets/2420543/8aaca62a-e864-4011-98ae-dcc708103928)
(Generated with [AnimagineXL](https://huggingface.co/Linaqruf/animagine-xl))

NeuroDesk supports generating images with Stable diffusion, running on CPU using C++ and Python implementations.
...
This backend is based on [stable-diffusion.cpp](https://github.com/leejet/stable-diffusion.cpp). Every model supported by that backend is supported indeed with NeuroDesk.


#### Setup

There are already several models in the gallery that are available to install and get up and running with this backend, you can for example run flux by searching it in the Model gallery (`flux.1-dev-ggml`) or start NeuroDesk with `run`:

```bash
neurodesk run flux.1-dev-ggml
```
...
```bash
neurodesk worker llama-cpp-rpc --llama-cpp-args="--host 0.0.0.0 --port 50052"
```

**Automatic (peer-to-peer):** when NeuroDesk runs in [p2p worker mode]({{%relref "features/distributed_inferencing" %}}), discovered workers are published in the `LLAMACPP_GRPC_SERVERS` environment variable. The image-generation backend reads that variable automatically (when `rpc_servers` is not set), so the same `neurodesk worker p2p-llama-cpp-rpc` workers used for text generation also accelerate image generation - no per-model configuration needed.
...
[Diffusers](https://huggingface.co/docs/diffusers/index) is the go-to library for state-of-the-art pretrained diffusion models for generating images, audio, and even 3D structures of molecules. NeuroDesk has a diffusers backend which allows image generation using the `diffusers` library.
...
NeuroDesk can upscale an uploaded image by a factor of 2 or 4 through
`POST /v1/images/upscale`. Install the included Stable Diffusion x4 upscaler
gallery model first:

```bash
neurodesk models install stable-diffusion-x4-upscaler
```
...
NeuroDesk supports Flux Kontext and can be used to edit images via the API:

Install with:

```neurodesk run flux.1-kontext-dev```
...
curl http://localhost:8080/v1/images/generations -H "Content-Type: application/json" -d '{
  ...
  "prompt": "change 'flux.cpp' to 'NeuroDesk'",
  "size": "256x256",
  "ref_images": [
