...
Once NeuroDesk is started and whisper models are installed, you can use the `/v1/audio/transcriptions` API endpoint.
...
| `diarize` | NeuroDesk extension - speaker diarization. WhisperX requires `HF_TOKEN`; requests fail with `FailedPrecondition` when it is missing. |
...
```bash
neurodesk models import https://huggingface.co/mudler/parakeet-cpp-gguf/resolve/main/tdt_ctc-110m-f16.gguf
```
