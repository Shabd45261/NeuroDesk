...
NeuroDesk supports voice (speaker) recognition: speaker verification
...
```bash
neurodesk models install voice-detect-ecapa-tdnn
```
...
```bash
neurodesk models install voice-detect-emotion-wav2vec2
...
registered speakers are lost when NeuroDesk restarts. Persistent storage
