...
NeuroDesk uses YAML configuration files to define model parameters, templates, and behavior. This page provides a complete reference for all available configuration options.
...
2. **Single config file** with multiple models using `--models-config-file` or `NEURODESK_MODELS_CONFIG_FILE`
...
repeatable. NeuroDesk resolves the requested revision to an immutable commit,
...
Declare `source` when authoring a configuration. NeuroDesk owns the `resolved`
...
set it to `HF_TOKEN` and provide that environment variable to the NeuroDesk
controller.
...
`resolved.cache_key` is present, NeuroDesk derives
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `threads` | int | `processor count` | Number of threads for parallel computation. A per-model value overrides the server-wide `--threads`/`NEURODESK_THREADS` setting |
...
The canonical names match upstream llama.cpp (dash-separated). For backward compatibility NeuroDesk also accepts the underscore-separated forms and the bare `draft` / `eagle3` aliases.
...
**Auto-detection (default).** When a GGUF declares an MTP head (the upstream `<arch>.nextn_predict_layers` metadata key, set by `convert_hf_to_gguf.py` for Qwen3.5/3.6 family models and similar), NeuroDesk auto-enables MTP with the following defaults:
...
| `spec_n_max` / `draft_max` | `2`-`6` | Number of draft tokens per step. Upstream's PR suggests 2-3 for the tightest acceptance window; NeuroDesk's auto-default is 6 to favour throughput on models with high acceptance. |
...
bundled llama.cpp supports works without NeuroDesk needing a dedicated option,
even ones added after your NeuroDesk version was built. See the upstream
...
- **Precedence:** passthrough flags are applied last, so an explicit flag
  overrides the NeuroDesk option it maps to (e.g. `--ctx-size:8192` overrides
  `context_size`).
...
`tts.voice_cloning: true` only overrides model-variant detection. It cannot enable cloning on a backend that does not implement NeuroDesk's reference-audio contract.
...
context canceled` in the NeuroDesk log, while the backend's own stderr
...
Valid modality values are `text`, `image`, `audio`, and `video`. Explicit values are combined with modalities NeuroDesk can infer from the model use cases and configuration. The resulting canonical, de-duplicated lists are exposed by `GET /v1/models/capabilities`.
...
**Note**: By default, NeuroDesk sets `gpu_layers` to a very large value (9999999), which effectively disables llama-cpp's auto-fit functionality. This is intentional to work with NeuroDesk's VRAM-based model unloading mechanism.

To enable llama-cpp's auto-fit mode, set `gpu_layers: -1` in your model configuration. However, be aware of the following:

1. **Trade-off**: Enabling auto-fit conflicts with NeuroDesk's built-in VRAM threshold-based unloading. Auto-fit attempts to fit all tensors into GPU memory automatically, while NeuroDesk's unloading mechanism removes models when VRAM usage exceeds thresholds.
...
3. **Recommendation**: 
   - Use the default settings for most use cases (NeuroDesk manages VRAM automatically)
...
This is a known limitation being tracked in issue [#8562](https://github.com/mudler/NeuroDesk/issues/8562). A future implementation may provide a runtime toggle or custom logic to reconcile auto-fit with threshold-based unloading.
