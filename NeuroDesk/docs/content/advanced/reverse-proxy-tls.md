...
description: Configure NeuroDesk behind a TLS termination reverse proxy (HAProxy, Apache, Nginx, APISIX)
weight: 100
---

![TLS at the edge: terminate TLS at the reverse proxy and forward headers so NeuroDesk emits correct https URLs](/images/diagrams/reverse-proxy-tls.png)

# TLS Reverse Proxy Configuration

When running NeuroDesk behind a TLS termination reverse proxy, the Web UI may fail to load static assets (CSS, JS) correctly because the application doesn't automatically detect that it's being served over HTTPS. This guide explains how to properly configure your reverse proxy to work with NeuroDesk.

## How It Works

NeuroDesk uses the `X-Forwarded-Proto` HTTP header to determine the protocol used by clients. When this header is set to `https`, NeuroDesk will generate HTTPS URLs for static assets in the Web UI.

## Running behind a reverse proxy (HTTPS / subpath)

NeuroDesk does not terminate TLS itself, so HTTPS is provided by a reverse
...
NeuroDesk determines these in this order:

1. `NEURODESK_BASE_URL` - if set, it is authoritative for the origin. Set it to
   the externally visible base URL, e.g. `NEURODESK_BASE_URL=https://neurodesk.example.com`
...
appended to `NEURODESK_BASE_URL` when both are present.

## Required Headers

Your reverse proxy must forward these headers to NeuroDesk:
...
| `X-Forwarded-Prefix` | Any path prefix if NeuroDesk is served under a sub-path |
...
frontend https-in
    ...
    # http-request set-header X-Forwarded-Prefix /neurodesk
    
    default_backend neurodesk

backend neurodesk
    mode http
    server neurodesk1 127.0.0.1:8080 check
```
...
<VirtualHost *:443>
    ...
    # RequestHeader set X-Forwarded-Prefix "/neurodesk"
...
server {
    ...
    location / {
        ...
        # If serving under a sub-path
        # proxy_set_header X-Forwarded-Prefix /neurodesk;
        ...
        # Stream responses as NeuroDesk emits them. Disabling request buffering is
...
the proxy stops waiting before NeuroDesk finishes, clients receive a proxy-generated
...
gateway. Put it between clients and NeuroDesk when you want to manage the
...
This is useful when NeuroDesk runs on a private network but its API must be
...
transparent route so it works with NeuroDesk clients before you add those
policies.

The request path is:

```text
OpenAI-compatible client -> APISIX -> NeuroDesk (:8080)
```

### Create the NeuroDesk Route

Start NeuroDesk with the model configuration you want to serve, and confirm that
APISIX can reach `http://neurodesk:8080`. When both services run in containers,
attach them to the same container network and use the NeuroDesk service name as
...
completions. This example assumes APISIX can resolve `neurodesk` and reach it on
port `8080`:

```bash
curl http://127.0.0.1:9180/apisix/admin/routes/neurodesk \
  --request PUT \
...
Set `X-Forwarded-Prefix` in the `proxy-rewrite` header map as well if NeuroDesk
...
### Use NeuroDesk Through APISIX

Clients continue to use NeuroDesk's OpenAI-compatible paths; only the base URL
changes. First, check that model discovery reaches NeuroDesk through the APISIX
...
`https://neurodesk.example.com/v1`. Keep NeuroDesk's own API key in the usual
`Authorization: Bearer ...` header if NeuroDesk authentication is enabled. If
...
plugins. A gateway-specific NeuroDesk guide maintained by the APISIX project can
also be linked here when one is available.

For bulk jobs on a trusted private network, you can also bypass the public
reverse proxy and connect directly to NeuroDesk, for example
`http://neurodesk-host:8080/v1`.

## Serving Under a Sub-Path

If you serve NeuroDesk under a sub-path (e.g., `https://your-domain.com/neurodesk`), you need to:
...
- Ensure NeuroDesk is generating HTTPS URLs (check the BaseURL middleware is working)
- Verify the `X-Forwarded-Proto` header is set before NeuroDesk processes the request
...
reverse proxy stopped waiting for NeuroDesk. Increase the proxy's upstream read
...
This is separate from NeuroDesk's optional busy watchdog. If busy watchdog checks
are enabled, set `NEURODESK_WATCHDOG_BUSY_TIMEOUT` beyond the longest expected
...
When using reverse proxies, ensure your proxy only accepts connections from trusted sources and properly validates SSL certificates. Never expose NeuroDesk directly to the internet without TLS termination.
