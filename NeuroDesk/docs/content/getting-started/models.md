...
weight = 3
icon = "hub"
description = "Learn how to install, configure, and manage models in NeuroDesk"
+++
...
This section covers everything you need to know about installing and configuring models in NeuroDesk. You'll learn multiple methods to get models running.

## Prerequisites

- NeuroDesk installed and running (see [Quickstart]({{% relref "getting-started/quickstart" %}}) if you haven't set it up yet)
...
1. Open the NeuroDesk WebUI at `http://localhost:8080`
...
neurodesk models list

# Install a specific model
neurodesk models install llama-3.2-1b-instruct:q4_k_m

# Start NeuroDesk with a model from the gallery
neurodesk run llama-3.2-1b-instruct:q4_k_m
```

To run models available in the NeuroDesk gallery, you can use the model name as the URI. For example, to run NeuroDesk with the Hermes model, execute:

```bash
neurodesk run hermes-2-theta-llama-3-8b
```

To install only the model, use:

```bash
neurodesk models install hermes-2-theta-llama-3-8b
```

Note: The galleries available in NeuroDesk can be customized to point to a different URL or a local directory. For more information on how to setup your own gallery, see the [Gallery Documentation]({{% relref "features/model-gallery" %}}).
...
Leaving the backend on auto-detect lets NeuroDesk choose from the source. If more
...
NeuroDesk uses Hugging Face's pipeline metadata to select `mlx-vlm` for
...
NeuroDesk can directly install models from Hugging Face:

```bash
# Install and run a model from Hugging Face
neurodesk run huggingface://TheBloke/phi-2-GGUF
```
...
{{% notice note %}}
On every model download — Ollama and OCI registries, the model gallery, and plain HTTP(S) file URLs alike — NeuroDesk identifies itself with a `NeuroDesk/<version> (<os>; <arch>)` `User-Agent` header (for example `NeuroDesk/v3.2.1 (linux; amd64)`) so registry and gallery operators can attribute usage to NeuroDesk. Builds from source that carry no stamped version send `NeuroDesk (<os>; <arch>)` instead.
{{% /notice %}}

### Run Models via URI

To run models via URI, specify a URI to a model file or a configuration file when starting NeuroDesk. Valid syntax includes:
...
### Step 3: Run NeuroDesk

Choose one of the following methods to run NeuroDesk:
...
docker run -p 8080:8080 -v $PWD/models:/models -ti --rm neurodesk/neurodesk:latest --models-path /models --context-size 700 --threads 4
...
```bash
git clone https://github.com/mudler/NeuroDesk
...
NeuroDesk binary releases are available on [GitHub](https://github.com/mudler/NeuroDesk/releases).

```bash
# With binary
neurodesk --models-path ./models
```

{{% notice tip %}}
If installing on macOS, you might encounter a message saying:

> "neurodesk-git-Darwin-arm64" (or the name you gave the binary) can't be opened because Apple cannot check it for malicious software.

Hit OK, then go to Settings > Privacy & Security > Security and look for the message:

> "neurodesk-git-Darwin-arm64" was blocked from use because it is not from an identified developer.
...
For instructions on building NeuroDesk from source, see the [Build from Source guide]({{% relref "getting-started/build" %}}).
...
For more model configurations, visit the [Examples Section](https://github.com/mudler/NeuroDesk-examples/tree/main/configurations).
...
1. **Check backend**: Ensure the required backend is installed
   ...
   ```bash
   neurodesk backends list
   neurodesk backends install llama-cpp  # if needed
   ```

2. **Check logs**: Enable debug mode

   ```bash
   DEBUG=true neurodesk
   ```
