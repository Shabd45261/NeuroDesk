...
The agent platform ships inside the NeuroDesk binary and container image, and it is enabled by default. If you already have NeuroDesk running and a model installed, you have everything you need to build an agent. This page walks you from an empty Agents page to an agent that answers a message and uses one tool.
...
- **As a default for every new agent:** start NeuroDesk with an environment variable so new agents are created with that model already selected:

```bash
NEURODESK_AGENT_POOL_DEFAULT_MODEL=qwen3-4b
```
...
That is a complete agent: a model, a system prompt, and one tool, all running inside your NeuroDesk process.
...
- **A skill the agent expects is not available:** skills are disabled by default. Start NeuroDesk with `NEURODESK_AGENT_POOL_ENABLE_SKILLS=true` to turn the skills service on (the default is `NEURODESK_AGENT_POOL_ENABLE_SKILLS=false`).
