...
NeuroDesk can be built as a container image or as a single, portable binary. Note that some model architectures might require Python libraries, which are not included in the binary.

NeuroDesk's extensible architecture allows you to add your own backends, which can be written in any language, and as such the container images contains also the Python dependencies to run all the available backends (for example, in order to run backends like __Diffusers__ that allows to generate images and videos from text).

This section contains instructions on how to build NeuroDesk from source.

#### Build NeuroDesk locally

##### Requirements

In order to build NeuroDesk locally, you need the following requirements:
...
This should produce the binary `neurodesk`
...
In order to build the `NeuroDesk` container image locally you can use `docker`, for example:

```
docker build -t neurodesk .
docker run neurodesk
```
...
./neurodesk --models-path=./models/ --debug=true
...
NeuroDesk have several backends available for installation in the backend gallery. The backends can be also built by source. As backends might vary from language and dependencies that they require, the documentation will provide generic guidance for few of the backends, which can be applied with some slight modifications also to the others.

### Manually

Typically each backend include a Makefile which allow to package the backend.

In the NeuroDesk repository, for instance you can build a backend by doing:

```
git clone https://github.com/mudler/NeuroDesk.git

make -C NeuroDesk/backend/python/vllm
```

### With Docker

Building with docker is simpler as abstracts away all the requirement, and focuses on building the final OCI images that are available in the gallery. This allows for instance also to build locally a backend and install it with NeuroDesk. You can refer to [Backends](https://localai.io/backends/) for general guidance on how to install and develop backends.

In the NeuroDesk repository, you can build a backend by doing:
...
```bash
docker build --build-arg BUILD_TYPE=$(BUILD_TYPE) --build-arg BASE_IMAGE=$(BASE_IMAGE) -t neurodesk-backend:<backend-name> -f NeuroDesk/backend/Dockerfile.golang --build-arg BACKEND=<backend-name> .               
```
