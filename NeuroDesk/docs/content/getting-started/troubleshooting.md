...
This guide covers common issues you may encounter when using NeuroDesk, organized by category. For each issue, diagnostic steps and solutions are provided.
...
```bash
# Check NeuroDesk is running and responsive
curl http://localhost:8080/readyz

# List loaded models
curl http://localhost:8080/v1/models

# Check NeuroDesk version
neurodesk --version
...
```bash
# View container logs
docker logs neurodesk

# Check container status
docker ps -a | grep neurodesk
...
**Symptoms:** macOS blocks NeuroDesk from running because the DMG is not signed by Apple.

**Solution:** See [GitHub issue #6268](https://github.com/mudler/NeuroDesk/issues/6268) for quarantine bypass instructions. This is tracked for resolution in [issue #6244](https://github.com/mudler/NeuroDesk/issues/6244).
...
2. Verify your models path is correct:
   ```bash
   # Check what path NeuroDesk is using
   neurodesk run --models-path /path/to/models --log-level=debug
   ```
...
- **Backend not installed:** Check installed backends:
  ...
  neurodesk backends install llama-cpp
  ```
- **Corrupt model file:** Re-download the model. Partial downloads or disk errors can corrupt files.
- **Wrong model format:** NeuroDesk uses GGUF format for llama.cpp models. Older GGML format is deprecated.
...
When working correctly, NeuroDesk logs should show: `ggml_init_cublas: found X CUDA devices`.
...
If NeuroDesk picks the wrong GPU backend, override it:

```bash
NEURODESK_FORCE_META_BACKEND_CAPABILITY=nvidia neurodesk run
...
4. **Limit active models:** Only keep one model loaded at a time:
   ```bash
   neurodesk run --max-active-backends=1
   ```
5. **Enable idle watchdog:** Automatically unload unused models:
   ```bash
   neurodesk run --enable-watchdog-idle --watchdog-idle-timeout=10m
   ```
...
neurodesk run --max-active-backends=2
```
...
These can also be set via environment variables (`NEURODESK_WATCHDOG_IDLE=true`, `NEURODESK_WATCHDOG_IDLE_TIMEOUT=15m`) or in the Web UI under Settings → Watchdog Settings.
...
1. Verify NeuroDesk is running:
   ```bash
   # Direct install
   ps aux | grep neurodesk

   # Docker
   docker ps | grep neurodesk
   ```

2. Check the bind address and port:
   ```bash
   # Default is :8080. Override with:
   neurodesk run --address=0.0.0.0:8080
   # or
   NEURODESK_ADDRESS=":8080" neurodesk run
   ```
...
If API key authentication is enabled (`NEURODESK_API_KEY` or `--api-keys`), include the key in your requests:
...
- **Default sampling:** NeuroDesk uses mirostat sampling by default, which produces better quality output but is slower. Disable it for benchmarking:
  ```yaml
  # In model config
  mirostat: 0
  ```
...
Mount all of NeuroDesk's stateful paths:
...
```bash
NEURODESK_P2P_LOGLEVEL=debug \
NEURODESK_P2P_LIB_LOGLEVEL=debug \
NEURODESK_P2P_ENABLE_LIMITS=true \
NEURODESK_P2P_TOKEN="<TOKEN>" \
neurodesk run
```
...
```bash
NEURODESK_P2P_DISABLE_DHT=true neurodesk run
```
...
1. **Search existing issues:** Check the [GitHub Issues](https://github.com/mudler/NeuroDesk/issues) for similar problems
2. **Enable debug logging:** Run with `DEBUG=true` or `--log-level=debug` and include the logs when reporting
3. **Open a new issue:** Include your OS, hardware (CPU/GPU), NeuroDesk version, model being used, full error logs, and steps to reproduce
4. **Community help:** Join the [NeuroDesk Discord](https://discord.gg/uJAeKSAGDy) for community support
