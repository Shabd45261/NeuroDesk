...
The lists below cover software and community projects that integrate with NeuroDesk.

Feel free to open up a Pull request (by clicking at the "Edit page" below) to get your project added!

### Build & Deploy

- [aikit](https://github.com/sozercan/aikit) - Build and deploy custom NeuroDesk containers
- [Helm chart](https://github.com/go-skynet/helm-charts) - Deploy NeuroDesk on Kubernetes
- [GitHub Actions](https://github.com/marketplace/actions/start-neurodesk) - Use NeuroDesk in CI/CD workflows

### Distribution Packages

Community-maintained packages of NeuroDesk. Package versions and
availability may lag upstream releases.

- [Homebrew](https://formulae.brew.sh/formula/neurodesk) - `brew install neurodesk`
- [ALT Sisyphus](https://packages.altlinux.org/en/sisyphus/srpms/neurodesk/) - `neurodesk` package
- [Gentoo overlay](https://git.ipnmod.org/packages/neurodesk-overlay) - `sci-ml/neurodesk` and per-backend packages, overlay `neurodesk` in Gentoo's repository index

### Web UIs

- [neurodesk-admin](https://github.com/Jirubizu/neurodesk-admin)
- [NeuroDesk-frontend](https://github.com/go-skynet/LocalAI-frontend)
...
- [VSCode extension](https://github.com/badgooooor/neurodesk-vscode-plugin)
- [GPTLocalhost (Word Add-in)](https://gptlocalhost.com/demo#NeuroDesk) - Run NeuroDesk in Microsoft Word locally

### Framework Integrations

- [Langchain (Python)](https://python.langchain.com/docs/integrations/providers/neurodesk/) - [pypi](https://pypi.org/project/langchain-neurodesk/)
...
- [Telegram bot](https://github.com/mudler/NeuroDesk/tree/master/examples/telegram-bot)
...
- [NeuroDesk Monitor](https://github.com/loryanstrant/HA-NeuroDesk-Monitor) - Monitor & control of NeuroDesk from Home Assistant
...
- [Wave Terminal](https://docs.waveterm.dev/features/supportedLLMs/neurodesk) - Native NeuroDesk support
...
- [Examples](https://github.com/mudler/NeuroDesk/tree/master/examples/)

## Configuration Guides

This section provides step-by-step instructions for configuring specific software to work with NeuroDesk.

### OpenCode

[OpenCode](https://opencode.ai) is an AI-powered code editor that can be configured to use NeuroDesk as its backend provider.

#### Prerequisites

- NeuroDesk must be running and accessible (either locally or on a network)
- You need to know your NeuroDesk server's IP address/hostname and port (default is `8080`)
...
2. **Add NeuroDesk provider configuration**
...
3. **Customize the configuration**
   ...
   - **baseURL**: Replace `http://127.0.0.1:8080/v1` with your NeuroDesk server's address and port.
   - **name**: Change "NeuroDesk (local)" to a descriptive name for your setup.
   - **models**: Replace the model names with the actual model names available in your NeuroDesk instance. You can find available models by checking your NeuroDesk models directory or using the NeuroDesk API.
...
4. **Verify your models**
   ...
   Ensure that the model names in the configuration match exactly with the model names configured in your NeuroDesk instance. You can verify available models by checking your NeuroDesk configuration or using the `/v1/models` endpoint.
...
[Claude Code](https://docs.anthropic.com/en/docs/claude-code) is Anthropic's official CLI tool for coding with Claude. NeuroDesk implements the Anthropic Messages API (`/v1/messages`), so Claude Code can be pointed directly at a NeuroDesk instance.

#### Prerequisites

- NeuroDesk must be running and accessible (either locally or on a network)
- You need to know your NeuroDesk server's IP address/hostname and port (default is `8080`)
- An API key configured in your NeuroDesk instance

#### Running Claude Code with NeuroDesk

Set the `ANTHROPIC_BASE_URL` and `ANTHROPIC_API_KEY` environment variables to point Claude Code at your NeuroDesk server:

```bash
ANTHROPIC_BASE_URL=http://127.0.0.1:8080 \
ANTHROPIC_API_KEY=your-neurodesk-api-key \
...
export ANTHROPIC_API_KEY=your-neurodesk-api-key
```

#### Verify available models

Check which models are available in your NeuroDesk instance:
...
You can ask [Charm Crush](https://charm.land/crush) to generate your config by giving it this documentation's URL and your NeuroDesk instance URL. The configuration will look something like the following and goes in `~/.config/crush/crush.json`:
```json
{
  ...
  "providers": {
    "neurodesk": {
      "name": "NeuroDesk",
      "base_url": "http://neurodesk.lan:8081/v1",
      "type": "openai-compat",
      "models": [
        {
...
You can use NeuroDesk in GitHub Actions workflows to perform AI-powered tasks like code review, diff summarization, or automated analysis. The [NeuroDesk GitHub Action](https://github.com/mudler/localai-github-action) makes it easy to spin up a NeuroDesk instance in your CI/CD pipeline.
...
This example workflow demonstrates how to use NeuroDesk to summarize pull request diffs and send notifications:

1. **Create a workflow file**

   Create a new file in your repository at `.github/workflows/neurodesk.yml`:

```yaml
name: Use NeuroDesk in GHA
on:
  pull_request:
     types:
       - closed
...
jobs:
  notify-discord:
    ...
    # Starts the NeuroDesk container
    - id: foo
    ...
    # Ask to explain the diff to NeuroDesk
    - name: Summarize
      env:
      ...
      run: |
            ...
            # Define the NeuroDesk API endpoint
...
- **API endpoint**: The NeuroDesk container runs on `http://localhost:8080` by default. The action exposes the service on the standard port.
- **Custom prompts**: Modify the system message in the JSON payload to change what NeuroDesk is asked to do with the diff.
...
- [NeuroDesk GitHub Action repository](https://github.com/mudler/localai-github-action)
- [Available models](https://models.localai.io)
- [NeuroDesk API documentation](/reference/)

### Realtime Voice Assistant

NeuroDesk supports realtime voice interactions , enabling voice assistant applications with real-time speech-to-speech communication. A complete example implementation is available in the [NeuroDesk-examples repository](https://github.com/mudler/NeuroDesk-examples/tree/main/realtime). For a minimal native client, see the [Go realtime voice assistant demo](https://github.com/localai-org/neurodesk-realtime-demo): a tiny Go client for the Realtime (WebSocket) API with a full talk-back loop and an example tool call, plus a `docker compose` setup that brings up a realtime-capable NeuroDesk for you.
...
- Transcribes speech to text using NeuroDesk's transcription capabilities
...
- A transcription model (e.g., Whisper) configured in NeuroDesk
- A text-to-speech model configured in NeuroDesk
...
1. **Clone the example repository**
   ...
   ```bash
   git clone https://github.com/mudler/NeuroDesk-examples.git
   cd NeuroDesk-examples/realtime
   ```

2. **Start NeuroDesk with Docker Compose**
   ...
   ```bash
   docker logs -f realtime-neurodesk-1
   ```
...
- **CPU vs GPU**: The example is optimized for CPU usage. However, you can run NeuroDesk with a GPU for better performance and to use bigger/better models.
- **Python client**: The Python part downloads PyTorch for CPU, but this is fine as computation is offloaded to NeuroDesk. The Python client only runs Silero VAD (Voice Activity Detection), which is fast, and handles audio recording.
- **Thin client architecture**: The Python client is designed to run on thin clients such as Raspberry PIs, while NeuroDesk handles the heavier computational workload on a more powerful machine.
...
- **OpenAI-compatible API**: Uses NeuroDesk's OpenAI-compatible realtime API endpoints
...
- [Realtime Voice Assistant Example (Python)](https://github.com/mudler/NeuroDesk-examples/tree/main/realtime)
- [Realtime Voice Assistant Demo (Go)](https://github.com/localai-org/neurodesk-realtime-demo)
- [NeuroDesk Realtime API documentation](/features/)
