...
the target-only entry as its variant. NeuroDesk can then prefer the faster entry
...
Use NeuroDesk's Hugging Face artifact source for each repository. Stage complete
...
No model download or GPU benchmark is part of this NeuroDesk change. The
