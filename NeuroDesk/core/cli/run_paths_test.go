...
//
// The historical defaults (/tmp/generated/content and /tmp/neurodesk/upload) are
// shared across every user of a machine. On macOS /tmp is routed to the shared
// /private/tmp for all accounts, so the first account to run NeuroDesk creates the
...
var _ = Describe("default writable paths", func() {
	userScope := fmt.Sprintf("neurodesk-%d", os.Getuid())
	...
	Describe("DefaultUploadPath", func() {
		...
		It("is not the historical shared path", func() {
			Expect(DefaultUploadPath()).ToNot(Equal("/tmp/neurodesk/upload"))
		})
	})
})
