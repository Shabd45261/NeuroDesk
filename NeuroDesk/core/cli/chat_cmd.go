...
import (
	...
	chatcli "github.com/mudler/NeuroDesk/core/cli/chat"
...
// subcommands (plugin, skill, mcp) and their flags work unchanged. NeuroDesk's
...
type ChatCMD struct {
	...
	Endpoint  string `env:"NEURODESK_CHAT_ENDPOINT" default:"http://127.0.0.1:8080" help:"NeuroDesk server endpoint. The /v1 path is added automatically when omitted"`
	APIKey    string `env:"NEURODESK_API_KEY,API_KEY" help:"API key to use when the NeuroDesk server requires authentication"`
	ConfigDir string `env:"NEURODESK_CHAT_CONFIG_DIR" help:"Directory holding the agent's config, plugins, and skills. Defaults to ~/.config/neurodesk/chat" type:"path"`
	TraceDir  string `env:"NEURODESK_CHAT_TRACE_DIR" help:"Write a session LLM trace (NDJSON) to this directory" type:"path"`
	...
	Yolo   bool   `env:"NEURODESK_CHAT_YOLO" help:"Auto-approve every tool call without prompting"`
...
// agentArgs rebuilds the argument vector the agent expects: NeuroDesk's mode
