...
import (
	...
	"github.com/mudler/NeuroDesk/core/cli/workerregistry"
	"github.com/mudler/NeuroDesk/core/config"
	mcpTools "github.com/mudler/NeuroDesk/core/http/endpoints/mcp"
	"github.com/mudler/NeuroDesk/core/services/agents"
	"github.com/mudler/NeuroDesk/core/services/jobs"
	mcpRemote "github.com/mudler/NeuroDesk/core/services/mcp"
	"github.com/mudler/NeuroDesk/core/services/messaging"
	"github.com/mudler/NeuroDesk/pkg/sanitize"
...
// Usage:
//
//	neurodesk agent-worker --nats-url nats://... --register-to http://neurodesk:8080
type AgentWorkerCMD struct {
	...
	// API access
	APIURL   string `env:"NEURODESK_API_URL" help:"NeuroDesk API URL for inference (auto-derived from RegisterTo if not set)" group:"api"`
	APIToken string `env:"NEURODESK_API_TOKEN" help:"API token for NeuroDesk inference (auto-provisioned during registration if not set)" group:"api"`

	// NATS subjects
	Subject string `env:"NEURODESK_AGENT_SUBJECT" default:"agent.execute" help:"NATS subject for agent execution" group:"distributed"`
	Queue   string `env:"NEURODESK_AGENT_QUEUE" default:"agent-workers" help:"NATS queue group name" group:"distributed"`

	NatsJWT         string `env:"NEURODESK_NATS_JWT" help:"NATS user JWT override (defaults to nats_jwt from registration)" group:"distributed"`
	NatsUserSeed    string `env:"NEURODESK_NATS_USER_SEED" help:"NATS user seed override (defaults to nats_user_seed from registration)" group:"distributed"`
	NatsServiceJWT  string `env:"NEURODESK_NATS_SERVICE_JWT" help:"Fallback NATS service JWT when registration does not mint agent JWT" group:"distributed"`
	NatsServiceSeed string `env:"NEURODESK_NATS_SERVICE_SEED" help:"Fallback NATS service seed paired with NEURODESK_NATS_SERVICE_JWT" group:"distributed"`
	NatsRequireAuth bool   `env:"NEURODESK_NATS_REQUIRE_AUTH" default:"false" help:"Require NATS JWT+seed to connect" group:"distributed"`
	...
	DistributedRequireAuth bool   `env:"NEURODESK_DISTRIBUTED_REQUIRE_AUTH" default:"false" help:"Umbrella switch implying --nats-require-auth (agent workers have no file-transfer server)" group:"distributed"`
	...
	// Timeouts
	MCPCIJobTimeout string `env:"NEURODESK_MCP_CI_JOB_TIMEOUT" default:"10m" help:"Timeout for MCP CI job execution" group:"distributed"`
}
...
func (cmd *AgentWorkerCMD) Run(ctx *cliContext.Context) error {
	...
	case cmd.NatsJWT != "" || cmd.NatsUserSeed != "":
		if (cmd.NatsJWT == "") != (cmd.NatsUserSeed == "") {
			return fmt.Errorf("NEURODESK_NATS_JWT and NEURODESK_NATS_USER_SEED must be set together")
		}
	...
	case cmd.NatsServiceJWT != "" || cmd.NatsServiceSeed != "":
		if (cmd.NatsServiceJWT == "") != (cmd.NatsServiceSeed == "") {
			return fmt.Errorf("NEURODESK_NATS_SERVICE_JWT and NEURODESK_NATS_SERVICE_SEED must be set together")
		}
		natsOpts = append(natsOpts, messaging.WithUserJWT(cmd.NatsServiceJWT, cmd.NatsServiceSeed))
	case cmd.natsAuthRequired():
		return fmt.Errorf("NATS JWT+seed required: enable frontend minting or set NEURODESK_NATS_* env vars")
	}
	if natsTLS.Enabled() {
...
// The agent worker can create MCP sessions (has docker) and call the NeuroDesk API for inference.
