...
var _ = Describe("Agent state directory", func() {
	Describe("StateDir", func() {
		...
		It("uses XDG_CONFIG_HOME when set", func() {
			...
			Expect(StateDir("")).To(Equal(filepath.Join(tmp, "neurodesk", "chat")))
		})

		It("falls back to ~/.config/neurodesk/chat", func() {
			tmp := GinkgoT().TempDir()
			GinkgoT().Setenv("XDG_CONFIG_HOME", "")
			GinkgoT().Setenv("HOME", tmp)
			Expect(StateDir("")).To(Equal(filepath.Join(tmp, ".config", "neurodesk", "chat")))
		})
