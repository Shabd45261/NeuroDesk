...
// may target a remote NeuroDesk, so it does not belong under NEURODESK_CONFIG_DIR.
func StateDir(override string) (string, error) {
	if override != "" {
		return override, nil
	}
	if xdg := os.Getenv("XDG_CONFIG_HOME"); xdg != "" {
		return filepath.Join(xdg, "neurodesk", "chat"), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return "", fmt.Errorf("resolving home directory for the agent state dir: %w", err)
	}
	return filepath.Join(home, ".config", "neurodesk", "chat"), nil
}
