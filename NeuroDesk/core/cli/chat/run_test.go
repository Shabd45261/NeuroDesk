...
// modelServer answers /v1/models with the given ids, as NeuroDesk does.
...
var _ = Describe("prepare", func() {
	...
	It("advises how to start a server when none is reachable", func() {
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("neurodesk run"))
	...
	It("treats a server that never answers as one that is not there", func(ctx SpecContext) {
		...
		_, err := prepare(context.Background(), opts, false)
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("neurodesk run"), "want the offer-a-server advice, got %v", err)
	...
	// signal skips: the process dies where it stands and leaves 'neurodesk run'
	// reparented to init.
	...
	// out=$(neurodesk chat --height 50%) puts a pipe on stdout by construction,
	...
	Describe("agentOptions", func() {
		...
		Describe("stdout", func() {
			...
			It("hands nib nothing for the process stdout, so the capture widget is not refused", func() {
				...
				Expect(o.Stdout).To(BeNil(), "injecting os.Stdout is what refuses out=$(neurodesk chat)")
			})
		...
		It("names the command a user would type, not the binary nib ships as", func() {
			o := agentOptions(dir, "a-model", optionsWithStreams(os.Stdin, os.Stdout, os.Stderr))
			Expect(o.ProgramName).To(Equal("neurodesk chat"),
	...
	// read is the one users meet: 'echo q | neurodesk chat' names --cli, and a
	...
	Describe("ExitStatus", func() {
		...
		It("leaves an ordinary failure to be reported", func() {
			_, reported := ExitStatus(errors.New("no NeuroDesk server at http://127.0.0.1:8080"))
