...
var (
	...
	ErrUnreachable = errors.New("no NeuroDesk server reachable")
	// ErrUnauthorized means the server answered but rejected the credentials.
	ErrUnauthorized = errors.New("NeuroDesk server rejected the API key")
)
...
// order. NeuroDesk happily lists non-model entries it finds in the models
...
// occur against a real NeuroDesk: it returns *openai.APIError when the body
// parses as an OpenAI error envelope, which is what NeuroDesk's normal error
// handler sends, and *openai.RequestError when it does not, which is what
// NeuroDesk sends when started with opaque errors, since that handler replies
