...
var errServerExited = errors.New("the NeuroDesk server exited before it became ready")
...
const (
	...
	//
	// This is not a theoretical guard for NeuroDesk. 'neurodesk run' spawns backend
...
// OfferToStart asks whether to start a NeuroDesk server and, if allowed, spawns
...
func OfferToStart(ctx context.Context, opts StartOptions) (*StartedServer, error) {
	...
	ok, err := opts.Confirm(fmt.Sprintf("No NeuroDesk server at %s. Start one now?", opts.Endpoint))
	if err != nil {
	...
	if bin == "" {
		if bin, err = os.Executable(); err != nil {
			return nil, fmt.Errorf("locating the neurodesk binary: %w", err)
		}
	}

	cmd := newServerCommand(bin, opts.Stderr)
	if err := cmd.Start(); err != nil {
		return nil, fmt.Errorf("starting a NeuroDesk server with %s: %w", bin, err)
	}
	...
	if err := waitReady(ctx, opts.Endpoint, timeout, s.exited); err != nil {
		...
		s.Stop()
		return nil, fmt.Errorf("%w. Run 'neurodesk run' in another terminal to see why it did not come up", err)
	}
	return s, nil
}
...
func (s *StartedServer) Stop() {
	...
	s.stopOnce.Do(func() {
		// SIGINT rather than SIGKILL: neurodesk run installs its own handler and
...
func waitReady(ctx context.Context, endpoint string, timeout time.Duration, exited <-chan struct{}) error {
	...
	for {
		...
		case <-waitCtx.Done():
			...
			return fmt.Errorf("the NeuroDesk server did not become ready within %s", timeout)
		case <-ticker.C:
		}
