...
// is the one a user is most likely to meet: 'echo q | neurodesk chat' names
...
// deferred call, and a 'neurodesk run' started for the session is reparented to
...
func prepare(ctx context.Context, opts Options, interactive bool) (_ *preparation, err error) {
	...
	if err != nil {
		if errors.Is(err, ErrUnauthorized) {
			return nil, fmt.Errorf("the NeuroDesk server at %s rejected the API key. Pass --api-key or set NEURODESK_API_KEY", opts.Endpoint)
		}
		...
		if startErr != nil {
			...
			if errors.Is(startErr, ErrDeclined) {
				err = fmt.Errorf("no NeuroDesk server at %s. Start one with 'neurodesk run', or point elsewhere with --endpoint", opts.Endpoint)
			}
			return nil, err
		}
		say(opts.ErrOut, "Started a temporary NeuroDesk server; it stops when you exit. Use 'neurodesk run' for a persistent one.\n")
...
// is the one users meet: 'echo q | neurodesk chat' says to re-run with --cli
...
// nib's shell-capture idiom, out=$(neurodesk chat --height 50%), which is what
...
// 'neurodesk chat > out.txt' no longer refuses either, and renders on /dev/tty
...
func agentOptions(dir, model string, opts Options) app.Options {
	...
	// NeuroDesk checked. Pointing somewhere else for good is NEURODESK_CHAT_ENDPOINT
