...
func generateBashCompletion(app *kong.Application) string {
	...
	sb.WriteString(`# bash completion for neurodesk
# Generated by neurodesk completion bash

_neurodesk_completions()
{
...
complete -F _neurodesk_completions neurodesk
`)
...
func generateZshCompletion(app *kong.Application) string {
	...
	sb.WriteString(`#compdef neurodesk
# Generated by neurodesk completion zsh
...
`)
    ...
    case $state in
        command)
            _describe -t commands 'neurodesk commands' commands
            ;;
        args)
            case $words[1] in
`)
...
func generateFishCompletion(app *kong.Application) string {
	...
	sb.WriteString("# fish completion for neurodesk\n")
	sb.WriteString("# Generated by neurodesk completion fish\n\n")

	// Disable file completions by default
	sb.WriteString("complete -c neurodesk -f\n\n")

	// Global flags
	for _, f := range globalFlags {
		help := strings.ReplaceAll(f.help, "'", "\\'")
		args := fmt.Sprintf("complete -c neurodesk -l %s", f.name)
		if f.short != 0 {
	...
	for _, cmd := range cmds {
		...
		topLevelCmds = append(topLevelCmds, cmd.name)
		help := strings.ReplaceAll(cmd.help, "'", "\\'")
		sb.WriteString(fmt.Sprintf("complete -c neurodesk -n '__fish_use_subcommand' -a %s -d '%s'\n", cmd.name, help))
	}
	sb.WriteString("\n")
	...
	for _, cmd := range cmds {
		...
		for _, sub := range cmds {
			...
			if found && parent == cmd.name && !strings.Contains(child, " ") {
				help := strings.ReplaceAll(sub.help, "'", "\\'")
				sb.WriteString(fmt.Sprintf("complete -c neurodesk -n '__fish_seen_subcommand_from %s' -a %s -d '%s'\n", cmd.name, child, help))
			}
		}
		...
		for _, f := range flags {
			help := strings.ReplaceAll(f.help, "'", "\\'")
			args := fmt.Sprintf("complete -c neurodesk -n '__fish_seen_subcommand_from %s' -l %s", cmd.name, f.name)
			if f.short != 0 {
				args += fmt.Sprintf(" -s %s", string(f.short))
			}
			args += fmt.Sprintf(" -d '%s'", help)
