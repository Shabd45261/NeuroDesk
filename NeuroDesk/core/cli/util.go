...
type UtilCMD struct {
	...
	UsecaseHeuristic UsecaseHeuristicCMD `cmd:"" name:"usecase-heuristic" help:"Checks a specific model config and prints what usecase NeuroDesk will offer for it."`
}
...
type CreateOCIImageCMD struct {
	...
	ImageName string   `default:"neurodesk" help:"Image name"`
...
func (u *CreateOCIImageCMD) Run(ctx *cliContext.Context) error {
	...
	dir, err := os.MkdirTemp("", "neurodesk")
	if err != nil {
		return err
	}
...
func (u *GGUFInfoCMD) Run(ctx *cliContext.Context) error {
	if len(u.Args) == 0 {
		return fmt.Errorf("no GGUF file provided. Usage: neurodesk util gguf-info <path-to-file.gguf>\nGGUF is a binary format for storing quantized language models. You can download GGUF models from https://huggingface.co or install one with 'neurodesk models install <model>'")
	}
...
func (hfscmd *HFScanCMD) Run(ctx *cliContext.Context) error {
	...
	xlog.Info("NeuroDesk Security Scanner - This is BEST EFFORT functionality! Currently limited to huggingface models!")
