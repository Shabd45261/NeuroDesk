...
// data-parallel inference. The head runs NeuroDesk's existing single-
...
type VLLMDistributed struct {
	...
	RegisterTo        string `env:"NEURODESK_REGISTER_TO" help:"Frontend URL for self-registration. Empty = no registration." group:"registration"`
	...
	NodeName          string `env:"NEURODESK_NODE_NAME" help:"Node name for registration (defaults to vllm-<hostname>)" group:"registration"`
	NodeLabels        string `env:"NEURODESK_NODE_LABELS" help:"Comma-separated key=value labels for this node (node.role=vllm-follower is always added)" group:"registration"`
...
// NeuroDesk gRPC backends. The node.role label lets operators scope
