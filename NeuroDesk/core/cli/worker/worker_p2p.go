...
type P2P struct {
	WorkerFlags        `embed:""`
	Token              string `env:"NEURODESK_TOKEN,NEURODESK_P2P_TOKEN,TOKEN" help:"P2P token to use"`
	NoRunner           bool   `env:"NEURODESK_NO_RUNNER,NO_RUNNER" help:"Do not start the llama-cpp-rpc-server"`
	RunnerAddress      string `env:"NEURODESK_RUNNER_ADDRESS,RUNNER_ADDRESS" help:"Address of the llama-cpp-rpc-server"`
	RunnerPort         string `env:"NEURODESK_RUNNER_PORT,RUNNER_PORT" help:"Port of the llama-cpp-rpc-server"`
...
func (r *P2P) Run(ctx *cliContext.Context) error {
	...
	if r.Token == "" {
		return fmt.Errorf("a P2P token is required to join the network. Set it via the NEURODESK_TOKEN environment variable or the --token flag. You can generate a token by running 'neurodesk run --p2p' on the main node. See https://localai.io/features/distribute/ for more information")
	}
