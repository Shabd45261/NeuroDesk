...
type DS4Distributed struct {
	WorkerFlags  `embed:""`
	ExtraDS4Args string `name:"ds4-args" env:"NEURODESK_EXTRA_DS4_ARGS,EXTRA_DS4_ARGS" help:"Arguments passed to ds4-worker (e.g. '--role worker --model m.gguf --layers 20:output --coordinator HOST PORT')"`
}
...
func (r *DS4Distributed) Run(ctx *cliContext.Context) error {
	if r.ExtraDS4Args == "" && len(os.Args) < 4 {
		return fmt.Errorf("usage: neurodesk worker ds4-distributed -- --role worker --model <gguf> --layers <START:END|START:output> --coordinator <host> <port>")
	}
