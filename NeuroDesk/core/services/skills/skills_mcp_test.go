...
import (
	...
	localskills "github.com/mudler/NeuroDesk/core/services/skills"
...
// NeuroDesk FilesystemManager, then list_skills on the still-open session.
...
var _ = Describe("Skills exposed to agent via MCP", func() {
	...
	It("returns a skill created after the MCP session was established", func() {
		...
		// Create a skill via the NeuroDesk FilesystemManager — same code path the
