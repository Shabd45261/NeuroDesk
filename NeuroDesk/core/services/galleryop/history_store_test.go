...
import (
	...
	"github.com/mudler/NeuroDesk/core/services/testutil"
)
...
var _ = Describe("OpCache history source", func() {
	...
	Context("distributed, with a gallery store wired", func() {
		...
		It("answers from the store rather than the local ring", func() {
			...
			recordLocally("llama-cpp", "job-local")
			seed("job-peer", "neurodesk@qwen3-4b", "neurodesk@qwen3-4b", "model_install", "completed", "", false)

			history := cache.History()
			Expect(history).To(HaveLen(1),
				"the store is the whole record; the ring must not be merged in on top of it")
			Expect(history[0].JobID).To(Equal("job-peer"))
			Expect(history[0].ID).To(Equal("neurodesk@qwen3-4b"))
		...
		It("reports a removal as a deletion", func() {
			seed("job-del", "neurodesk@qwen3-4b", "neurodesk@qwen3-4b", "model_delete", "completed", "", false)
		...
		It("falls back to the gallery element name when the cache key never landed", func() {
			...
			seed("job-nokey", "", "neurodesk@qwen3-4b", "model_install", "failed", "disk full", false)
		...
		It("carries a cancellation through as cancelled", func() {
			seed("job-cancel", "neurodesk@qwen3-4b", "neurodesk@qwen3-4b", "model_install", "cancelled", "", false)
		...
		It("returns the record newest first", func() {
			seed("job-1", "neurodesk@one", "neurodesk@one", "model_install", "completed", "", false)
			seed("job-2", "neurodesk@two", "neurodesk@two", "model_install", "completed", "", false)
			seed("job-3", "neurodesk@three", "neurodesk@three", "model_install", "completed", "", false)
		...
		It("records a cancelled install as cancelled, not as a context-cancelled failure", func() {
			...
			Expect(store.Create(&distributed.GalleryOperationRecord{
				ID: "job-cancel", GalleryElementName: "neurodesk@qwen3-4b",
				OpType: "model_install", Status: "downloading",
			})).To(Succeed())
			Expect(store.UpsertCacheKey("job-cancel", "neurodesk@qwen3-4b", false)).To(Succeed())
