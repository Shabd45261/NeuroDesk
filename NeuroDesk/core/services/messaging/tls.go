...
// Use tls:// in NEURODESK_NATS_URL; CA and client cert paths are optional extras.
type TLSFiles struct {
	CA   string // NEURODESK_NATS_TLS_CA — private CA for server verification
	Cert string // NEURODESK_NATS_TLS_CERT — client certificate (mTLS)
	Key  string // NEURODESK_NATS_TLS_KEY — client private key
}
...
func (f TLSFiles) Validate() error {
	if f.Cert != "" && f.Key == "" {
		return fmt.Errorf("NEURODESK_NATS_TLS_KEY is required when NEURODESK_NATS_TLS_CERT is set")
	}
	if f.Key != "" && f.Cert == "" {
		return fmt.Errorf("NEURODESK_NATS_TLS_CERT is required when NEURODESK_NATS_TLS_KEY is set")
	}
	for _, path := range []struct {
		name, path string
	}{
		{"NEURODESK_NATS_TLS_CA", f.CA},
		{"NEURODESK_NATS_TLS_CERT", f.Cert},
		{"NEURODESK_NATS_TLS_KEY", f.Key},
	} {
		if path.path == "" {
			continue
		}
