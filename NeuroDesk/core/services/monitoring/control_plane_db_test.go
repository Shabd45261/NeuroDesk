	...
	Describe("scraping the gauges", func() {
		...
		It("exports the three gauges on a scrape", func() {
			...
			Eventually(func() []string {
				...
				"neurodesk_control_plane_oldest_xmin_age",
				"neurodesk_control_plane_longest_transaction_seconds",
				"neurodesk_control_plane_dead_tuple_ratio",
			))
		})

		It("serves a scrape from the last good sample while the database is down", func() {
			Expect(RegisterControlPlaneDBMetrics(db, 0)).To(Succeed())
			before := collectedInt64Gauge(reader, "neurodesk_control_plane_oldest_xmin_age")

			closeDB(db)

			Expect(collectedMetricNames(reader)).To(ContainElement("neurodesk_control_plane_oldest_xmin_age"),
				"a scrape during a database outage must not drop the gauges")
			Expect(collectedInt64Gauge(reader, "neurodesk_control_plane_oldest_xmin_age")).To(Equal(before))
		})
		...
		It("omits the gauges from a scrape taken before any sample succeeded", func() {
			...
			Expect(collectedMetricNames(reader)).ToNot(ContainElement("neurodesk_control_plane_oldest_xmin_age"))
		})
	})
})
