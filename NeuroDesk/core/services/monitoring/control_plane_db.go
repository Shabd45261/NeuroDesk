...
func RegisterControlPlaneDBMetrics(db *gorm.DB, minInterval time.Duration) error {
	...
	// monitoring.NewNeuroDeskMetricsService has called otel.SetMeterProvider in
	...
	xminAge, err := meter.Int64ObservableGauge("neurodesk_control_plane_oldest_xmin_age",
	...
	longest, err := meter.Float64ObservableGauge("neurodesk_control_plane_longest_transaction_seconds",
		metric.WithDescription("Age of the longest open transaction, in seconds."))
	if err != nil {
		return err
	}
	deadRatio, err := meter.Float64ObservableGauge("neurodesk_control_plane_dead_tuple_ratio",
