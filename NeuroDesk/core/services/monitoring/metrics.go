...
type NeuroDeskMetricsService struct {
	Meter         metric.Meter
	Provider      *metricApi.MeterProvider
	ApiTimeMetric metric.Float64Histogram
}

func (m *NeuroDeskMetricsService) ObserveAPICall(method string, path string, duration float64) {
...
func NewNeuroDeskMetricsService() (*NeuroDeskMetricsService, error) {
	...
	meter := provider.Meter("github.com/mudler/NeuroDesk")
	...
	return &NeuroDeskMetricsService{
		Meter:         meter,
		Provider:      provider,
...
func (lams NeuroDeskMetricsService) Shutdown() error {
	// TODO: Not sure how to actually do this:
	//// setupOTelSDK bootstraps the OpenTelemetry pipeline.
	//// If it does not return an error, make sure to call shutdown for proper cleanup.

	xlog.Warn("NeuroDeskMetricsService Shutdown called, but OTelSDK proper shutdown not yet implemented?")
	return nil
}
