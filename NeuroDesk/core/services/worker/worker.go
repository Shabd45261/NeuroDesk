...
func Run(ctx *cliContext.Context, cfg *Config) error {
	...
	if cfg.RegistrationAuthRequired() && cfg.RegistrationToken == "" {
		return fmt.Errorf("registration auth is required (NEURODESK_REGISTRATION_REQUIRE_AUTH or NEURODESK_DISTRIBUTED_REQUIRE_AUTH) but NEURODESK_REGISTRATION_TOKEN is empty — refusing to start an unauthenticated file-transfer server")
	}
	...
	// if the gallery is unreachable, an ID is unknown, or NEURODESK_GALLERIES is
