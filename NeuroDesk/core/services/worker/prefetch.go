...
func prefetchModels(
	...
	if err != nil {
		...
		xlog.Warn("Skipping model prefetch: invalid NEURODESK_GALLERIES", "error", err)
		return
	}
	if len(modelGalleries) == 0 {
		xlog.Warn("Skipping model prefetch: no model galleries configured (set NEURODESK_GALLERIES)", "models", models)
		return
	}
...
// parseModelGalleries parses the JSON-encoded NEURODESK_GALLERIES value the same
