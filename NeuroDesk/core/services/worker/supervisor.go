...
func (s *backendSupervisor) allocatePort(key string) (int, error) {
	...
	if len(s.freePorts) > 0 {
		...
		xlog.Warn("gRPC port range is exhausted; reusing a port that belonged to another backend. A stale controller row for the previous owner could briefly misroute to this backend — raise NEURODESK_GRPC_MAX_PORT to restore headroom",
			"backend", key, "port", port, "previousOwner", owners[port], "min", minPort, "max", maxPort)
		return s.claimPort(key, port), nil
	}

	return 0, fmt.Errorf("%w: %d-%d is fully consumed by %d running backend(s) and %d port(s) still in quarantine; raise NEURODESK_GRPC_MAX_PORT to widen the range",
