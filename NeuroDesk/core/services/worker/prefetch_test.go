...
var _ = Describe("prefetchModels", func() {
	var (
		...
		galleries = `[{"name":"neurodesk","url":"file:///dev/null"}]`
	)
	...
	It("skips prefetch when NEURODESK_GALLERIES is malformed (non-fatal)", func() {
		f := newFakeInstaller()
		cfg := &Config{
...
var _ = Describe("parseModelGalleries", func() {
	...
	It("parses a valid JSON list", func() {
		g, err := parseModelGalleries(`[{"name":"neurodesk","url":"github:mudler/NeuroDesk/gallery/index.yaml@master"}]`)
		Expect(err).ToNot(HaveOccurred())
		Expect(g).To(HaveLen(1))
		Expect(g[0].Name).To(Equal("neurodesk"))
	})
