...
import (
	...
	"github.com/mudler/NeuroDesk/pkg/safefile"
...
func startFileTransferServer(lis net.Listener, stagingDir, modelsDir, dataDir, token string, maxUploadSize int64, readiness *WorkerReadiness, capacity EphemeralCapacity, logStore ...*model.BackendLogStore) (*http.Server, error) {
	...
	// gets. Set NEURODESK_REGISTRATION_TOKEN (and NEURODESK_REGISTRATION_REQUIRE_AUTH
	// to fail closed) to protect it.
	if token == "" {
		xlog.Warn("HTTP file transfer server starting WITHOUT a registration token — read/write to models/staging/data is unauthenticated for anyone who can reach this port; set NEURODESK_REGISTRATION_TOKEN")
	}
...
func handleAllocTemp(w http.ResponseWriter, r *http.Request, stagingDir string) {
	...
	f, err := os.CreateTemp(stagingDir, "neurodesk-output-*.tmp")
	if err != nil {
