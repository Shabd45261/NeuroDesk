...
func NewHTTPFileStager(httpAddrFor func(nodeID string) (string, error), token string) *HTTPFileStager {
	...
	if v := os.Getenv("NEURODESK_FILE_TRANSFER_TIMEOUT"); v != "" {
	...
	maxRetries := 3
	if v := os.Getenv("NEURODESK_FILE_TRANSFER_RETRIES"); v != "" {
...
func (h *HTTPFileStager) FetchRemote(ctx context.Context, nodeID, remotePath, localDst string) error {
	...
	// remotePath is an absolute path on the worker (e.g. "/staging/neurodesk-output-123.tmp"),
