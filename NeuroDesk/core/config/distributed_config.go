...
import (
	...
	"github.com/mudler/NeuroDesk/pkg/natsauth"
...
type DistributedConfig struct {
	Enabled           bool   // --distributed / NEURODESK_DISTRIBUTED
	InstanceID        string // --instance-id / NEURODESK_INSTANCE_ID (auto-generated UUID if empty)
	NatsURL           string // --nats-url / NEURODESK_NATS_URL
	StorageURL        string // --storage-url / NEURODESK_STORAGE_URL (S3 endpoint)
	RegistrationToken string // --registration-token / NEURODESK_REGISTRATION_TOKEN (required token for node registration)
	...
	RegistrationRequireAuth bool // NEURODESK_REGISTRATION_REQUIRE_AUTH
	// RequireAuth is the umbrella switch (NEURODESK_DISTRIBUTED_REQUIRE_AUTH) for
	...
	RequireAuth      bool // NEURODESK_DISTRIBUTED_REQUIRE_AUTH
	AutoApproveNodes bool // --auto-approve-nodes / NEURODESK_AUTO_APPROVE_NODES (skip admin approval for new workers)
	...
	SharedModels bool // --distributed-shared-models / NEURODESK_DISTRIBUTED_SHARED_MODELS

	// NATS JWT auth (optional; see pkg/natsauth and docs/features/distributed-mode.md)
	NatsAccountSeed  string        // NEURODESK_NATS_ACCOUNT_SEED — account signing seed to mint per-node worker JWTs
	NatsServiceJWT   string        // NEURODESK_NATS_SERVICE_JWT — user JWT for frontends / agent workers
	NatsServiceSeed  string        // NEURODESK_NATS_SERVICE_SEED — signing seed paired with service JWT
	NatsWorkerJWTTTL time.Duration // NEURODESK_NATS_WORKER_JWT_TTL — minted worker JWT lifetime (default 24h)
	NatsRequireAuth  bool          // NEURODESK_NATS_REQUIRE_AUTH — fail startup if NATS credentials are missing
	NatsTLSCA        string        // NEURODESK_NATS_TLS_CA — PEM file for private CA (server verify)
	NatsTLSCert      string        // NEURODESK_NATS_TLS_CERT — client cert for NATS mTLS
	NatsTLSKey       string        // NEURODESK_NATS_TLS_KEY — client key paired with NatsTLSCert

	// S3 configuration (used when StorageURL is set)
	StorageBucket    string // --storage-bucket / NEURODESK_STORAGE_BUCKET
	StorageRegion    string // --storage-region / NEURODESK_STORAGE_REGION
	StorageAccessKey string // --storage-access-key / NEURODESK_STORAGE_ACCESS_KEY
	StorageSecretKey string // --storage-secret-key / NEURODESK_STORAGE_SECRET_KEY
	...
	// idle timeout kills a 20-minute request regardless of what NeuroDesk does —
	...
	// NEURODESK_MODEL_LOAD_WAIT=0.
	ModelLoadWait time.Duration

	MaxUploadSize int64 // Maximum upload body size in bytes (default 50 GB)

	AgentWorkerConcurrency int `yaml:"agent_worker_concurrency" json:"agent_worker_concurrency" env:"NEURODESK_AGENT_WORKER_CONCURRENCY"`
	JobWorkerConcurrency   int `yaml:"job_worker_concurrency" json:"job_worker_concurrency" env:"NEURODESK_JOB_WORKER_CONCURRENCY"`
...
func (c DistributedConfig) Validate() error {
	...
	if c.NatsURL == "" {
		return fmt.Errorf("distributed mode requires --nats-url / NEURODESK_NATS_URL")
	}
	...
	if c.RegistrationToken == "" {
		if c.RegistrationAuthRequired() {
			return fmt.Errorf("registration auth is required (NEURODESK_REGISTRATION_REQUIRE_AUTH or NEURODESK_DISTRIBUTED_REQUIRE_AUTH) but NEURODESK_REGISTRATION_TOKEN is empty")
		}
		xlog.Warn("distributed mode running without registration token — node endpoints and the worker file-transfer server are unprotected; set NEURODESK_REGISTRATION_TOKEN, or NEURODESK_DISTRIBUTED_REQUIRE_AUTH=true to fail closed")
	}
...
const (
	...
	// NeuroDesk (with progress the client can act on) rather than from a proxy
	// dropping the connection.
	DefaultModelLoadWait = 60 * time.Second
)

// ModelLoadWaitUnbounded records NEURODESK_MODEL_LOAD_WAIT=0 — "wait as long as
