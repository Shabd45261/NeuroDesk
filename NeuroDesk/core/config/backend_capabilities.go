...
var BackendCapabilities = map[string]BackendCapability{
	...
	"vllm-cpp": {
		...
		DefaultUsecases:  []string{UsecaseChat},
		AcceptsImages:    true,
		Description:      "vllm.cpp — the NeuroDesk team's C++20 port of vLLM; text generation plus MiniMax-H3 video+audio generation",
	},
	"vllm-omni": {
...
// from llama.cpp's, so NeuroDesk must NOT inject llama.cpp's top_k=40 default for
...
// (concurrent slots). These are llama.cpp server flags; NeuroDesk injects them as
