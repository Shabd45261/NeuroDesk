...
parameters:
  model: "foo-bar"`)
			...
			// download https://raw.githubusercontent.com/mudler/NeuroDesk/v2.25.0/embedded/models/hermes-2-pro-mistral.yaml
			httpClient := http.Client{}
			resp, err := httpClient.Get("https://raw.githubusercontent.com/mudler/NeuroDesk/v2.25.0/embedded/models/hermes-2-pro-mistral.yaml")
			Expect(err).To(BeNil())
...
`)
	...
	// NeuroDesk-generated grammar sent alongside would override the backend's
	...
	Context("TopK default is backend-gated (issue #6632)", func() {
		...
		It("leaves TopK nil for the mlx backend", func() {
			...
			Expect(cfg.TopK).To(BeNil(),
				"mlx_lm's intended default is top_k=0 (disabled); NeuroDesk must not inject 40")
		})
