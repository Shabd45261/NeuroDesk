...
var _ = Describe("default galleries", func() {
	It("serves the model gallery from index.localai.io with GitHub as a mirror", func() {
		...
		Expect(galleries[0].Name).To(Equal("neurodesk"))
		Expect(galleries[0].URL).To(Equal("https://index.localai.io/models"))
		Expect(galleries[0].Mirrors).To(Equal([]string{"github:mudler/NeuroDesk/gallery/index.yaml@master"}))
	})
	...
	It("serves the backend gallery from index.localai.io with GitHub as a mirror", func() {
		...
		Expect(galleries[0].Mirrors).To(Equal([]string{"github:mudler/NeuroDesk/backend/index.yaml@master"}))
	})
	...
	Context("as the option-less startup baseline", func() {
		...
		It("treats NEURODESK_GALLERIES as env-set even when it only drops the mirror", func() {
			o := config.DefaultRuntimeBaseline()
			// Same primary as the default, mirror removed: env-set.
			o.Galleries = []config.Gallery{{Name: "neurodesk", URL: "https://index.localai.io/models"}}
			...
			Expect(o.Galleries[0].Name).To(Equal("neurodesk"),
