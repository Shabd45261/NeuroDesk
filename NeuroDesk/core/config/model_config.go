...
import (
	...
	"github.com/mudler/NeuroDesk/core/services/routing/piipattern"
...
type LimitsConfig struct {
	...
	// local backends whose memory budget tops out before NeuroDesk's
	// queue depth would.
...
//
//   - mode: translate — the backend converts NeuroDesk's internal proto
...
type ProxyConfig struct {
	...
	// Useful when the NeuroDesk-facing model alias differs from the
...
type EmbeddingCacheConfig struct {
	// EmbeddingModel names the loaded NeuroDesk model used to embed
...
type Pipeline struct {
	...
	// selection (NeuroDesk classifier mode): each user turn is scored
	...
	// can still enable per session via session.update neurodesk_classifier.
	...
	// realtime session start. By default (false) NeuroDesk loads every configured
...
// neurodesk_classifier extension (see
...
func (cfg *ModelConfig) SetDefaults(opts ...ConfigLoaderOption) {
	...
	// a NeuroDesk-generated grammar alongside overrides the backend's native
...
// IsGoSidePooling reports whether scheme pools in NeuroDesk's Go layer,
