...
var _ = Describe("DistributedConfig.Validate registration auth", func() {
	It("rejects an empty registration token when RequireAuth is set", func() {
		...
		err := c.Validate()
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("NEURODESK_REGISTRATION_REQUIRE_AUTH"))
		Expect(err.Error()).To(ContainSubstring("NEURODESK_REGISTRATION_TOKEN"))
	})
	...
	It("rejects an empty token when the umbrella RequireAuth is set", func() {
		...
		err := c.Validate()
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("NEURODESK_DISTRIBUTED_REQUIRE_AUTH"))
		Expect(err.Error()).To(ContainSubstring("NEURODESK_REGISTRATION_TOKEN"))
	})
	...
	It("the umbrella implies NATS auth is required", func() {
		...
		err := c.Validate()
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("NEURODESK_NATS_REQUIRE_AUTH"))
	})
})
