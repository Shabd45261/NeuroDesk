...
type RuntimeSettings struct {
	...
	// NeuroDesk Assistant settings — read live by the chat handler at request
	// entry, so flipping the toggle takes effect on the next request.
	NeuroDeskAssistantEnabled *bool `json:"neurodesk_assistant_enabled,omitempty"` // negation of DisableNeuroDeskAssistant for UI clarity
	...
	// All optional — empty values fall back to bundled NeuroDesk defaults.
