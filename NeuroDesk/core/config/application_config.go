...
type ApplicationConfig struct {
	...
	// (scheme+host[:port]), set via NEURODESK_BASE_URL. When non-empty it is
	// authoritative for every self-referential URL NeuroDesk emits (OAuth
	...
	// without NeuroDesk holding the upstream API key. Clients set
	// HTTPS_PROXY=http://neurodesk:port and trust the CA cert
	// NeuroDesk exposes at /api/middleware/proxy-ca.crt.
	MITMListen string
	...
	// --require-backend-integrity / NEURODESK_REQUIRE_BACKEND_INTEGRITY.
	...
	// middleware wraps the response writer and attaches an "X-NeuroDesk-Node"
	...
	// --expose-node-header / NEURODESK_EXPOSE_NODE_HEADER for debugging,
	// observability and load-balancer attribution.
	ExposeNodeHeader bool

	// NeuroDesk Assistant chat modality. Hard-disable the in-process admin MCP
	// server with this flag; runtime-toggleable via /api/settings.
	DisableNeuroDeskAssistant bool
	...
	// strings fall back to bundled NeuroDesk defaults.
...
type AgentPoolConfig struct {
	Enabled               bool   // default: true (disabled by NEURODESK_DISABLE_AGENTS=true)
	StateDir              string // default: DynamicConfigsDir (NeuroDesk configuration folder)
	APIURL                string // default: self-referencing NeuroDesk (http://127.0.0.1:<port>)
	APIKey                string // default: first API key from NeuroDesk config
	DefaultModel          string
...
// NEURODESK_PII_DEFAULT_DETECTORS. Empty leaves the value to
...
// WithDisableNeuroDeskAssistant hard-disables the in-process admin MCP server.
// When set, the chat-handler branch for metadata.neurodesk_assistant=true
// returns a "feature unavailable" error.
func WithDisableNeuroDeskAssistant(disabled bool) AppOption {
	return func(o *ApplicationConfig) {
		o.DisableNeuroDeskAssistant = disabled
	}
}

// WithExposeNodeHeader enables the X-NeuroDesk-Node response header on
