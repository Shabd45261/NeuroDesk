...
var _ = Describe("Gallery mirrors", func() {
	// Galleries are configured as a JSON list in NEURODESK_GALLERIES and edited
	...
	It("round-trips through JSON", func() {
		const in = `[{"url":"https://primary.example/index.yaml","name":"neurodesk",` +
			`"mirrors":["github:mudler/NeuroDesk/gallery/index.yaml@master","file:///srv/index.yaml"]}]`
		...
		want := []string{"github:mudler/NeuroDesk/gallery/index.yaml@master", "file:///srv/index.yaml"}
		...
		Expect(again[0].Name).To(Equal("neurodesk"))
	})
	...
	It("round-trips through YAML", func() {
		const in = "- url: https://primary.example/index.yaml\n" +
			"  name: neurodesk\n" +
			"  mirrors:\n" +
			"    - github:mudler/NeuroDesk/gallery/index.yaml@master\n" +
			"    - https://fallback.example/index.yaml\n"

		var galleries []config.Gallery
		Expect(yaml.Unmarshal([]byte(in), &galleries)).To(Succeed())
		want := []string{"github:mudler/NeuroDesk/gallery/index.yaml@master", "https://fallback.example/index.yaml"}
		Expect(galleries[0].Mirrors).To(Equal(want))
	...
	Context("a gallery without mirrors", func() {
		...
		It("marshals to unchanged YAML", func() {
			y, err := yaml.Marshal(config.Gallery{URL: "https://x/index.yaml", Name: "neurodesk"})
			Expect(err).ToNot(HaveOccurred())
...
var _ = Describe("GalleriesEqual", func() {
	...
	withMirror := []config.Gallery{{URL: "https://x/index.yaml", Name: "n", Mirrors: []string{"github:mudler/NeuroDesk/gallery/index.yaml@master"}}}
	...
	It("reports identical mirrored lists as equal", func() {
		Expect(config.GalleriesEqual(withMirror, []config.Gallery{
			{URL: "https://x/index.yaml", Name: "n", Mirrors: []string{"github:mudler/NeuroDesk/gallery/index.yaml@master"}},
		})).To(BeTrue())
	})
