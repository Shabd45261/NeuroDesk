...
func DefaultRegistry() map[string]FieldMetaOverride {
	...
	return map[string]FieldMetaOverride{
		...
		"artifacts": {
			...
			Description: "Controller-managed model sources. NeuroDesk resolves, downloads, verifies, and binds these sources before a backend loads.",
		...
		"parameters.pooling": {
			...
			Description: "How per-token embedding vectors are reduced to one embedding: the backend pools itself (default), or NeuroDesk pools Go-side (mean, last, decayed_mean) from raw per-token vectors",
			Component:   "select",
			Options: []FieldOption{
		...
		"pipeline.disable_warmup": {
			...
			Description: "Turn off eager pre-loading of the pipeline's sub-models at realtime session start. By default NeuroDesk loads every configured sub-model backend (VAD, transcription, LLM, TTS, sound detection, voice recognition) before the session starts and blocks until they are ready, so the first turn pays no cold-start cost and a model that fails to load is reported at session start instead of mid-call. Enable this to restore the lazy 'load on first use' behavior — session start no longer waits on loading and load errors surface on the first turn instead. Useful to keep idle sessions from holding model memory they may never use.",
			Component:   "toggle",
			Order:       90,
		},
		...
		"proxy.upstream_model": {
			...
			Description: "Model name sent to the upstream. Leave empty to forward the client's model field unchanged. Useful when the NeuroDesk alias differs from the upstream's canonical name.",
			Component:   "input",
			Order:       212,
		},
		...
		"router.knn.embedding_revision": {
			...
			Description: "Optional identity suffix for embedding services or remote models whose weights may change without their NeuroDesk model name/config changing. Bump this value after replacing such a model so the persisted corpus is re-embedded.",
			Component:   "input",
			Order:       242,
		},
		"router.knn.k": {
