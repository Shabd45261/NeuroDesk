...
import (
	...
	"github.com/mudler/NeuroDesk/pkg/distributedhdr"
...
func initDistributed(cfg *config.ApplicationConfig, authDB *gorm.DB, configLoader *config.ModelConfigLoader) (*DistributedServices, error) {
	...
	if !cfg.Auth.Enabled {
		return nil, fmt.Errorf("distributed mode requires authentication to be enabled (--auth / NEURODESK_AUTH=true)")
	}
	...
	if natsAuth.RequireAuth && (natsAuth.ServiceUserJWT == "" || natsAuth.ServiceUserSeed == "") {
		return nil, fmt.Errorf("NEURODESK_NATS_REQUIRE_AUTH requires NEURODESK_NATS_SERVICE_JWT and NEURODESK_NATS_SERVICE_SEED")
	}
	...
	// Seed declarative per-model scheduling config (NEURODESK_MODEL_SCHEDULING /
	// NEURODESK_MODEL_SCHEDULING_CONFIG). Authoritative: overwrites matching models
	...
	if cfg.Distributed.DiskHeadroomDisabled {
		xlog.Info("Disk-headroom admission check is DISABLED: node selection will ignore whether a worker can store the model, and staging may fail with ENOSPC partway through a transfer",
			"knob", config.FlagDiskHeadroomCheck, "env", "NEURODESK_DISTRIBUTED_DISK_HEADROOM_CHECK")
	}
