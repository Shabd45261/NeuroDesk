...
import (
	...
	corebackend "github.com/mudler/NeuroDesk/core/backend"
	...
	"github.com/mudler/NeuroDesk/core/services/agentpool"
	"github.com/mudler/NeuroDesk/core/services/cloudproxy/mitm"
	"github.com/mudler/NeuroDesk/core/services/facerecognition"
	...
	"github.com/mudler/NeuroDesk/core/services/routing/admission"
	...
	"github.com/mudler/NeuroDesk/core/services/routing/piidetector"
	"github.com/mudler/NeuroDesk/core/services/routing/router"
	"github.com/mudler/NeuroDesk/core/services/voiceprofile"
	"github.com/mudler/NeuroDesk/core/services/voicerecognition"
	"github.com/mudler/NeuroDesk/core/templates"
	"github.com/mudler/NeuroDesk/core/trace"
	pkggrpc "github.com/mudler/NeuroDesk/pkg/grpc"
	neurodesktools "github.com/mudler/NeuroDesk/pkg/mcp/neurodesktools"
	neurodeskInproc "github.com/mudler/NeuroDesk/pkg/mcp/neurodesktools/inproc"
...
type Application struct {
	...
	// NeuroDesk Assistant in-process MCP server. nil when DisableNeuroDeskAssistant
	// is set; otherwise initialised in start() after galleryService.
	neuroDeskAssistant *mcpTools.NeuroDeskAssistantHolder
...
func newApplication(appConfig *config.ApplicationConfig) *Application {
	...
	// Face-recognition registry backed by NeuroDesk's built-in vector store.
	...
	const (
		faceStoreName  = "neurodesk-face-biometrics"
		voiceStoreName = "neurodesk-voice-biometrics"
	)
...
// NeuroDeskAssistant returns the in-process MCP holder used by the chat handler
// when an admin opts into the assistant modality. Returns nil when the feature
// is disabled at startup.
func (a *Application) NeuroDeskAssistant() *mcpTools.NeuroDeskAssistantHolder {
	return a.neuroDeskAssistant
}
...
func (a *Application) MetricsService() *monitoring.NeuroDeskMetricsService {
...
func (a *Application) start() error {
	...
	// NeuroDesk Assistant: in-process MCP server exposing admin tools. Initialised
	// once at startup and reused across chat sessions that opt in via metadata.
	if !a.applicationConfig.DisableNeuroDeskAssistant {
		holder := mcpTools.NewNeuroDeskAssistantHolder()
		var nodeRegistry *nodes.NodeRegistry
		if a.distributed != nil {
			nodeRegistry = a.distributed.Registry
		}
		assistantClient := neurodeskInproc.New(
			a.applicationConfig,
		...
		if err := holder.Initialize(a.applicationConfig.Context, assistantClient, neurodesktools.Options{}); err != nil {
			// Why log+continue instead of fail: the assistant is an optional
			// feature; a failure here must not take down the whole server.
			xlog.Warn("NeuroDesk Assistant initialisation failed; feature unavailable", "error", err)
		} else {
			a.neuroDeskAssistant = holder
