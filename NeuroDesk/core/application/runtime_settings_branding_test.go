...
var _ = Describe("loadRuntimeSettingsFromFile", func() {
	...
	// endpoint fell back to NeuroDesk defaults.
	...
	Describe("adjacent restart-loss fields", func() {
		...
		It("disables the NeuroDesk Assistant when neurodesk_assistant_enabled=false", func() {
			cfg := &config.ApplicationConfig{DynamicConfigsDir: seedSettings(`{"neurodesk_assistant_enabled": false}`)}
			loadRuntimeSettingsFromFile(cfg)
			Expect(cfg.DisableNeuroDeskAssistant).To(BeTrue())
		})
	...
	Describe("PII default detectors", func() {
		...
		It("does not override an env/CLI-set value (NEURODESK_PII_DEFAULT_DETECTORS)", func() {
			cfg := &config.ApplicationConfig{
	...
	Describe("agent pool fields", func() {
		...
		It("loads agent_pool_collection_db_path", func() {
			cfg := &config.ApplicationConfig{DynamicConfigsDir: seedSettings(`{"agent_pool_collection_db_path": "/var/lib/neurodesk/collections.db"}`)}
			loadRuntimeSettingsFromFile(cfg)
			Expect(cfg.AgentPool.CollectionDBPath).To(Equal("/var/lib/neurodesk/collections.db"))
		})
		...
		It("loads agent_pool_database_url", func() {
			cfg := &config.ApplicationConfig{DynamicConfigsDir: seedSettings(`{"agent_pool_database_url": "postgres://user:pass@db:5432/neurodesk"}`)}
			loadRuntimeSettingsFromFile(cfg)
			Expect(cfg.AgentPool.DatabaseURL).To(Equal("postgres://user:pass@db:5432/neurodesk"))
		})
	...
	// NEURODESK_THREADS/CLI still wins. WithThreads no longer eagerly
