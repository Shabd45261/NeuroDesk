...
// ModelLoadRequest asks NeuroDesk to pre-load a model into memory by name, so the
...
type TTSRequest struct {
	...
	// (NeuroDesk extension, e.g. Chatterbox exaggeration/cfg_weight/temperature).
...
// for external routers that want NeuroDesk's classifier opinion without
// committing NeuroDesk to handle the request.
