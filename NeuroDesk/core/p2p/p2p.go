...
func newNodeOpts(token string) ([]node.Option, error) {
	...
	noDHT := os.Getenv("NEURODESK_P2P_DISABLE_DHT") == "true"
	noLimits := os.Getenv("NEURODESK_P2P_ENABLE_LIMITS") != "true"

	var listenMaddrs []string
	var bootstrapPeers []string

	laddrs := os.Getenv("NEURODESK_P2P_LISTEN_MADDRS")
	if laddrs != "" {
		listenMaddrs = strings.Split(laddrs, ",")
	}

	bootmaddr := os.Getenv("NEURODESK_P2P_BOOTSTRAP_PEERS_MADDRS")
	if bootmaddr != "" {
		bootstrapPeers = strings.Split(bootmaddr, ",")
	}

	dhtAnnounceMaddrs := stringsToMultiAddr(strings.Split(os.Getenv("NEURODESK_P2P_DHT_ANNOUNCE_MADDRS"), ","))

	libp2ploglevel := os.Getenv("NEURODESK_P2P_LIB_LOGLEVEL")
	if libp2ploglevel == "" {
