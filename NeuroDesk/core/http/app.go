...
import (
	...
	httpMiddleware "github.com/mudler/NeuroDesk/core/http/middleware"
	"github.com/mudler/NeuroDesk/core/http/routes"

	"github.com/mudler/NeuroDesk/core/application"
	"github.com/mudler/NeuroDesk/core/schema"
	"github.com/mudler/NeuroDesk/core/services/distributed"
...
// @title NeuroDesk API
// @version 2.0.0
// @description The NeuroDesk Rest API.
// @termsOfService
// @contact.name NeuroDesk
// @contact.url https://localai.io
// @license.name MIT
// @license.url https://raw.githubusercontent.com/mudler/NeuroDesk/master/LICENSE
// @BasePath /
// @schemes http https
...
func API(application *application.Application) (*echo.Echo, error) {
	...
	// monitoring.NewNeuroDeskMetricsService a second time, which would
	...
	if metricsService := application.MetricsService(); metricsService != nil {
		e.Use(neurodesk.NeuroDeskMetricsAPIMiddleware(metricsService))
	...
	if application.ApplicationConfig().CORS {
		if application.ApplicationConfig().CORSAllowOrigins == "" {
			xlog.Warn("NEURODESK_CORS=true but NEURODESK_CORS_ALLOW_ORIGINS is empty; refusing to register a wildcard CORS policy. Set the allowlist or unset NEURODESK_CORS.")
		} else {
	...
	// CSRF middleware (enabled by default, disable with NEURODESK_DISABLE_CSRF=true)
	//
	...
	// Create opcache for tracking UI operations (used by both UI and NeuroDesk routes)
	...
	routes.RegisterNeuroDeskRoutes(e, requestExtractor, application.ModelConfigLoader(), application.ModelLoader(), application.ApplicationConfig(), application.GalleryService(), opcache, application.TemplatesEvaluator(), application, adminMiddleware, mcpJobsMw, mcpMw)
	...
	// Log startup message
	e.Server.RegisterOnShutdown(func() {
		xlog.Info("NeuroDesk API server shutting down")
	})
