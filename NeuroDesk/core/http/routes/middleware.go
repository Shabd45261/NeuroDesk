...
func RegisterMiddlewareRoutes(e *echo.Echo, app *application.Application) {
	...
	e.GET("/api/middleware/proxy-ca.crt", func(c echo.Context) error {
		...
		c.Response().Header().Set("Content-Disposition", `attachment; filename="neurodesk-mitm-ca.crt"`)
	...
	decideHandler := neurodesk.RouterDecideEndpoint(
	...
	corpusAdd := neurodesk.RouterCorpusAddEndpoint(app.ModelConfigLoader(), app.ApplicationConfig(), app.RouterCorpus(), corpusDeps)
	corpusStats := neurodesk.RouterCorpusStatsEndpoint(app.ModelConfigLoader(), app.ApplicationConfig(), app.RouterCorpus())
	corpusClear := neurodesk.RouterCorpusClearEndpoint(app.ModelConfigLoader(), app.ApplicationConfig(), app.RouterCorpus(), corpusDeps)
