...
import (
	...
	"github.com/mudler/NeuroDesk/core/http/endpoints/ollama"
...
func RegisterOllamaRoutes(app *echo.Echo,
	...
	chatMiddleware := []echo.MiddlewareFunc{
		...
		re.SetModelAndConfig(func() schema.NeuroDeskRequest { return new(schema.OllamaChatRequest) }),
	...
	generateMiddleware := []echo.MiddlewareFunc{
		...
		traceMiddleware,
		re.BuildFilteredFirstAvailableDefaultModel(config.BuildUsecaseFilterFn(config.FLAG_CHAT)),
		re.SetModelAndConfig(func() schema.NeuroDeskRequest { return new(schema.OllamaGenerateRequest) }),
	...
	embedMiddleware := []echo.MiddlewareFunc{
		...
		re.SetModelAndConfig(func() schema.NeuroDeskRequest { return new(schema.OllamaEmbedRequest) }),
...
func setOllamaChatRequestContext(appConfig *config.ApplicationConfig) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			input, ok := c.Get(middleware.CONTEXT_LOCALS_KEY_NEURODESK_REQUEST).(*schema.OllamaChatRequest)
			if !ok || input.Model == "" {
			...
			c.Set(middleware.CONTEXT_LOCALS_KEY_NEURODESK_REQUEST, input)
...
func setOllamaGenerateRequestContext(appConfig *config.ApplicationConfig) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			input, ok := c.Get(middleware.CONTEXT_LOCALS_KEY_NEURODESK_REQUEST).(*schema.OllamaGenerateRequest)
			if !ok || input.Model == "" {
				return echo.ErrBadRequest
			}
