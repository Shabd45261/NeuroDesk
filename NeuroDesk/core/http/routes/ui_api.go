...
func RegisterUIAPIRoutes(app *echo.Echo, cl *config.ModelConfigLoader, ml *model.ModelLoader, appConfig *config.ApplicationConfig, galleryService *galleryop.GalleryService, opcache *galleryop.OpCache, applicationInstance *application.Application, adminMiddleware echo.MiddlewareFunc) {
	...
	app.GET("/api/models", func(c echo.Context) error {
		...
		case "":
		case cpuOnlyCapability:
			// A CPU-only box should only be offered models whose real engine can
			// serve it without a GPU. The signal is the same capability machinery
			// the install resolver uses (a "default"-capability host), trusted
			// per engine rather than guessed from a model's size, name or tag.
	...
	app.POST("/api/models/install/:id", func(c echo.Context) error {
		...
		// URL decode the gallery ID (e.g., "neurodesk%40model" -> "neurodesk@model")
	...
	app.GET("/api/models/config-metadata", neurodesk.ConfigMetadataEndpoint(), adminMiddleware)

	// Autocomplete providers for config fields (dynamic values only)
	app.GET("/api/models/config-metadata/autocomplete/:provider", neurodesk.AutocompleteEndpoint(cl, ml, appConfig), adminMiddleware)

	// PATCH config endpoint - partial update using nested JSON merge
	app.PATCH("/api/models/config-json/:name", neurodesk.PatchConfigEndpoint(cl, galleryService, appConfig, modelRevisionLifecycleFor(applicationInstance)), adminMiddleware)

	// VRAM estimation endpoint
	app.POST("/api/models/vram-estimate", neurodesk.VRAMEstimateEndpoint(cl, appConfig), adminMiddleware)
	...
	if !appConfig.DisableRuntimeSettings {
		// Settings API
		app.GET("/api/settings", neurodesk.GetSettingsEndpoint(applicationInstance), adminMiddleware)
		app.POST("/api/settings", neurodesk.UpdateSettingsEndpoint(applicationInstance), adminMiddleware)
	}
	...
	app.GET("/api/branding", neurodesk.GetBrandingEndpoint(appConfig))
	app.GET("/branding/asset/:kind", neurodesk.ServeBrandingAssetEndpoint(appConfig))
	app.POST("/api/branding/asset/:kind", neurodesk.UploadBrandingAssetEndpoint(appConfig), adminMiddleware)
	app.DELETE("/api/branding/asset/:kind", neurodesk.DeleteBrandingAssetEndpoint(appConfig), adminMiddleware)
