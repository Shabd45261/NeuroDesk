...
import (
	...
	"github.com/mudler/NeuroDesk/core/http/endpoints/openai"
...
func RegisterOpenAIRoutes(app *echo.Echo,
	...
	// X-NeuroDesk-Node attribution middleware: wraps the response writer and
	...
	chatHandler := openai.ChatEndpoint(application.ModelConfigLoader(), application.ModelLoader(), application.TemplatesEvaluator(), application.ApplicationConfig(), natsClient, application.NeuroDeskAssistant(), chatCompressor)
	chatMiddleware := []echo.MiddlewareFunc{
		nodeHeaderMiddleware,
		usageMiddleware,
		traceMiddleware,
		re.BuildFilteredFirstAvailableDefaultModel(config.BuildUsecaseFilterFn(config.FLAG_CHAT)),
		re.SetModelAndConfig(func() schema.NeuroDeskRequest { return new(schema.OpenAIRequest) }),
	...
	moderationMiddleware := []echo.MiddlewareFunc{
		...
		re.BuildFilteredFirstAvailableDefaultModel(config.BuildUsecaseFilterFn(config.FLAG_COMPLETION)),
		re.BuildConstantDefaultModelNameMiddleware("gpt-4o"),
		re.SetModelAndConfig(func() schema.NeuroDeskRequest { return new(schema.ModerationRequest) }),
		middleware.AdmissionControl(application.AdmissionLimiter(), application.PIIEvents()),
	}
	...
	audioSpeechHandler := neurodesk.TTSEndpoint(application.ModelConfigLoader(), application.ModelLoader(), application.ApplicationConfig(), application.VoiceProfileStore())
	audioSpeechMiddleware := []echo.MiddlewareFunc{
		...
		re.SetModelAndConfig(func() schema.NeuroDeskRequest { return new(schema.TTSRequest) }),
	}
	...
	// (NeuroDesk-specific, additive superset of /v1/models).
