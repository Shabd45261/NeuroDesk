...
func RegisterNodeSelfServiceRoutes(e *echo.Echo, registry *nodes.NodeRegistry, registrationToken string, autoApprove bool, authDB *gorm.DB, hmacSecret string, natsCfg natsauth.Config) {
	...
	node.POST("/register", neurodesk.RegisterNodeEndpoint(registry, registrationToken, autoApprove, authDB, hmacSecret, natsCfg))
	node.POST("/:id/heartbeat", neurodesk.HeartbeatEndpoint(registry))
	node.POST("/:id/drain", neurodesk.DrainNodeEndpoint(registry))
	node.POST("/:id/resume", neurodesk.ResumeNodeEndpoint(registry))
	node.POST("/:id/deregister", neurodesk.DeactivateNodeEndpoint(registry))
	node.GET("/:id/models", neurodesk.GetNodeModelsEndpoint(registry))
	node.DELETE("/:id", neurodesk.DeactivateNodeEndpoint(registry))
}
...
func RegisterNodeAdminRoutes(e *echo.Echo, registry *nodes.NodeRegistry, unloader nodes.NodeCommandSender, galleryService *galleryop.GalleryService, opcache *galleryop.OpCache, appConfig *config.ApplicationConfig, adminMw echo.MiddlewareFunc, authDB *gorm.DB, hmacSecret string, registrationToken string, natsCfg natsauth.Config) {
	...
	admin.GET("", neurodesk.ListNodesEndpoint(registry))

	// Cluster-wide loaded models (registered before /:id to avoid route conflicts)
	admin.GET("/models", neurodesk.ListAllNodeModelsEndpoint(registry))

	// Model scheduling (registered before /:id to avoid route conflicts)
	admin.GET("/scheduling", neurodesk.ListSchedulingEndpoint(registry))
	admin.GET("/scheduling/:model", neurodesk.GetSchedulingEndpoint(registry))
	admin.POST("/scheduling", neurodesk.SetSchedulingEndpoint(registry))
	admin.DELETE("/scheduling/:model", neurodesk.DeleteSchedulingEndpoint(registry))

	admin.GET("/:id", neurodesk.GetNodeEndpoint(registry))
	admin.GET("/:id/models", neurodesk.GetNodeModelsEndpoint(registry))
	admin.DELETE("/:id", neurodesk.DeregisterNodeEndpoint(registry))
	admin.POST("/:id/drain", neurodesk.DrainNodeEndpoint(registry))
	admin.POST("/:id/resume", neurodesk.ResumeNodeEndpoint(registry))
	admin.POST("/:id/approve", neurodesk.ApproveNodeEndpoint(registry, authDB, hmacSecret, natsCfg))

	// Backend management on workers
	admin.GET("/:id/backends", neurodesk.ListBackendsOnNodeEndpoint(unloader, registry))
	admin.POST("/:id/backends/install", neurodesk.InstallBackendOnNodeEndpoint(unloader, galleryService, opcache, appConfig))
	...
	admin.POST("/:id/backends/upgrade", neurodesk.UpgradeBackendOnNodeEndpoint(galleryService, opcache, appConfig))
	admin.POST("/:id/backends/delete", neurodesk.DeleteBackendOnNodeEndpoint(unloader))

	// Model management on workers
	admin.POST("/:id/models/unload", neurodesk.UnloadModelOnNodeEndpoint(unloader, registry))
	admin.POST("/:id/models/delete", neurodesk.DeleteModelOnNodeEndpoint(unloader, registry))

	// Backend log streaming (proxied from worker HTTP server)
	admin.GET("/:id/backend-logs", neurodesk.NodeBackendLogsListEndpoint(registry, registrationToken))
	admin.GET("/:id/backend-logs/:modelId", neurodesk.NodeBackendLogsLinesEndpoint(registry, registrationToken))

	// Label management
	admin.GET("/:id/labels", neurodesk.GetNodeLabelsEndpoint(registry))
	admin.PUT("/:id/labels", neurodesk.SetNodeLabelsEndpoint(registry))
	admin.PATCH("/:id/labels", neurodesk.MergeNodeLabelsEndpoint(registry))
	admin.DELETE("/:id/labels/:key", neurodesk.DeleteNodeLabelEndpoint(registry))
	...
	admin.PUT("/:id/max-replicas-per-model", neurodesk.UpdateMaxReplicasPerModelEndpoint(registry))
	admin.DELETE("/:id/max-replicas-per-model", neurodesk.ResetMaxReplicasPerModelEndpoint(registry))
	...
	admin.PUT("/:id/vram-budget", neurodesk.UpdateVRAMBudgetEndpoint(registry))
	admin.DELETE("/:id/vram-budget", neurodesk.ResetVRAMBudgetEndpoint(registry))

	// WebSocket proxy for real-time log streaming from workers
	e.GET("/ws/nodes/:id/backend-logs/:modelId", neurodesk.NodeBackendLogsWSEndpoint(registry, registrationToken), readyMw, adminMw)
}
...
// set a registration token via NEURODESK_REGISTRATION_TOKEN or config.
//
