...
import (
	...
	"github.com/mudler/NeuroDesk/core/http/endpoints/anthropic"
...
func RegisterAnthropicRoutes(app *echo.Echo,
	...
	messagesMiddleware := []echo.MiddlewareFunc{
		...
		re.SetModelAndConfig(func() schema.NeuroDeskRequest { return new(schema.AnthropicRequest) }),
...
func setAnthropicRequestContext(appConfig *config.ApplicationConfig) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			input, ok := c.Get(middleware.CONTEXT_LOCALS_KEY_NEURODESK_REQUEST).(*schema.AnthropicRequest)
			if !ok || input.Model == "" {
