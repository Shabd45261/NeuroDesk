...
func clusterModelEnv(ctx context.Context, systemState *system.SystemState, memory ClusterMemoryProvider, capabilities neurodesk.ClusterCapabilityProvider) gallery.ResolveEnv {
	reading := resolveClusterMemory(ctx, memory)
	caps := neurodesk.ResolveClusterCapabilities(ctx, capabilities)
