...
const CONTEXT_LOCALS_KEY_NEURODESK_REQUEST = "NEURODESK_REQUEST"
...
func (re *RequestExtractor) SetModelAndConfig(initializer func() schema.NeuroDeskRequest) echo.MiddlewareFunc {
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			input := initializer()
			if input == nil {
			...
			c.Set(CONTEXT_LOCALS_KEY_NEURODESK_REQUEST, input)
...
func (re *RequestExtractor) SetOpenAIRequest(c echo.Context) error {
	input, ok := c.Get(CONTEXT_LOCALS_KEY_NEURODESK_REQUEST).(*schema.OpenAIRequest)
	if !ok || input.Model == "" {
...
func mergeOpenAIRequestAndModelConfig(config *config.ModelConfig, input *schema.OpenAIRequest) error {
	...
	// Per-request Go-side pooling override for /v1/embeddings (NeuroDesk
...
func (re *RequestExtractor) SetOpenResponsesRequest(c echo.Context) error {
	input, ok := c.Get(CONTEXT_LOCALS_KEY_NEURODESK_REQUEST).(*schema.OpenResponsesRequest)
	if !ok || input.Model == "" {
		return echo.ErrBadRequest
	}
