...
// Route-level integration coverage for the X-NeuroDesk-Node middleware.
//
...
//          Write/Flush, so X-NeuroDesk-Node lands on the response map
...
//     MCP NATS client, and the NeuroDesk Assistant holder; standing
...
// orderRecorder snapshots the X-NeuroDesk-Node header value AT THE MOMENT
...
var _ = Describe("ExposeNodeHeader middleware (route-level integration)", func() {
	...
	It("stamps X-NeuroDesk-Node before the first SSE byte via the real router + middleware chain", func() {
		...
		e.POST("/v1/chat/completions", func(c echo.Context) error {
			...
			// wrapper's maybeSet, so the X-NeuroDesk-Node header
		...
		Expect(first).To(ContainSubstring("node="+fakeNodeID),
			"first writer event must carry the X-NeuroDesk-Node header (chain: middleware.Use -> e.POST -> handler.Write/Flush); got events: %v", events)
