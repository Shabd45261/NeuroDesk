...
// events happen. Used by the streaming spec to assert that the X-NeuroDesk-Node
...
func (o *orderedWriter) Write(b []byte) (int, error) {
	// Snapshot the X-NeuroDesk-Node header value AT THE INSTANT the underlying
...
var _ = Describe("ExposeNodeHeader middleware", func() {
	...
	When("ExposeNodeHeader is false", func() {
		It("does not set the X-NeuroDesk-Node header", func() {
	...
	When("ExposeNodeHeader is true and the router stamps a node ID", func() {
		It("sets the X-NeuroDesk-Node header on a buffered response", func() {
