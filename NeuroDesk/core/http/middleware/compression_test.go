...
var _ = Describe("Context compression middleware", func() {
	It("transforms requests and stamps metadata", func() {
		...
		c.Set(httpmiddleware.CONTEXT_LOCALS_KEY_NEURODESK_REQUEST, &schema.OpenAIRequest{Messages: []schema.Message{{Role: "user", Content: "old"}}})
		...
		handler := middleware(func(c echo.Context) error {
			input := c.Get(httpmiddleware.CONTEXT_LOCALS_KEY_NEURODESK_REQUEST).(*schema.OpenAIRequest)
	...
	It("maps overflow to HTTP 413", func() {
		...
		ctxSize := 1
		c.Set(httpmiddleware.CONTEXT_LOCALS_KEY_MODEL_CONFIG, &config.ModelConfig{Name: "primary", LLMConfig: config.LLMConfig{ContextSize: &ctxSize}, Compression: config.CompressionConfig{Enabled: true}})
		c.Set(httpmiddleware.CONTEXT_LOCALS_KEY_NEURODESK_REQUEST, &schema.OpenAIRequest{Messages: []schema.Message{{Role: "user"}, {Role: "user"}}})
	...
	It("resolves the automatic context-size sentinel", func() {
		...
		c.Set(httpmiddleware.CONTEXT_LOCALS_KEY_NEURODESK_REQUEST, &schema.OpenAIRequest{Messages: []schema.Message{{Role: "user", Content: "hello"}}})
