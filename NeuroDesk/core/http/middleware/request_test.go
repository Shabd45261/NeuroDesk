...
func newRequestApp(re *RequestExtractor) *echo.Echo {
	...
	e.POST("/v1/chat/completions",
		...
		re.SetModelAndConfig(func() schema.NeuroDeskRequest {
...
var _ = Describe("SetModelAndConfig middleware", func() {
	...
	BeforeEach(func() {
		var err error
		modelDir, err = os.MkdirTemp("", "neurodesk-test-models-*")
...
var _ = Describe("SetModelAndConfig alias resolution", func() {
	...
	BeforeEach(func() {
		var err error
		modelDir, err = os.MkdirTemp("", "neurodesk-alias-*")
		Expect(err).ToNot(HaveOccurred())
	})
...
var _ = Describe("SetModelAndConfig per-request pooling override (embeddings)", func() {
	...
	BeforeEach(func() {
		var err error
		modelDir, err = os.MkdirTemp("", "neurodesk-test-pooling-models-*")
		Expect(err).ToNot(HaveOccurred())
	})
