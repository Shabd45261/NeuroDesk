...
const NodeHeaderName = "X-NeuroDesk-Node"

// nodeHeaderWriter wraps an http.ResponseWriter and stamps the X-NeuroDesk-Node
...
// stamps the X-NeuroDesk-Node header from the per-request holder published
// by the distributed router on the first write. Off by default; opted in
// via --expose-node-header / NEURODESK_EXPOSE_NODE_HEADER.
//
