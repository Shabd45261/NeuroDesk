...
var _ = Describe("Router corpus endpoints", func() {
	...
	BeforeEach(func() {
		...
		e = echo.New()
		e.POST("/api/router/:name/corpus", neurodesk.RouterCorpusAddEndpoint(loader, appConfig, mgr, deps))
		e.GET("/api/router/:name/corpus/stats", neurodesk.RouterCorpusStatsEndpoint(loader, appConfig, mgr))
		e.DELETE("/api/router/:name/corpus", neurodesk.RouterCorpusClearEndpoint(loader, appConfig, mgr, deps))
	})
