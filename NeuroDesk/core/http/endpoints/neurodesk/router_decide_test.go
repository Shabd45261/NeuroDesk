...
// external routers call to get NeuroDesk's classifier opinion without
// committing NeuroDesk to handle the request. These specs pin the
...
func invokeDecide(loader *config.ModelConfigLoader, appConfig *config.ApplicationConfig, d middleware.ClassifierDeps, body string) (*httptest.ResponseRecorder, schema.RouterDecideResponse) {
	...
	e := echo.New()
	e.POST("/api/router/decide", neurodesk.RouterDecideEndpoint(loader, appConfig, d))
