...
import (
	...
	"github.com/mudler/NeuroDesk/swagger"
...
var instructionDefs = []instructionDef{
	...
	},
	{
		...
		Intro:       "POST /v1/moderations accepts a text string or array plus a NeuroDesk completion model. NeuroDesk constrains the model to the OpenAI moderation category schema and returns one result per input. Multimodal moderation inputs are not yet supported.",
	},
	{
		Name:        "audio",
	...
	},
	{
		...
		Intro:       "Profiles persist below the NeuroDesk data directory as private PCM-WAV references plus exact transcripts. GET and audio preview require the audio_speech feature; create and delete are admin-only. Pass the returned `voice` URI to /v1/audio/speech or /tts. NeuroDesk resolves the URI at request time and never exposes a filesystem path.",
	},
	{
		Name:        "images",
