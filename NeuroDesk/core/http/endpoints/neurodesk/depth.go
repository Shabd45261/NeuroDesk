...
// DepthEndpoint is the NeuroDesk Depth endpoint exposing the full Depth Anything 3
...
func DepthEndpoint(cl *config.ModelConfigLoader, ml *model.ModelLoader, appConfig *config.ApplicationConfig) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		input, ok := c.Get(middleware.CONTEXT_LOCALS_KEY_NEURODESK_REQUEST).(*schema.DepthRequest)
		if !ok || input.Model == "" {
