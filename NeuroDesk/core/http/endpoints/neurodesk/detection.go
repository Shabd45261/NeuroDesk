...
// DetectionEndpoint is the NeuroDesk Detection endpoint https://localai.io/docs/api-reference/detection
...
func DetectionEndpoint(cl *config.ModelConfigLoader, ml *model.ModelLoader, appConfig *config.ApplicationConfig) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		input, ok := c.Get(middleware.CONTEXT_LOCALS_KEY_NEURODESK_REQUEST).(*schema.DetectionRequest)
		if !ok || input.Model == "" {
