...
var _ = Describe("InstallBackendOnNodeEndpoint async behavior", func() {
	...
	It("returns 202 with a jobID and dispatches a TargetNodeID-scoped op", func() {
		...
		handler := neurodesk.InstallBackendOnNodeEndpoint(nil, galleryService, opcache, appCfg)
	...
	Describe("UpgradeBackendOnNodeEndpoint", func() {
		It("returns 202 with a jobID and dispatches an Upgrade op scoped to the node", func() {
			...
			handler := neurodesk.UpgradeBackendOnNodeEndpoint(galleryService, opcache, appCfg)
			Expect(handler(c)).To(Succeed())
			Expect(rec.Code).To(Equal(http.StatusAccepted))
