...
import (
	...
	"github.com/mudler/NeuroDesk/core/services/cloudproxy"
	"github.com/mudler/NeuroDesk/pkg/functions"
	reason "github.com/mudler/NeuroDesk/pkg/reasoning"

	"github.com/mudler/NeuroDesk/core/templates"
	pb "github.com/mudler/NeuroDesk/pkg/grpc/proto"
...
// system prompt when the NeuroDesk Assistant modality is on. Empty / whitespace
...
func ChatEndpoint(cl *config.ModelConfigLoader, ml *model.ModelLoader, evaluator *templates.Evaluator, startupOptions *config.ApplicationConfig, natsClient mcpTools.MCPNATSClient, assistantHolder *mcpTools.NeuroDeskAssistantHolder, compressor middleware.ChatCompressor) echo.HandlerFunc {
	return func(c echo.Context) error {
		var textContentToReturn string
		id := uuid.New().String()
		created := int(time.Now().Unix())

		input, ok := c.Get(middleware.CONTEXT_LOCALS_KEY_NEURODESK_REQUEST).(*schema.OpenAIRequest)
		if !ok || input.Model == "" {
		...
		// NeuroDesk Assistant modality: an admin opted into the in-process MCP
		...
		assistantMode := mcpTools.NeuroDeskAssistantFromMetadata(input.Metadata)
		if assistantMode {
			...
			if startupOptions.DisableNeuroDeskAssistant {
				return echo.NewHTTPError(http.StatusServiceUnavailable, "NeuroDesk Assistant is disabled on this server")
			}
			if assistantHolder == nil || !assistantHolder.HasTools() {
				return echo.NewHTTPError(http.StatusServiceUnavailable, "NeuroDesk Assistant is not available on this server")
			}
			mcpExecutor = assistantHolder.Executor()
			mcpFuncs, discErr := mcpExecutor.DiscoverTools(c.Request().Context())
			if discErr != nil {
				xlog.Error("Failed to discover NeuroDesk Assistant tools", "error", discErr)
			...
			xlog.Debug("NeuroDesk Assistant tools injected", "count", len(mcpFuncs))
		}
		...
		switch {
		// Generates grammar with internal's NeuroDesk engine
