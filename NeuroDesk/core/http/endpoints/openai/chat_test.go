...
var _ = Describe("applyAutoparserOverride", func() {
	// Regression test for https://github.com/mudler/NeuroDesk/issues/9985.
	// When NeuroDesk templates a <think>-style reasoning model outside of jinja
...
var _ = Describe("system message helpers", func() {
	...
	Describe("stripEmptySystemMessages", func() {
		...
		It("keeps a non-empty system turn", func() {
			in := []schema.Message{
				{Role: "system", Content: "You are NeuroDesk.", StringContent: "You are NeuroDesk."},
				{Role: "user", Content: "hi", StringContent: "hi"},
			}
			out := stripEmptySystemMessages(in)
			Expect(out).To(HaveLen(2))
			Expect(out[0].StringContent).To(Equal("You are NeuroDesk."))
		})
	})
})
