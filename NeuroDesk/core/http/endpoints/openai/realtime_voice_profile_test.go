...
import (
	...
	grpcPkg "github.com/mudler/NeuroDesk/pkg/grpc"
...
var _ = Describe("realtime pipeline voice profiles", func() {
	...
	DescribeTable("returns actionable reference errors",
		...
		Entry("malformed reference", "neurodesk://voice-profiles/not-a-uuid", &config.ModelConfig{}, nil, "invalid voice profile reference"),
		Entry("unsupported model", "neurodesk://voice-profiles/00000000-0000-0000-0000-000000000001", &config.ModelConfig{Backend: "piper"}, nil, "does not support reference-audio voice cloning"),
		Entry("unavailable store", "neurodesk://voice-profiles/00000000-0000-0000-0000-000000000001", &config.ModelConfig{Name: "clone-base", Backend: "qwen3-tts-cpp", TTSConfig: config.TTSConfig{VoiceCloning: ptrTo(true)}}, nil, "voice profile store is unavailable"),
	)

	It("reports a missing profile", func() {
		store := voiceprofile.NewStore(GinkgoT().TempDir())
		DeferCleanup(func() { Expect(store.Close()).To(Succeed()) })
		_, _, release, err := resolveRealtimeVoice(context.Background(), "neurodesk://voice-profiles/00000000-0000-0000-0000-000000000001", &config.ModelConfig{
...
var _ = Describe("realtime session voice switching", func() {
	...
	It("rolls back the model, wrapper, voice, and lease when preparation fails", func(ctx SpecContext) {
		...
		err := update(f, session, &types.RealtimeSession{
			Model: "pipe-b",
			Audio: &types.RealtimeSessionAudio{Output: &types.SessionAudioOutput{Voice: "neurodesk://voice-profiles/00000000-0000-0000-0000-000000000001"}},
		})
	...
	It("releases a candidate profile lease when later validation fails", func(ctx SpecContext) {
		...
		err := update(f, session, &types.RealtimeSession{
			...
			NeuroDeskClassifier: classifierTestConfig(0, nil),
		})
