...
// Classifier mode (NeuroDesk extension): instead of autoregressive
...
func resolveClassifier(sessionCfg *types.ClassifierConfig, overrides *types.ResponseCreateParams) *types.ClassifierConfig {
	if overrides != nil && overrides.NeuroDeskClassifier != nil {
		return overrides.NeuroDeskClassifier
	}
	return sessionCfg
}
...
// emit the neurodesk.classifier.result observability event, then either the
