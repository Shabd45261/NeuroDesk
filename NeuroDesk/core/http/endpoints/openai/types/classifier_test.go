...
var _ = Describe("ClassifierConfig", func() {
	Describe("JSON round-trip", func() {
		...
		It("is carried by RealtimeSession under neurodesk_classifier", func() {
			s := types.RealtimeSession{NeuroDeskClassifier: validClassifier()}
			data, err := json.Marshal(s)
			Expect(err).ToNot(HaveOccurred())
			Expect(string(data)).To(ContainSubstring(`"neurodesk_classifier"`))

			var back types.RealtimeSession
			Expect(json.Unmarshal(data, &back)).To(Succeed())
			Expect(back.NeuroDeskClassifier).ToNot(BeNil())
			Expect(back.NeuroDeskClassifier.Options).To(HaveLen(2))
		})

		It("is carried by ResponseCreateParams under neurodesk_classifier", func() {
			var params types.ResponseCreateParams
			Expect(json.Unmarshal([]byte(`{"neurodesk_classifier":{"enabled":false}}`), &params)).To(Succeed())
			Expect(params.NeuroDeskClassifier).ToNot(BeNil())
			Expect(params.NeuroDeskClassifier.Enabled).ToNot(BeNil())
			Expect(*params.NeuroDeskClassifier.Enabled).To(BeFalse())
		})
	})
	...
	Describe("ClassifierResultEvent", func() {
		It("marshals with the neurodesk.classifier.result type tag", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(string(data)).To(ContainSubstring(`"type":"neurodesk.classifier.result"`))
