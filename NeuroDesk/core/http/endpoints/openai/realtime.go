...
import (
	...
	"github.com/mudler/NeuroDesk/core/http/endpoints/openai/respcoord"
	"github.com/mudler/NeuroDesk/core/http/endpoints/openai/turncoord"
	"github.com/mudler/NeuroDesk/core/http/endpoints/openai/types"
	...
	laudio "github.com/mudler/NeuroDesk/pkg/audio"
	"github.com/mudler/NeuroDesk/pkg/functions"
	"github.com/mudler/NeuroDesk/pkg/grpc"
	"github.com/mudler/NeuroDesk/pkg/grpc/proto"
	model "github.com/mudler/NeuroDesk/pkg/model"
	"github.com/mudler/NeuroDesk/pkg/reasoning"
	"github.com/mudler/NeuroDesk/pkg/sound"
...
type Session struct {
	...
	// NeuroDesk Assistant tool surface. Tool calls whose name matches this
	...
	// event for those. Mirrors the chat handler's metadata.neurodesk_assistant
	// path.
	...
	if s.TranscriptionOnly {
		...
		return types.SessionUnion{
			Realtime: &types.RealtimeSession{
				...
				NeuroDeskClassifier: s.Classifier,
...
type Model interface {
	...
	// continuation of the conversation (NeuroDesk classifier-mode extension)
...
// `metadata.neurodesk_assistant` — admin-only opt-in to the in-process
// management tool surface.
type RealtimeSessionOptions struct {
	NeuroDeskAssistant bool
...
func Realtime(application *application.Application) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		assistantFlag, _ := strconv.ParseBool(c.QueryParam("neurodesk_assistant"))
		opts := RealtimeSessionOptions{
			NeuroDeskAssistant: assistantFlag,
...
func runRealtimeSession(application *application.Application, t Transport, model string, evaluator *templates.Evaluator, opts RealtimeSessionOptions) {
	...
	// NeuroDesk Assistant opt-in: gate on admin (same rule as chat.go's
	...
	if opts.NeuroDeskAssistant {
		if !opts.IsAdmin {
			sendError(t, "forbidden", "neurodesk_assistant requires admin", "", "")
			return
		}
		appCfg := application.ApplicationConfig()
		if appCfg != nil && appCfg.DisableNeuroDeskAssistant {
			sendError(t, "unavailable", "NeuroDesk Assistant is disabled on this server", "", "")
			return
		}
		holder := application.NeuroDeskAssistant()
		if holder == nil || !holder.HasTools() {
			sendError(t, "unavailable", "NeuroDesk Assistant is not available on this server", "", "")
			return
		}
		exec := holder.Executor()
		fns, discErr := exec.DiscoverTools(context.Background())
		if discErr != nil {
			xlog.Error("realtime: failed to discover NeuroDesk Assistant tools", "error", discErr)
		...
		xlog.Debug("realtime: NeuroDesk Assistant tools injected", "count", len(fns))
	}
	...
	for {
		...
		case types.ResponseCreateEvent:
			xlog.Debug("recv", "message", string(msg))
			if err := validateClassifierActivation(session.ModelInterface, e.Response.NeuroDeskClassifier); err != nil {
...
func updateSession(session *Session, update *types.SessionUnion, cl *config.ModelConfigLoader, ml *model.ModelLoader, appConfig *config.ApplicationConfig, evaluator *templates.Evaluator, routing *RealtimeRoutingContext, profiles *voiceprofile.Store) error {
	...
	if rt.NeuroDeskClassifier != nil {
		if err := validateClassifierActivation(candidateModel, rt.NeuroDeskClassifier); err != nil {
	...
	if rt.NeuroDeskClassifier != nil {
		...
		session.Classifier = rt.NeuroDeskClassifier
		prewarmClassifier(session)
	}
...
// calls, the terminal response.done, and — for server-side NeuroDesk Assistant
...
//   - NeuroDesk Assistant tools (session.AssistantExecutor.IsTool) run server-side;
