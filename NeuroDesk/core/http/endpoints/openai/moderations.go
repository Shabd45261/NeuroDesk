...
// API using any NeuroDesk completion model and constrained JSON generation.
...
func moderationEndpoint(generate moderationGenerator) echo.HandlerFunc {
	return func(c echo.Context) error {
		input, ok := c.Get(middleware.CONTEXT_LOCALS_KEY_NEURODESK_REQUEST).(*schema.ModerationRequest)
		if !ok || input == nil {
