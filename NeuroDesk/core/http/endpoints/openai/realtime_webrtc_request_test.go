...
var _ = Describe("decodeRealtimeCallRequest", func() {
	It("decodes the legacy JSON request", func() {
		request := httptest.NewRequest(http.MethodPost, "/v1/realtime/calls", bytes.NewBufferString(`{"sdp":"offer","model":"voice","neurodesk_assistant":true}`))
		request.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)

		req, plainSDPResponse, err := decodeRealtimeCallRequest(echo.New().NewContext(request, httptest.NewRecorder()))

		Expect(err).NotTo(HaveOccurred())
		Expect(req).To(Equal(RealtimeCallRequest{SDP: "offer", Model: "voice", NeuroDeskAssistant: true}))
	...
	It("decodes the OpenAI multipart request", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		_, err = sessionPart.Write([]byte(`{"type":"realtime","model":"voice","neurodesk_assistant":true}`))
		Expect(err).NotTo(HaveOccurred())
