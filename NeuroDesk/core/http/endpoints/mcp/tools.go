...
var (
	...
	client = mcp.NewClient(&mcp.Implementation{Name: "NeuroDesk", Version: "v1.0.0"}, nil)
)
...
// MetadataKeyNeuroDeskAssistant is the request-metadata key the chat handler
...
const MetadataKeyNeuroDeskAssistant = "neurodesk_assistant"

// NeuroDeskAssistantFromMetadata reports whether the request opted into the
// "NeuroDesk Assistant" chat modality (admin in-process MCP tool surface).
// The MetadataKeyNeuroDeskAssistant key is consumed so it doesn't leak to
// the backend. Truthy values: "1", "true", "yes" (case-insensitive).
func NeuroDeskAssistantFromMetadata(metadata map[string]string) bool {
	raw, ok := metadata[MetadataKeyNeuroDeskAssistant]
	if !ok {
		return false
	}
	delete(metadata, MetadataKeyNeuroDeskAssistant)
...
// forever" (mudler/NeuroDesk#10880) - most visibly for cloud-proxy models, whose
