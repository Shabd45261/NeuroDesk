...
test.describe('Models Explore - the view scrolls, not the page', () => {
  ...
  test('a long detail scrolls the pane and leaves the page height alone', async ({ page }) => {
    ...
    // Let the detail's lazy artwork finish: a resizeable backdrop that is still
    // loading changes the pane's content height, so measuring mid-load races
    // the very assertion this file exists for.
    await page.locator('.split-view__pane img').evaluateAll((imgs) =>
