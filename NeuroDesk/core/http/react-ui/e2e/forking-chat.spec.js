...
async function seedChat(page, history) {
  await page.addInitScript((h) => {
    if (localStorage.getItem('neurodesk_chats_data')) return
    const chat = {
    ...
    localStorage.setItem('neurodesk_chats_data', JSON.stringify({
...
test('saved message edits persist without sending a completion request', async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => {
    const data = JSON.parse(localStorage.getItem('neurodesk_chats_data'))
