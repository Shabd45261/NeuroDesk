...
const profile = {
  ...
  voice: `neurodesk://voice-profiles/${VOICE_ID}`,
...
test.describe('Voice Library', () => {
  ...
  test('shows inline API usage and installed model compatibility', async ({ page }) => {
    ...
    await expect(apiHelp.locator('code')).toContainText(`neurodesk://voice-profiles/${VOICE_ID}`)
  })
  ...
  test('offers server-declared gallery models when none are installed', async ({ page }) => {
    ...
    await expect.poll(() => installedModel).toBe('neurodesk@omnivoice-cpp')
  ...
  test('passes the stable voice URI from the library into TTS', async ({ page }) => {
    ...
    await expect.poll(() => ttsBody?.voice).toBe(`neurodesk://voice-profiles/${VOICE_ID}`)
