...
// The use-case chips live inline in the refine band; these specs never need
// to open anything to reach them.
...
test.describe("Models Gallery - Backend Features", () => {
  ...
  test("expanded row shows backend in detail", async ({ page }) => {
    ...
    // The detail view should show Backend label and value. Scoped to a table
    // cell: the rail-column DiskUsage panel lends the pane a "Backends" row of
    // its own, so an unscoped `text=Backend` matches two elements.
    const detail = page.locator(PANE);
    await expect(page.locator(PANE + " table td", { hasText: /^Backend$/ }).first()).toBeVisible();
...
test.describe("Models Gallery - Collapsed Listing", () => {
  ...
  test("a legacy '0' in storage is not read as a choice", async ({ page }) => {
    ...
    await page.evaluate(() => {
      localStorage.setItem("neurodesk-models-collapse-variants-filter", "0");
    });
    await page.reload();
    await railReady(page);
...
// The filter block is three deliberate bands in their own column (the refine
// sidebar): query scope (search + backend select), the use-case chip rows, and
// the refinements (collapse / fits-in-GPU / CPU-only + context). These assert
// the separation holds, because the regression they guard against is the
// refinements being swept back into the chip row's wrap, where their position
// depends on how many chips happen to wrap at the current width.
test.describe("Models Gallery - Filter layout structure", () => {
  ...
  test("the backend select sits above the use-case control it gates", async ({
    ...
    const taxonomy = page.locator(".models-refine__taxonomy");
    await expect(taxonomy).toBeVisible();
    // Picking a backend disables the use cases it cannot serve, so it still
    // reads first even though both now stack inside the refine sidebar.
    const selectBox = await selectBtn.boundingBox();
    const taxonomyBox = await taxonomy.boundingBox();
    expect(selectBox.y).toBeLessThan(taxonomyBox.y);
  });
  ...
  test("refinements stay grouped and on one band at a narrow width", async ({
    ...
    const refine = page.getByTestId("models-filters-refine");
    await expect(refine).toBeVisible();
    // One synchronous pass so both rects come from the same frame: two
    // boundingBox() calls race with lazy-refined chips between their reads and
    // flake ~23px on a busy page, while no real reflow happens at all.
    const { refineTop, taxonomyBottom } = await page.evaluate(() => {
      const r = document.querySelector('[data-testid="models-filters-refine"]');
      const t = document.querySelector(".models-refine__taxonomy");
      return { refineTop: r.getBoundingClientRect().top, taxonomyBottom: t.getBoundingClientRect().bottom };
    });
    // Below the use-case chips, not interleaved with them.
    expect(refineTop).toBeGreaterThanOrEqual(taxonomyBottom - 1);
    await expect(refine.getByText("Fits in GPU")).toBeVisible();
    await expect(refine.locator("#models-context-size")).toBeVisible();
  });
...
test.describe("Models Gallery - Explore split view", () => {
  ...
  test("the rail moves the selection from the keyboard", async ({ page }) => {
    ...
    const current = () => page.locator('[data-entity][aria-current="true"]').getAttribute("data-entity");
    await page.keyboard.press("ArrowDown");
    // Let the moved selection commit to the DOM. Firing ArrowUp before React
    // flushes the Down press re-reads the stale selectedId and jumps to the row
    // above the OLD entry instead of coming back to it.
    await expect.poll(current).not.toBe("llama-model");
    await expect(page).not.toHaveURL(/[?&]model=llama-model(&|$)/);
    await expect(page.locator('[data-testid="discover-back"]')).toBeVisible();

    await page.keyboard.press("ArrowUp");
    await expect.poll(current).toBe("llama-model");
    await expect(page).toHaveURL(/[?&]model=llama-model(&|$)/);
  });
});

// Host panel, download strip and per-model execution mode. These are the three
// things the three-column discover layout adds beyond a renamed SplitView: the
// rail column answers "what is happening", the pane answers "what am I running
// on", and a build the backend declares CPU-capable can be pinned to CPU RAM
// without ever being told it will silently fall back to the GPU.
test.describe("Models Gallery - Host panel, downloads strip, CPU mode", () => {
  test.beforeEach(async ({ page }) => {
    // One build is declared CPU-capable by the backend, the rest are not, so a
    // test can show both the enabled and the disabled toggles.
    const models = MOCK_MODELS_RESPONSE.models.map((m) =>
      m.name === "llama-model" ? { ...m, runs_on_cpu: true } : m);
    await page.route("**/api/models*", (route) =>
      route.fulfill({ contentType: "application/json", body: JSON.stringify({ ...MOCK_MODELS_RESPONSE, models }) }));
    // Registered after the list route: Playwright matches the newest route
    // first, so the estimate call gets the multi-context payload the VRAM
    // chart needs while the list route still owns /api/models.
    await page.route("**/api/models/estimate/*", (route) => {
      const url = new URL(route.request().url());
      const id = decodeURIComponent(url.pathname.split("/").pop() || "");
      route.fulfill({
    ...
    await page.route("**/api/resources", (route) =>
      route.fulfill({
        ...
        body: JSON.stringify({
          ...
          storage_size: 150 * 1024 * 1024 * 1024,
        }),
      }));
    // One active model install, so the rail column has a strip to show.
  ...
  test("with nothing selected the pane hosts the hardware and disk panels", async ({ page }) => {
    const host = page.getByTestId("models-your-host");
    await expect(host).toBeVisible();
    await expect(host).toContainText("Mock GPU");
    await expect(host).toContainText("System RAM");

    const disk = page.getByTestId("models-disk");
    await expect(disk).toBeVisible();
    await expect(disk).toContainText("150 GB");
  });

  test("disk usage states only what this host reports", async ({ page }) => {
    const disk = page.getByTestId("models-disk");
    await expect(disk).toBeVisible();
    // The models store is real; backends, cache and free space are not
    // separated by this host's API and must read as unknown, never guessed.
    await expect(disk.locator(".resource-summary-row").nth(1)).toContainText("—");
    await expect(disk).toContainText("not reported by the API");
  });

  test("active model installs surface in the rail column", async ({ page }) => {
    const strip = page.getByTestId("models-downloads");
    await expect(strip).toBeVisible();
    await expect(strip).toContainText("model-a");
    await expect(strip.getByRole("progressbar")).toHaveAttribute("aria-valuenow", "42");
    await expect(strip.getByRole("button", { name: "Pause model-a" })).toBeVisible();
    await expect(strip.getByRole("button", { name: "Cancel model-a" })).toBeVisible();
  });

  test("a CPU-capable build can be pinned to CPU and keeps its framing", async ({ page }) => {
    await railItem(page, "llama-model").click();
    await expect(page.locator('[data-testid="discover-back"]')).toBeVisible();

    const execMode = page.getByTestId("discover-exec-mode");
    await expect(execMode).toBeVisible();
    // Default frame is the GPU one: the VRAM curve is on the page.
    await expect(page.locator(".discover__chart")).not.toHaveCount(0);

    await execMode.locator(".toggle").click();
    // The CPU frame reports RAM in place of VRAM and says the GPU is unused.
    await expect(page.locator(PANE)).toContainText("RAM at 8K");
    await expect(page.locator(PANE)).toContainText("GPU not used");
    // The VRAM curve compares against this host's GPU, so it has no place.
    await expect(page.locator(".discover__chart")).toHaveCount(0);

    // Pinned in local storage, so a reload remembers the frame.
    await page.reload();
    await railReady(page);
    await railItem(page, "llama-model").click();
    await expect(page.locator('[data-testid="discover-back"]')).toBeVisible();
  ...
  test("a build the backend declares GPU-only cannot be pinned to CPU", async ({ page }) => {
    await railItem(page, "whisper-model").click();
    const execMode = page.getByTestId("discover-exec-mode");
    await expect(execMode).toBeVisible();
    await expect(execMode.locator("input[type=checkbox]")).toBeDisabled();
  });

  test("CPU-only narrows the gallery request to declared-capable builds", async ({ page }) => {
    let cpuRequests = 0;
    await page.unroute("**/api/models*");
    await page.route("**/api/models*", (route) => {
      if (route.request().url().includes("capability=cpu")) cpuRequests += 1;
      route.fulfill({ contentType: "application/json", body: JSON.stringify(MOCK_MODELS_RESPONSE) });
    });
    await page.locator('label.filter-bar-group__toggle', { hasText: "CPU only" }).click();
    await expect.poll(() => cpuRequests).toBeGreaterThan(0);
  });
});
