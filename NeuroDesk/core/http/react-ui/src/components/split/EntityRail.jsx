...
//
// items:  [{ id, name, icon, meta, metaTone, stripe, badge, groupId }]
// groups: [{ id, label, icon }] - pass null, or grouped=false, for a flat list.
//   metaTone: 'ok' | 'bad' | 'warn' | 'busy'
//   badge:    a short inline qualifier that sits next to the name (e.g. "CPU")
  ...
  return (
    ...
    >
      ...
      <span className="entity-rail__main">
        <span className="entity-rail__name">
          {item.name}
          {item.badge && <span className="entity-rail__badge">{item.badge}</span>}
        </span>
        {item.meta && (
