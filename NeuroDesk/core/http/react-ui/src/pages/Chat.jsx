...
const FOCUS_MODE_KEY = 'neurodesk_chat_focus_mode'
      ...
      try {
        ...
        if (hasMessage || wantsAssistant) {
          ...
          if (wantsAssistant && targetChat) {
            updateChatSettings(targetChat.id, { neurodeskAssistant: true })
          }
          if (hasMessage) {
  ...
  useEffect(() => {
    if (focusActive) {
      if (prevAppCollapseRef.current === null) {
        try {
          prevAppCollapseRef.current = localStorage.getItem('neurodesk_sidebar_collapsed') === 'true'
          ...
          <div className="chat-settings-drawer-body">
            {isAdmin && (
              <div className="form-group chat-settings-toggle-row">
                ...
                <Toggle
                  checked={!!activeChat.neurodeskAssistant}
                  onChange={(next) => updateChatSettings(activeChat.id, { neurodeskAssistant: next })}
                />
              </div>
            )}
            <div className="form-group chat-settings-toggle-row">
        ...
        <div className="chat-messages" ref={messagesRef}>
          {activeChat.history.length === 0 && !isStreaming && (
            <div className="chat-empty-state">
              <h2 className="chat-empty-title">{activeChat.neurodeskAssistant ? t('empty.manageTitle') : t('empty.startTitle')}</h2>
