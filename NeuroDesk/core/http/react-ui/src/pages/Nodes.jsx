    ...
    <div className="card pad-lg mb-xl">
      ...
      <div className="stack">
        <div>
          ...
          <CommandBlock
            command={`neurodesk ${workerCmd} \\\n  --register-to "${frontendUrl}" \\\n  --nats-url "nats://nats:4222" \\\n  --registration-token "$NEURODESK_REGISTRATION_TOKEN"`}
            addToast={addToast}
          />
        </div>
        <div>
          ...
          <CommandBlock
            command={`docker run --net host ${flagsStr}\\\n  -e NEURODESK_REGISTER_TO="${frontendUrl}" \\\n  -e NEURODESK_NATS_URL="nats://nats:4222" \\\n  -e NEURODESK_REGISTRATION_TOKEN="$TOKEN" \\\n  ${dockerImage(option, dev)} ${workerCmd}`}
            addToast={addToast}
          />
        </div>
      </div>
          ...
          <div className="stack">
            <div className="hstack">
              ...
              <div className="flex-1">
                <p className="fw-medium mb-xs">Start NeuroDesk with distributed mode</p>
                <CommandBlock
                  command={`neurodesk run --distributed \\\n  --distributed-db "postgres://user:pass@host/db" \\\n  --distributed-nats "nats://host:4222"`}
                  addToast={addToast}
                />
              </div>
            </div>
            <div className="hstack">
              ...
              <div className="flex-1">
                ...
                <CommandBlock
                  command={`neurodesk worker \\\n  --register-to "http://neurodesk-host:8080" \\\n  --nats-url "nats://nats:4222" \\\n  --node-name "gpu-node-1"`}
                  addToast={addToast}
                />
              </div>
            </div>
