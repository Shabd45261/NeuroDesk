...
export default function Home() {
  ...
  const [assistantUsed, setAssistantUsed] = useState(() => {
    try { return localStorage.getItem('neurodesk_assistant_used') === '1' } catch { return false }
  })
  ...
  useEffect(() => {
    fetch(apiUrl('/api/features'))
      ...
      .then(data => {
        ...
        setAssistantAvailable(!!data.neurodesk_assistant)
      })
      .catch(() => {})
  }, [])
  ...
  const doSubmit = useCallback(() => {
    ...
    localStorage.setItem('neurodesk_index_chat_data', JSON.stringify(chatData))
  ...
  const openAssistantChat = useCallback(() => {
    const chatData = {
      model: selectedModel || '',
      mcpMode: false,
      neurodeskAssistant: true,
      newChat: true,
    }
    localStorage.setItem('neurodesk_index_chat_data', JSON.stringify(chatData))
    try { localStorage.setItem('neurodesk_assistant_used', '1') } catch { /* ignore */ }
    ...
    <div className="home-page">
      {hasModels ? (
        <>
          ...
          {/* NeuroDesk Assistant — prominent CTA on first run. Once the
