...
var _ = Describe("availableModelMemory", func() {
	...
	stateWithCapability := func(capability string, vram uint64) *system.SystemState {
		previous, had := os.LookupEnv("NEURODESK_FORCE_META_BACKEND_CAPABILITY")
		Expect(os.Setenv("NEURODESK_FORCE_META_BACKEND_CAPABILITY", capability)).To(Succeed())
		DeferCleanup(func() {
			if had {
				Expect(os.Setenv("NEURODESK_FORCE_META_BACKEND_CAPABILITY", previous)).To(Succeed())
				return
			}
			Expect(os.Unsetenv("NEURODESK_FORCE_META_BACKEND_CAPABILITY")).To(Succeed())
		})
