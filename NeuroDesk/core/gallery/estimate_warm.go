...
import (
	...
	"github.com/mudler/NeuroDesk/pkg/concurrency"
...
//
//	NEURODESK_VRAM_WARM_LIMIT        entries to warm; 0 disables the warm-up
//	NEURODESK_VRAM_WARM_CONCURRENCY  estimates in flight at once
//
...
func EstimateWarmConfigFromEnv() EstimateWarmConfig {
	...
	if v, ok := os.LookupEnv("NEURODESK_VRAM_WARM_LIMIT"); ok {
		if n, err := strconv.Atoi(strings.TrimSpace(v)); err == nil && n >= 0 {
			cfg.Limit = n
		}
	}
	if v, ok := os.LookupEnv("NEURODESK_VRAM_WARM_CONCURRENCY"); ok {
		if n, err := strconv.Atoi(strings.TrimSpace(v)); err == nil && n > 0 {
			cfg.Concurrency = n
		}
	}
