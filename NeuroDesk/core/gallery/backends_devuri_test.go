...
var _ = Describe("developmentURI", func() {
	...
	It("rewrites a released image to its branch (development) image", func() {
		got, ok := developmentURI("quay.io/go-skynet/neurodesk-backends:latest-metal-darwin-arm64-llama-cpp", latest, master)
		Expect(ok).To(BeTrue())
		Expect(got).To(Equal("quay.io/go-skynet/neurodesk-backends:master-metal-darwin-arm64-llama-cpp"))
	})

	It("leaves an image already on the branch tag untouched", func() {
		_, ok := developmentURI("quay.io/go-skynet/neurodesk-backends:master-metal-darwin-arm64-llama-cpp", latest, master)
