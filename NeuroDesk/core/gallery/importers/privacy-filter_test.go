...
import (
	...
	hfapi "github.com/mudler/NeuroDesk/pkg/huggingface-api"
...
var _ = Describe("PrivacyFilterImporter", func() {
	...
	Context("detection (Match)", func() {
		It("matches an HF repo shipping a privacy-filter GGUF", func() {
			d := privacyFilterDetails("huggingface://NeuroDesk-io/privacy-filter-multilingual-GGUF", "",
		...
		It("matches the GGUF distribution repo by name when HF metadata is absent", func() {
			d := importers.Details{URI: "huggingface://NeuroDesk-io/privacy-filter-multilingual-GGUF", Preferences: json.RawMessage("")}
			Expect(imp.Match(d)).To(BeTrue())
		})
	...
	Context("import (Import)", func() {
		It("emits a privacy-filter token_classify config from an HF GGUF repo", func() {
			d := privacyFilterDetails("huggingface://NeuroDesk-io/privacy-filter-multilingual-GGUF", `{"name":"pii"}`,
