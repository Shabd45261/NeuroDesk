...
var _ = Describe("DiscoverModelConfig", func() {
	...
	Context("With only a repository URI", func() {
		...
		It("should discover and import using LlamaCPPImporter", func() {
			uri := "https://huggingface.co/mudler/NeuroDesk-functioncall-qwen2.5-7b-v0.5-Q4_K_M-GGUF"
			...
			Expect(modelConfig.Name).To(Equal("neurodesk-functioncall-qwen2.5-7b-v0.5-q4_k_m"), fmt.Sprintf("Model config: %+v", modelConfig))
			Expect(modelConfig.Description).To(Equal("Imported from https://huggingface.co/mudler/NeuroDesk-functioncall-qwen2.5-7b-v0.5-Q4_K_M-GGUF"), fmt.Sprintf("Model config: %+v", modelConfig))
			...
			Expect(modelConfig.Files[0].Filename).To(Equal("llama-cpp/models/neurodesk-functioncall-qwen2.5-7b-v0.5-q4_k_m/neurodesk-functioncall-qwen2.5-7b-v0.5-q4_k_m.gguf"), fmt.Sprintf("Model config: %+v", modelConfig))
			Expect(modelConfig.Files[0].URI).To(Equal("https://huggingface.co/mudler/NeuroDesk-functioncall-qwen2.5-7b-v0.5-Q4_K_M-GGUF/resolve/main/neurodesk-functioncall-qwen2.5-7b-v0.5-q4_k_m.gguf"), fmt.Sprintf("Model config: %+v", modelConfig))
...
var _ = Describe("audio-cpp importer registration", func() {
	...
	// core/http/endpoints/neurodesk/backend_test.go, against the /backends/known
