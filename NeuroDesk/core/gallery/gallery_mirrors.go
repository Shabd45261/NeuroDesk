...
import (
	...
	"github.com/mudler/NeuroDesk/pkg/xsync"
...
// come from the operator's own gallery configuration (NEURODESK_GALLERIES or the
...
// somewhere NeuroDesk should be dropping files. Only an absolute models
...
func persistGalleryIndex(basePath, url string, body []byte) {
	...
	// 0o750: the cache is NeuroDesk's own bookkeeping, so nothing outside the
