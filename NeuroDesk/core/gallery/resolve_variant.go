...
// list comes from pkg/system, so teaching NeuroDesk about a new serving feature
...
// a NeuroDesk-level vocabulary the gallery curators control and they mean the
...
func SelectVariant(options []VariantOption, env ResolveEnv, pin string) (VariantSelection, error) {
	...
	for i := range options {
		...
		// would make an entry every older NeuroDesk installs fine uninstallable on
		// newer ones.
		if !o.IsBase {
