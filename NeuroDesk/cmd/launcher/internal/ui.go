...
func NewLauncherUI() *LauncherUI {
	...
	return &LauncherUI{
		...
		startStopButton:   widget.NewButton("Start NeuroDesk", nil),
		...
		autoStartCheck:    widget.NewCheck("Start NeuroDesk when the launcher opens", nil),
		startOnBootCheck:  widget.NewCheck("Start NeuroDesk on system boot", nil),
...
func (ui *LauncherUI) CreateMainUI(launcher *Launcher) *fyne.Container {
	...
	tabs := container.NewVBox(
		widget.NewCard("NeuroDesk Launcher Settings", "", configTab),
	)
...
func (ui *LauncherUI) setupBindings() {
	...
	ui.webUIButton.Disable() // Disabled until NeuroDesk is running
...
// startNeuroDesk starts the NeuroDesk service
func (ui *LauncherUI) startNeuroDesk() {
	fyne.Do(func() {
		ui.startStopButton.Disable()
	})
	ui.UpdateStatus("Starting NeuroDesk...")

	go func() {
		err := ui.launcher.StartNeuroDesk()
		if err != nil {
			...
			fyne.Do(func() {
				ui.startStopButton.SetText("Stop NeuroDesk")
...
// stopNeuroDesk stops the NeuroDesk service
func (ui *LauncherUI) stopNeuroDesk() {
	fyne.Do(func() {
		ui.startStopButton.Disable()
	})
	ui.UpdateStatus("Stopping NeuroDesk...")

	go func() {
		err := ui.launcher.StopNeuroDesk()
		if err != nil {
			fyne.DoAndWait(func() {
				dialog.ShowError(err, ui.launcher.window)
			})
			...
			fyne.Do(func() {
				ui.startStopButton.SetText("Start NeuroDesk")
...
// UpdateRunningState updates UI based on NeuroDesk running state
...
func (ui *LauncherUI) ShowWelcomeWindow() {
	...
	fyne.DoAndWait(func() {
		...
		welcomeWindow := ui.launcher.app.NewWindow("Welcome to NeuroDesk Launcher")
		...
		// Title
		titleLabel := widget.NewLabel("Welcome to NeuroDesk Launcher!")
		titleLabel.TextStyle = fyne.TextStyle{Bold: true}
		titleLabel.Alignment = fyne.TextAlignCenter

		// Welcome message
		welcomeText := `NeuroDesk Launcher makes it easy to run NeuroDesk on your system.

What you can do:
• Start and stop NeuroDesk server
• Configure models and backends paths
• Set environment variables
• Check for updates automatically
• Access NeuroDesk WebUI when running

Getting Started:
1. Configure your models and backends paths
2. Click "Start NeuroDesk" to begin
...
4. Check the system tray for quick access`
		...
		githubButton := widget.NewButton("🐙 GitHub Repository", func() {
			ui.openURL("https://github.com/mudler/NeuroDesk")
		})
