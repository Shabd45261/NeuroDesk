...
func (sm *SystrayManager) setupMenu(desk desktop.App) {
	...
	sm.startStopItem = fyne.NewMenuItem("Start NeuroDesk", func() {
		sm.toggleNeuroDesk()
	})
...
// toggleNeuroDesk starts or stops NeuroDesk based on current state
func (sm *SystrayManager) toggleNeuroDesk() {
	if sm.launcher.IsRunning() {
		go func() {
			if err := sm.launcher.StopNeuroDesk(); err != nil {
				log.Printf("Failed to stop NeuroDesk: %v", err)
				sm.showErrorDialog("Failed to Stop NeuroDesk", err.Error())
			}
		}()
	} else {
		go func() {
			if err := sm.launcher.StartNeuroDesk(); err != nil {
				log.Printf("Failed to start NeuroDesk: %v", err)
...
// openWebUI opens the NeuroDesk WebUI in the default browser
func (sm *SystrayManager) openWebUI() {
	if !sm.launcher.IsRunning() {
		return // NeuroDesk is not running
	}
...
// openDocumentation opens the NeuroDesk documentation
...
func (sm *SystrayManager) recreateMenu() {
	...
	// Determine the action based on NeuroDesk installation and running state
	var actionItem *fyne.MenuItem
	if !sm.launcher.GetReleaseManager().IsNeuroDeskInstalled() {
		// NeuroDesk not installed - show install option
		actionItem = fyne.NewMenuItem("📥 Install Latest Version", func() {
			sm.launcher.showDownloadNeuroDeskDialog()
		})
	} else if sm.launcher.IsRunning() {
		// NeuroDesk is running - show stop option
		actionItem = fyne.NewMenuItem("🛑 Stop NeuroDesk", func() {
			sm.toggleNeuroDesk()
		})
	} else {
		// NeuroDesk is installed but not running - show start option
		actionItem = fyne.NewMenuItem("▶️ Start NeuroDesk", func() {
			sm.toggleNeuroDesk()
		})
	}
	...
	// Only show version if NeuroDesk is installed
	if sm.launcher.GetReleaseManager().IsNeuroDeskInstalled() {
	...
	// Only show WebUI option if NeuroDesk is installed
	if sm.launcher.GetReleaseManager().IsNeuroDeskInstalled() && sm.launcher.IsRunning() {
		menuItems = append(menuItems,
	...
	menu := fyne.NewMenu("NeuroDesk", menuItems...)
...
func (sm *SystrayManager) showStatusDetails(status, version string) {
	fyne.DoAndWait(func() {
		...
		statusWindow := sm.app.NewWindow("NeuroDesk Status Details")
...
func (sm *SystrayManager) showStartupErrorDialog(err error) {
	fyne.DoAndWait(func() {
		...
		// Create error window
		errorWindow := sm.app.NewWindow("NeuroDesk Startup Failed")
		errorWindow.Resize(fyne.NewSize(600, 500))
		errorWindow.CenterOnScreen()

		// Error message
		errorLabel := widget.NewLabel(fmt.Sprintf("Failed to start NeuroDesk:\n%s", err.Error()))
		...
		retryButton := widget.NewButton("Retry", func() {
			...
			// Try to start again
			go func() {
				if retryErr := sm.launcher.StartNeuroDesk(); retryErr != nil {
...
func (sm *SystrayManager) showDownloadProgress(version string) {
	sm.launcher.showDownloadProgressWindow(version, fmt.Sprintf("Downloading NeuroDesk version %s", version), func(win fyne.Window) {
		dialog.ShowConfirm("Update Downloaded",
			"NeuroDesk has been updated successfully. Please restart the launcher to use the new version.",
			func(restart bool) {
				if restart {
					sm.app.Quit()
				}
