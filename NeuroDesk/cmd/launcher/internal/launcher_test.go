...
var _ = Describe("Launcher", func() {
	...
	BeforeEach(func() {
		...
		app := app.NewWithID("com.neurodesk.launcher")
	...
	Describe("Process Management", func() {
		...
		It("should handle start when binary doesn't exist", func() {
			err := launcherInstance.StartNeuroDesk()
			Expect(err).To(HaveOccurred())
			// Could be either "not found" or "permission denied" depending on test environment
			errMsg := err.Error()
			hasExpectedError := strings.Contains(errMsg, "NeuroDesk binary") ||
		...
		It("should handle stop when not running", func() {
			err := launcherInstance.StopNeuroDesk()
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("NeuroDesk is not running"))
		})
	})
