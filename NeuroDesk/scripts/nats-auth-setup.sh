...
#
# NeuroDesk workers receive their own JWT and user seed when registering
# with the frontend.
...
OPERATOR="${NATS_OPERATOR_NAME:-neurodesk-operator}"
ACCOUNT="${NATS_ACCOUNT_NAME:-neurodesk}"
SYSTEM_ACCOUNT="${NATS_SYSTEM_ACCOUNT_NAME:-SYS}"
SERVICE_USER="${NATS_SERVICE_USER:-neurodesk-frontend}"
OUTPUT_DIR="${NATS_KEYS_DIR:-./nats-keys}"

CREDS_FILE="$OUTPUT_DIR/${SERVICE_USER}.creds"
ENV_FILE="$OUTPUT_DIR/neurodesk-nats.env"
...
# Create the NeuroDesk account if it does not exist.
...
# Generate the environment file consumed by the NeuroDesk frontend.
cat >"$ENV_FILE" <<EOF
NEURODESK_NATS_ACCOUNT_SEED=$ACCOUNT_SEED
NEURODESK_NATS_SERVICE_JWT=$SERVICE_JWT
NEURODESK_NATS_SERVICE_SEED=$SERVICE_SEED
EOF
...
echo
echo "=== NeuroDesk NATS JWT setup complete ==="
echo
echo "NeuroDesk environment: $ENV_FILE"
...
echo
echo "=== NeuroDesk NATS environment ==="
cat "$ENV_FILE"
