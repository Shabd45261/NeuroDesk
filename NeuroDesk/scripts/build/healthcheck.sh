#!/bin/bash
# Docker HEALTHCHECK command for the NeuroDesk image.
#
...
# `curl -f http://localhost:8080/readyz` therefore reported every `neurodesk
...
# configured in practice (compose/k8s set NEURODESK_ADDRESS, NEURODESK_SERVE_ADDR,
# ...). If you instead pass the bind address as a CLI flag, set
# HEALTHCHECK_ENDPOINT to match.
set -u

# Detect the arguments neurodesk was started with. PID 1 is the usual case
# (entrypoint.sh exec's neurodesk, so it inherits PID 1), but `init: true` puts
...
detect_argv() {
    if [ -n "${NEURODESK_HEALTHCHECK_ARGV:-}" ]; then
        printf '%s' "$NEURODESK_HEALTHCHECK_ARGV"
        return
    fi

    local cmdline proc pid d
    # NEURODESK_HEALTHCHECK_PROC exists so the regression test can point this at a
    # fixture tree; nothing in the image sets it.
    local procfs="${NEURODESK_HEALTHCHECK_PROC:-/proc}"
    # PID 1 first: entrypoint.sh exec's neurodesk, so it normally *is* PID 1.
    ...
    # somehow has more than one neurodesk process. The healthcheck's own shell is
...
# The mode is the first non-flag word after the neurodesk binary.
#
# `run` is kong's default command, and it is declared `default:"withargs"` — so
# `neurodesk gemma-4 whisper` is the *frontend* with two model arguments, not a
...
if [ -z "$endpoint" ]; then
    ...
    case "$mode" in
        worker)
            ...
            # /healthz) binds NEURODESK_HTTP_ADDR when set, otherwise the gRPC
            # base port minus one. See Config.resolveHTTPAddr.
            port=$(port_of "${NEURODESK_HTTP_ADDR:-}")
            if [ -z "$port" ]; then
                base=$(port_of "${NEURODESK_SERVE_ADDR:-}")
                port=$(( ${base:-50051} - 1 ))
            fi
        ...
        *)
            ...
            # default command is `run`, so a bare `neurodesk`, `neurodesk --flag`
            # and `neurodesk <model-name>` are all frontends on the API port.
            port=$(port_of "${NEURODESK_ADDRESS:-${ADDRESS:-}}")
