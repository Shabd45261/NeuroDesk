...
# an OCI tar that `neurodesk backends install` can consume.
#
...
IMAGE_NAME="${IMAGE_NAME:-neurodesk/privacy-filter-darwin}"
...
# Build an OCI image tar (with manifest.json) that `neurodesk backends install`
# can consume - mirrors ds4-darwin.sh.
PLATFORMARCH="${PLATFORMARCH:-darwin/arm64}"

./neurodesk util create-oci-image \
        build/darwin/. \
