...
Thank you for your interest in contributing to NeuroDesk! We appreciate your time and effort in helping to improve our project. Before you get started, please take a moment to review these guidelines.
...
1. **Clone the repository:**
   ...
   ```bash
   git clone https://github.com/mudler/NeuroDesk.git
   cd NeuroDesk
   ```
...
2. **Build NeuroDesk:**
   ...
   This runs protobuf generation, installs Go tools, builds the React UI, and compiles the `neurodesk` binary. Key build variables you can set:
...
NeuroDesk is configured primarily through environment variables (or equivalent CLI flags). The most useful ones for development are:

| Variable | Description | Default |
|---|---|---|
| `NEURODESK_DEBUG` | Enable debug mode | `false` |
| `NEURODESK_LOG_LEVEL` | Log verbosity (`error`, `warn`, `info`, `debug`, `trace`) | — |
| `NEURODESK_LOG_FORMAT` | Log format (`default`, `text`, `json`) | `default` |
| `NEURODESK_MODELS_PATH` | Path to model files | `./models` |
| `NEURODESK_BACKENDS_PATH` | Path to backend binaries | `./backends` |
| `NEURODESK_CONFIG_DIR` | Directory for dynamic config files (API keys, external backends) | `./configuration` |
| `NEURODESK_THREADS` | Number of threads for inference | — |
| `NEURODESK_ADDRESS` | Bind address for the API server | `:8080` |
| `NEURODESK_API_KEY` | API key(s) for authentication | — |
| `NEURODESK_CORS` | Enable CORS | `false` |
| `NEURODESK_DISABLE_WEBUI` | Disable the web UI | `false` |
...
If you find a bug, have a feature request, or encounter any issues, please check the [issue tracker](https://github.com/mudler/NeuroDesk/issues) to see if a similar issue has already been reported. If not, feel free to [create a new issue](https://github.com/mudler/NeuroDesk/issues/new) and provide as much detail as possible.
...
NeuroDesk follows the **same guidelines as the Linux kernel project** for AI-assisted contributions: <https://docs.kernel.org/process/coding-assistants.html>.
...
- Contributions must remain compatible with NeuroDesk's **MIT License**.
...
The e2e tests run NeuroDesk in a Docker container and exercise the API:
...
These tests build a standard NeuroDesk Docker image and run it with pre-configured model configs to verify that most endpoints work correctly:

```bash
# Build the NeuroDesk docker image
...
We welcome contributions to the documentation. Please open a new PR or create a new issue. The documentation is available under `docs/` https://github.com/mudler/NeuroDesk/tree/master/docs

### Gallery YAML Schema

NeuroDesk provides a JSON Schema for gallery model YAML files at:
...
- Open a new discussion at [Discussion](https://github.com/mudler/NeuroDesk/discussions)
